...
class PipelineStatistics extends React.Component<StageStatisticsProps, StageStatisticsState> {
    static getStages(queryInfo): Map<string, StageNodeInfo> {
        const stages: Map<string, StageNodeInfo> = new Map();
        PipelineStatistics.flattenStage(queryInfo.outputStage, stages);
        return stages;
    }

    static flattenStage(stageInfo, result) {
        stageInfo.subStages.forEach(function (stage) {
            PipelineStatistics.flattenStage(stage, result);
        });

        const nodes = new Map();
        PipelineStatistics.flattenNode(result, JSON.parse(stageInfo.plan.relNodeJson).rels, nodes);
...
export class PipelinePlan extends React.Component<LivePlanProps, LivePlanState> {
    timeoutId: TimeoutID;
    ...
    updateD3Stage(stage: StageNodeInfo, graph: any, allStages: Map<string, StageNodeInfo>) {
        ...
        const html = ReactDOMServer.renderToString(<PipelineStatistics key={stage.id} stage={stage}/>);
    ...
    updateD3Graph() {
        ...
        const stages = PipelineStatistics.getStages(this.state.query);
        ...
        svg.selectAll("g.cluster").on("click", PipelinePlan.handleStageClick);
