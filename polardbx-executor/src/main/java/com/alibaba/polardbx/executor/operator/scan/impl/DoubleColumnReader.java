...
public class DoubleColumnReader extends AbstractColumnReader {
    ...
    public DoubleColumnReader(int columnId, boolean isPrimaryKey, StripeLoader stripeLoader,
    ...
    private void skip(long numValues) throws IOException {
        if (dataStream != null) {
            long len = numValues * 8;
            while (len > 0) {
                len -= dataStream.skip(len);
            }
        }
    }
    ...
    public void next(RandomAccessBlock randomAccessBlock, int positionCount) throws IOException {
        ...
        if (present == null) {
            ...
            for (int i = 0; i < positionCount; i++) {
                // no null value.
                double doubleVal = utils.readDouble(dataStream);
                vector[i] = doubleVal;
                lastPosition++;
            }
