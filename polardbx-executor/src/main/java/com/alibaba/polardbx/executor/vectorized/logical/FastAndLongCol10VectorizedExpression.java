...
public class FastAndLongCol10VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[10][];
    private boolean[][] childNulls = new boolean[10][];
    private boolean[] childHasNull = new boolean[10];

    public FastAndLongCol10VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 10);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 10);
                outputVectorHasNull |= outputNulls[i];
            }
        }
