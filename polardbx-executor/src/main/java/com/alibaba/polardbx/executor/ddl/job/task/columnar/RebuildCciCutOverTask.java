...
import com.alibaba.polardbx.executor.ddl.job.meta.misc.RebuildCciMetaChanger;
...
@Getter
@TaskName(name = "RebuildCciCutOverTask")
public class RebuildCciCutOverTask extends BaseGmsTask {
    private final Map<String, String> tableNameMap;

    public RebuildCciCutOverTask(final String schemaName,
    ...
    protected void executeImpl(Connection metaDbConnection, ExecutionContext executionContext) {
        LOGGER.info(
            String.format(
                "[rebuild cci] start change meta during cutOver for primary table: %s.%s",
                schemaName, logicalTableName)
        );
        updateSupportedCommands(true, false, metaDbConnection);

        RebuildCciMetaChanger.alterTableModifyColumnCutOver(
            metaDbConnection,
            schemaName,
            logicalTableName,
            tableNameMap,
        ...
        LOGGER.info(
            String.format(
                "[rebuild cci] finish change meta during cutOver for primary table: %s.%s",
                schemaName, logicalTableName)
        );
    }
