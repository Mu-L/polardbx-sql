...
public class FastAndLongCol12VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[12][];
    private boolean[][] childNulls = new boolean[12][];
    private boolean[] childHasNull = new boolean[12];

    public FastAndLongCol12VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        for (int childIdx = 0; childIdx < 12; childIdx++) {
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 12);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 12);
                outputVectorHasNull |= outputNulls[i];
            }
        }
