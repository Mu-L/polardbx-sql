...
public class FastNEDecimalColDecimalConstVectorizedExpression extends AbstractVectorizedExpression {
    ...
    public FastNEDecimalColDecimalConstVectorizedExpression(int outputIndex, VectorizedExpression[] children) {
        ...
        if (useDecimal128Compare) {
            ...
            if (isSelectionInUse) {
                for (int i = 0; i < batchSize; i++) {
                    ...
                    boolean notEqual = !(leftHigh == rightHigh && leftLow == rightLow);
                    output[j] = notEqual ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
                }
            } else {
                for (int i = 0; i < batchSize; i++) {
                    // fetch left decimal value
                    ...
                    boolean notEqual = !(leftHigh == rightHigh && leftLow == rightLow);
                    output[i] = notEqual ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
                }
            }
            return;
        }
