...
/**
 * IntGroupTopNHeap is an optimized implementation for int-based GROUP BY with ORDER BY operations.
 * It uses TreeBasedRowHeap<IntIndexRow> as the base data structure and follows the framework of DefaultGroupTopNHeap.
 * Type-specific optimizations are borrowed from IntTopNHeap.
 */
public class IntGroupTopNHeap extends BaseGroupTopNHeap implements MemoryCountable {
    private static final long INSTANCE_SIZE = ClassLayout.parseClass(IntGroupTopNHeap.class).instanceSize();

    // MultiIntTopNHeap implementation for unified management of all groups
    private final MultiIntTopNHeap multiIntTopNHeap;

    @Override
    public long getMemoryUsage() {
        return INSTANCE_SIZE + multiIntTopNHeap.getMemoryUsage();
    }

    public IntGroupTopNHeap(
        ...
        this.multiIntTopNHeap = new MultiIntTopNHeap();
    }

    @Override
    public void addChunk(Chunk groupKeyChunk, Chunk inputChunk) {
        multiIntTopNHeap.processChunk(groupKeyChunk, inputChunk);
    }

    @Override
    public Iterator<Chunk> buildChunks() {
        return multiIntTopNHeap.buildChunks();
    }

    @Override
    public long estimateSize() {
        return multiIntTopNHeap.estimateSize();
    }

    @Override
    public void close() {
        multiIntTopNHeap.close();
    }

    /**
     * MultiIntTopNHeap structure for unified management of all groups with int-specific optimizations
     * Core structure: TreeBasedRowHeap<IntIndexRow>[] rowHeap for each group
     * Unified pageReference management and compact logic
     */
    private class MultiIntTopNHeap implements MemoryCountable {
        private static final int NOT_EXISTS = -1;
        private static final int MIN_POSITIONS_TO_COMPACT = 8 * 1024;
        private final long HEAP_INSTANCE_SIZE =
            ClassLayout.parseClass(MultiIntTopNHeap.class).instanceSize();
        ...
        private TreeBasedRowHeap<IntIndexRow>[] rowHeaps;
        ...
        private MemoryCountableObjectBigArray<IntPageReference> pageReferences = new MemoryCountableObjectBigArray<>();
        ...
        // Int-specific comparators and order by option
        @FieldMemoryCounter(value = false)
        private OrderByOption orderByOption;
        private NullableIntComparator reversedIntComparator;
        @FieldMemoryCounter(value = false)
        private Comparator<IntIndexRow> reversedRowComparator;
        ...
        public MultiIntTopNHeap() {
            ...
            // Initialize int-specific comparators (borrowed from IntTopNHeap)
            // Assume the first field in innerCollation is the int field to sort by
            ...
            // Build int-specific comparators
        ...
        /**
         * Process chunk with unified pageReference management and int-specific optimizations
         */
        public void processChunk(Chunk groupKeyChunk, Chunk inputChunk) {
            checkArgument(groupKeyChunk != null & inputChunk != null);

            // Create unified IntPageReference for the input chunk
            IntPageReference newPageReference =
                new IntPageReference(inputChunk, ImmutableList.copyOf(inputTypes), context, orderByOption);
            ...
            for (int position = 0; position < rowCount; position++) {
                ...
                TreeBasedRowHeap<IntIndexRow> rowHeap = getRowHeap(groupId);

                // Extract int value and null flag for this position (int-specific optimization)
                int value = getIntValue(newPageId, position);
                boolean isNull = isNull(newPageId, position);

                // Create IntIndexRow for this position
                ...
                if (rowHeap.size() < fetchValue) {
                    ...
                    IntIndexRow worstRow = rowHeap.first();
                    if (reversedRowComparator.compare(newRow, worstRow) > 0) {
                        // New row is better, replace worst row
                        rowHeap.dequeue();
                        IntPageReference previousPageReference = pageReferences.get(worstRow.getPageId());
        ...
        /**
         * Int-specific value extraction methods (borrowed from IntTopNHeap)
         */
        ...
        /**
         * Create a new group and corresponding rowHeap with int-specific comparator
         */
        private int createNewGroup(Chunk groupKeyChunk, int position) {
            ...
            if (groupId >= rowHeaps.length) {
                TreeBasedRowHeap<IntIndexRow>[] newRowHeaps = new TreeBasedRowHeap[rowHeaps.length * 2];
        ...
        private void createRowHeap(int groupId) {
            // Create TreeBasedRowHeap for this group with int-specific comparator
            Comparator<IntIndexRow> reversedHeapComparator = (l, r) -> {
                int cmp = reversedIntComparator.compare(l.value, l.isNull, r.value, r.isNull);

                // 1. Use intComparator for heap ordering (not reversedIntComparator)
            ...
            rowHeaps[groupId] = new TreeBasedRowHeap<>(reversedHeapComparator, IntIndexRow.INT_ROW_ENTRY_SIZE);
        }

        /**
         * Get rowHeap for the specified group
         */
        private TreeBasedRowHeap<IntIndexRow> getRowHeap(int groupId) {
        ...
        private class UnifiedResultIterator implements Iterator<Chunk> {
            ...
            private final Queue<Iterator<IntIndexRow>> groupIterators;
            ...
            UnifiedResultIterator() {
                ...
                for (int groupId = 0; groupId < groupCount; groupId++) {
                    TreeBasedRowHeap<IntIndexRow> rowHeap = rowHeaps[groupId];
            ...
            public Chunk next() {
                ...
                while (!pageBuilder.isFull() && hasMoreData) {
                    ...
                    Iterator<IntIndexRow> groupIterator = groupIterators.peek();
                    ...
                    if (groupIterator != null && groupIterator.hasNext()) {
                        ...
                        while (groupIterator.hasNext() && !pageBuilder.isFull()) {
                            IntIndexRow indexRow = groupIterator.next();
        ...
        public long estimateSize() {
            ...
            // RowHeap Size
            totalSize += groupCount * fetchValue * 4;
    ...
    /**
     * NullableIntComparator interface - borrowed from IntTopNHeap
     */
    ...
    /**
     * NullableIntComparatorImpl - borrowed from IntTopNHeap
     */
    ...
    /**
     * Build int-specific comparator - borrowed from IntTopNHeap
     */
