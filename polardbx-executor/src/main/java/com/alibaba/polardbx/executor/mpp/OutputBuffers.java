...
public final class OutputBuffers {
    ...
    public void checkValidTransition(OutputBuffers newOutputBuffers) {
        ...
        if (version == newOutputBuffers.version) {
            checkArgument(this.equals(newOutputBuffers),
                "newOutputBuffers is the same version but contains different information");
        }
        ...
        for (Entry<OutputBufferId, Integer> entry : buffers.entrySet()) {
            if (!entry.getValue().equals(newOutputBuffers.buffers.get(entry.getKey()))) {
                throw new IllegalArgumentException(
                    "newOutputBuffers has changed the assignment for task " + entry.getKey());
            }
        }
    }
    ...
    public static class OutputBufferId implements MemoryCountable {
        ...
        public OutputBufferId(int stageId, int id) {
            checkArgument(stageId >= 0, "stageId is negative");
        ...
        public OutputBufferId(int id) {
            this((id & 0xffff0000) >>> 16, id & 0x0000ffff);
        }
        ...
        public boolean equals(Object o) {
            ...
            return id == that.id && stageId == that.stageId;
        }
        ...
        public int getFullId() {
            return formatOutputBufferId(this.stageId, this.id);
        }
        ...
        @Override
        public String toString() {
            return String.valueOf(getFullId());
        }

        public static int formatOutputBufferId(int stageId, int id) {
            checkArgument(stageId <= 0x0000ffff, "stageId is large than " + 0x0000ffff);
            checkArgument(id <= 0x0000ffff, "id is large than " + 0x0000ffff);
            return stageId << 16 | id;
        }
    }
}
