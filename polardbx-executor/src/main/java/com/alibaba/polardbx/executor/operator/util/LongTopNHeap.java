...
public class LongTopNHeap implements TopNHeap {
    private static final Logger MPP_LOGGER = LoggerFactory.getLogger(Driver.class);
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(LongTopNHeap.class).instanceSize();
    ...
    private final MemoryCountableObjectBigArray<LongPageReference> pageReferences
    ...
    private final MemoryCountableIntArrayFIFOQueue emptyPageReferenceSlots = new MemoryCountableIntArrayFIFOQueue();
    ...
    public LongTopNHeap(List<DataType> sourceTypes,
    ...
    protected long getLongValue(int pageId, int position) {
        return pageReferences.get(pageId).getPage().getBlock(orderByOption.index).getLong(position);
    }
    ...
    public void processChunk(Chunk newPage) {
        ...
        while (!rowHeap.isEmpty()) {
            ...
            if (globalThreshold.isInitialized()) {
                ...
                if ((rowHeap.size() <= topN && cmp > 0) || (rowHeap.size() > topN && cmp >= 0)) {
                    ...
                    rowHeap.dequeue();
                    LongPageReference previousLongPageReference = pageReferences.get(firstRow.getPageId());
                    previousLongPageReference.dereference(firstRow.getPosition());
        ...
        LongPageReference newLongPageReference = new LongPageReference(newPage, sourceTypes, context, orderByOption);
        ...
        pageReferences.set(newPageId, newLongPageReference);
        ...
        for (int position = 0; position < newPage.getPositionCount(); position++) {
            ...
            if (rowHeap.size() < topN) {
                ...
                if (reversedRowComparator.compare(newRow, nThRow) > 0) {
                    ...
                    // discard the top of heap.
                    rowHeap.dequeue();
                    LongPageReference previousLongPageReference = pageReferences.get(nThRow.getPageId());
                    previousLongPageReference.dereference(nThRow.getPosition());
                    dequeueInProcessingChunkTimes++;
        ...
        if (savedPositions > MIN_POSITIONS_TO_COMPACT && compactThreshold * usedPositions <= savedPositions) {
            for (int i = 0; i < maxPageId; i++) {
                LongPageReference pageReference = pageReferences.get(i);
    ...
    private class ResultIterator extends AbstractIterator<Chunk> {
        ...
        protected Chunk computeNext() {
            ...
            while (!pageBuilder.isFull() && indexRowIterator.hasNext()) {
                ...
                LongPageReference pageReference = pageReferences.get(indexRow.getPageId());
