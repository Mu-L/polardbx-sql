...
public class FastAndLongCol5VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[5][];
    private boolean[][] childNulls = new boolean[5][];
    private boolean[] childHasNull = new boolean[5];

    public FastAndLongCol5VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 5);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 5);
                outputVectorHasNull |= outputNulls[i];
            }
        }
