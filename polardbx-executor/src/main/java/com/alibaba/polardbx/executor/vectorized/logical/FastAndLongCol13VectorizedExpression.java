...
public class FastAndLongCol13VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[13][];
    private boolean[][] childNulls = new boolean[13][];
    private boolean[] childHasNull = new boolean[13];

    public FastAndLongCol13VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        for (int childIdx = 0; childIdx < 13; childIdx++) {
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 13);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 13);
                outputVectorHasNull |= outputNulls[i];
            }
        }
