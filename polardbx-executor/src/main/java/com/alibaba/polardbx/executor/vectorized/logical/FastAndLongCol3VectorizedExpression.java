...
public class FastAndLongCol3VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[3][];
    private boolean[][] childNulls = new boolean[3][];
    private boolean[] childHasNull = new boolean[3];

    public FastAndLongCol3VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 3);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 3);
                outputVectorHasNull |= outputNulls[i];
            }
        }
