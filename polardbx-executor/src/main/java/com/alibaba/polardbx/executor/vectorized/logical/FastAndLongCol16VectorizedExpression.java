...
public class FastAndLongCol16VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[16][];
    private boolean[][] childNulls = new boolean[16][];
    private boolean[] childHasNull = new boolean[16];

    public FastAndLongCol16VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        for (int childIdx = 0; childIdx < 16; childIdx++) {
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 16);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 16);
                outputVectorHasNull |= outputNulls[i];
            }
        }
