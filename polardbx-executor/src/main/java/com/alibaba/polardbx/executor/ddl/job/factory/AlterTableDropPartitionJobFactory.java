...
public class AlterTableDropPartitionJobFactory extends AlterTableGroupBaseJobFactory {
    ...
    protected ExecutableDdlJob doDropInOriginTableGroup() {
        ...
        List<DdlTask> cleanColumnarDataTasks = cleanColumnarDataTask(schemaName);
        ...
        if (GeneralUtil.isEmpty(cleanColumnarDataTasks)) {
            ...
            taskList.add(disableDropPartitionMetaTask);
        ...
        if (stayAtPublic) {
            ...
            DdlTask dropUselessTableTask =
                ComplexTaskFactory.cleanUpUselessPhyTableTask(schemaName, preparedData.getTableName(),
                    getTheDeletedPartitionsLocation((AlterTableGroupDropPartitionPreparedData) preparedData,
                        preparedData.getTableName()), targetTablesTopology.get(preparedData.getTableName()),
            ...
            if (GeneralUtil.isNotEmpty(dropForeignKeyTasksBeforeRename)) {
                for (DdlTask task : dropForeignKeyTasksBeforeRename) {
                    executableDdlJob.addTaskRelationship(tableGroupSyncTask, task);
    ...
    protected ExecutableDdlJob doDropAndMoveToExistTableGroup() {
        ...
        if (GeneralUtil.isEmpty(cleanColumnarDataTasks)) {
            ...
            executableDdlJob.addTaskRelationship(validateSourceTableGroup, cleanColumnarDataTasks.get(0));
            executableDdlJob.addTaskRelationship(validateTargetTableGroup, cleanColumnarDataTasks.get(0));
            executableDdlJob.addTaskRelationship(cleanColumnarDataTasks.get(cleanColumnarDataTasks.size() - 1),
                disableDropPartitionMetaTask);
        }
        ...
        if (stayAtPublic) {
            ...
            if (GeneralUtil.isNotEmpty(dropForeignKeyTasksBeforeRename)) {
                for (DdlTask task : dropForeignKeyTasksBeforeRename) {
                    executableDdlJob.addTaskRelationship(tableGroupsSyncTask, task);
                    executableDdlJob.addTaskRelationship(task, dropUselessTableTask);
                }
            } else {
    ...
    public void constructSubTasks(String schemaName, ExecutableDdlJob executableDdlJob, DdlTask tailTask,
        ...
        if (subTaskJobFactory.getCdcTableGroupDdlMarkTask() != null) {
            ...
            executableDdlJob.addTaskRelationship(subTailTask, bringUpAlterTableGroupTasks.get(0));
        }
    }

    public List<DdlTask> cleanColumnarDataTask(String schemaName) {
        List<DdlTask> ddlTasks = new ArrayList<>();
        TableMeta tableMeta = OptimizerContext.getContext(schemaName).getLatestSchemaManager()
            .getTable(preparedData.getTableName());
        if (tableMeta.withCci() && executionContext.getParamManager()
            ...
            // 清理列存数据
            DdlTask cleanColumnarDataTask = new DropPrimaryTblPartitionCleanColumnarDataTask(
                schemaName,
                preparedData.getTableName(),
