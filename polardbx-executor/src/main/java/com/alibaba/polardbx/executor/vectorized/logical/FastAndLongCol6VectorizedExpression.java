...
public class FastAndLongCol6VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[6][];
    private boolean[][] childNulls = new boolean[6][];
    private boolean[] childHasNull = new boolean[6];

    public FastAndLongCol6VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        for (int childIdx = 0; childIdx < 6; childIdx++) {
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls);
                outputVectorHasNull |= outputNulls[i];
            }
        }
