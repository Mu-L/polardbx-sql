...
public class FastAndLongCol14VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[14][];
    private boolean[][] childNulls = new boolean[14][];
    private boolean[] childHasNull = new boolean[14];

    public FastAndLongCol14VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEvalWithShortCircuit(EvaluationContext ctx) {
        ...
        for (int childIdx = 0; childIdx < 14; childIdx++) {
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 14);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 14);
                outputVectorHasNull |= outputNulls[i];
            }
        }
