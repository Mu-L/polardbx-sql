...
public class AbstractIsNullColExpression extends AbstractVectorizedExpression {
    public AbstractIsNullColExpression(int outputIndex, VectorizedExpression[] children) {
    ...
    public void eval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                output[j] = inputNulls[j] ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                output[i] = inputNulls[i] ? LongBlock.TRUE_VALUE : LongBlock.FALSE_VALUE;
            }
        }
    }
