 ...
 */
public class LogicalCreateCclBlockerHandler extends HandlerCommon {

    private static final Logger logger = LoggerFactory.getLogger(LogicalCreateCclBlockerHandler.class);
    ...
    static {
        METRIC_KEYS.add("EXECUTION_TIME");
    ...
    public LogicalCreateCclBlockerHandler(IRepository repo) {
        super(repo);
    }
    ...
    public Cursor handle(RelNode logicalPlan, ExecutionContext executionContext) {
        ...
        SqlCreateCclBlocker createCclBlocker = (SqlCreateCclBlocker) plan.getSqlDal();
        String blockerName = createCclBlocker.getBlockerName().getSimple();
        String schemaName = createCclBlocker.getSchemaName().getSimple();
        try (Connection metaDbConn = MetaDbUtil.getConnection()) {
            ...
            List<CclBlockerRecord> existTriggers = cclBlockerAccessor.queryByIds(Lists.newArrayList(blockerName));
            if (CollectionUtils.isNotEmpty(existTriggers)) {
                if (createCclBlocker.isIfNotExits()) {
            ...
            cclBlockerRecord.id = blockerName;
            cclBlockerRecord.schema = schemaName;

            List<Pair<SqlNode, SqlNode>> ruleWiths = createCclBlocker.getRuleWiths();
            //不为空
            ...
            if (!withMap.containsKey("MAX_CONCURRENCY") && !withMap.containsKey("DRY_RUN")) {
                throw new TddlNestableRuntimeException(
                    "Ccl rule with option MAX_CONCURRENCY or DRY_RUN must be provided.");
            }
            ...
            cclBlockerRecord.ruleConfig = withJson;

            //check the
            List<Pair<SqlNode, SqlNode>> limits = createCclBlocker.getLimits();
            ...
            cclBlockerRecord.maxSQLSize = MAX_SQL_SIZE;
            ...
            cclBlockerRecord.maxCclRule = limitMap.get("MAX_CCL_RULE");
            if (cclBlockerRecord.maxCclRule > MAX_CCL_RULE) {
                throw new TddlNestableRuntimeException(
                    "The limit option MAX_CCL_RULE must not be larger than " + MAX_CCL_RULE);
            }
            if (limitMap.containsKey("MAX_SQL_SIZE")) {
                cclBlockerRecord.maxSQLSize = limitMap.get("MAX_SQL_SIZE");
            }
            if (limitMap.containsKey("QUERY_RULE_UPGRADE")) {
                cclBlockerRecord.ruleUpgrade = limitMap.get("QUERY_RULE_UPGRADE");
            } else {
                cclBlockerRecord.ruleUpgrade = DEFAULT_RULE_UPGRADE;
            }

            //deal with whens
            List<SqlNode> leftOperands = createCclBlocker.getLeftOperands();
            List<SqlNode> operators = createCclBlocker.getOperators();
            List<SqlNode> rightOperands = createCclBlocker.getRightOperands();
            ...
            cclBlockerRecord.user = createCclBlocker.getUser() == null ? "%" : createCclBlocker.getUser().getSimple();
            cclBlockerRecord.host = createCclBlocker.getHost() == null ? "%" : createCclBlocker.getHost().getSimple();
