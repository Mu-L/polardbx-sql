...
/**
 * Generate [INSERT INTO SELECT BINLOG] to [BLACK HOLE] when truncating entire primary table.
 * Used for [COLUMNAR].
 */
@TaskName(name = "TruncatePrimaryTblCleanColumnarDataTask")
@Getter
public class TruncatePrimaryTblCleanColumnarDataTask extends BaseDdlTask {
    ...
    @JSONCreator
    public TruncatePrimaryTblCleanColumnarDataTask(String schemaName, String tableName,
    ...
    protected void duringTransaction(Connection metaDbConnection, ExecutionContext executionContext) {
        ...
        // 分区名称提供器
        PrimaryTblCleanColumnarDataUtils.PartitionNamesProvider partitionNamesProvider =
            PrimaryTblCleanColumnarDataUtils::getAllTablePartitionNames;
        ...
        PrimaryTblCleanColumnarDataUtils.executeDuringTransaction(metaDbConnection, schemaName, tableName,
            ...
            partitionNamesProvider, stateCallback, executionContext);
    }
    ...
    private void markDNCleanedPhyPartSet(String dnId, String partitionName, Integer currentTotalRows,
        ...
        DdlEngineAccessorDelegate delegate = new DdlEngineAccessorDelegate<Integer>() {
            ...
            protected Integer invoke() {
                synchronized (TruncatePrimaryTblCleanColumnarDataTask.this) {
                    ...
                    TruncatePrimaryTblCleanColumnarDataTask newTask =
                        (TruncatePrimaryTblCleanColumnarDataTask) TaskHelper.fromDdlEngineTaskRecord(currTaskRec);

                    // 获取最新的 DN 级别已完成分区集合
                    Map<String, Set<String>> newestDnCleanedPhyPartSetMap = newTask.getDnCleanedPhyPartSetMap();

                    // 确保是 ConcurrentHashMap
                    ...
                    // 更新状态并写回数据库
                    TruncatePrimaryTblCleanColumnarDataTask.this.setDnCleanedPhyPartSetMap(
                        newestDnCleanedPhyPartSetMap);
                    TruncatePrimaryTblCleanColumnarDataTask.this.setCurrentTotalRows(currentTotalRows);
                    TruncatePrimaryTblCleanColumnarDataTask.this.setCurrentTotalTime(currentTotalTime);
                    TruncatePrimaryTblCleanColumnarDataTask.this.setTaskStartTime(taskStartTime);
                    TruncatePrimaryTblCleanColumnarDataTask.this.setCurrentSpeed(currentSpeed);
                    TruncatePrimaryTblCleanColumnarDataTask.this.setState(DdlTaskState.DIRTY);
        ...
        DdlEngineAccessorDelegate delegate = new DdlEngineAccessorDelegate<Integer>() {
            ...
            protected Integer invoke() {
                synchronized (TruncatePrimaryTblCleanColumnarDataTask.this) {
                    ...
                    // 更新状态并写回数据库
                    TruncatePrimaryTblCleanColumnarDataTask.this.setDnPartitionToLastPKMap(
