 ...
 * @author hongxi.chx
 */
public class SortWindowFramesExecFactory extends ExecutorFactory {
    ...
    public SortWindowFramesExecFactory(SortWindow overWindow, ExecutorFactory executorFactory, int parallelism) {
    ...
    public synchronized List<Executor> createAllExecutors(ExecutionContext context) {

        if (executors.isEmpty()) {
            for (int k = 0; k < parallelism; k++) {
                Executor exec = createOverWindowExec(context, k);
    ...
    private Executor createOverWindowExec(ExecutionContext context, int index) {
        ...
        if (isNoFrameWindowFunction(operator)) {
            return createNonFrameWindowExec(overWindow, context, index);
        }
        ...
        for (int i = 0; i < overWindow.groups.size(); i++) {
            ...
            List<Aggregator> aggregators = AggregateUtils
                .convertAggregators(group.getAggregateCalls(overWindow), context, memoryAllocator);
        ...
        registerRuntimeStat(overWindowFramesExec, overWindow, context);
    ...
    private com.alibaba.polardbx.executor.operator.Executor createNonFrameWindowExec(SortWindow overWindow,
        ...
        List<Aggregator> aggregators = AggregateUtils
            .convertAggregators(overWindow.groups.get(0).getAggregateCalls(overWindow), context, memoryAllocator);
