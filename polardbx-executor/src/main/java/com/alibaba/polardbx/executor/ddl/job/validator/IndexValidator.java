...
public class IndexValidator {
    ...
    public static void validateDropLastColumnarIndex(String schemaName, String logicalTableName,
                                                     ExecutionContext executionContext) {
        validateDropLastColumnarIndex(schemaName, logicalTableName, 1, executionContext);
    }

    public static void validateDropLastColumnarIndex(String schemaName, String logicalTableName, int dropCciCount,
                                                     ExecutionContext executionContext) {
        if (dropCciCount <= 0) {
            return;
        }
        ...
        boolean isPureColumnar = Engine.isPureColumnar(tableMeta.getEngine());
        if (!isPureColumnar) {
            return;
        }
        GsiMetaManager.GsiTableMetaBean gsiTableMetaBean = tableMeta.getGsiTableMetaBean();
        if (gsiTableMetaBean == null || gsiTableMetaBean.indexMap == null) {
            return;
        }
        long cciCount = gsiTableMetaBean.indexMap.values().stream()
            .filter(index -> index.columnarIndex)
            .count();
        if (cciCount - dropCciCount < 1) {
            if (executionContext != null
                && executionContext.getParamManager().getBoolean(ConnectionParams.ALLOW_DROP_LAST_CCI)) {
                return;
            }
            throw new TddlRuntimeException(ErrorCode.ERR_EXECUTOR,
                "Cannot drop the last columnar index on pure columnar table '" + logicalTableName + "'");
        }
    }

    public static void validateNoExternalizedColumnsInIndex(
        String schemaName, String tableName, List<SqlIndexColumnName> columns) {
        TableMeta tableMeta =
        ...
        for (SqlIndexColumnName col : columns) {
    ...
    public static void validateNoColumnarIndexOnExternalizedTable(String schemaName, String tableName) {
        TableMeta tableMeta =
            OptimizerContext.getContext(schemaName).getLatestSchemaManager().getTableWithNull(tableName);
        if (tableMeta != null
