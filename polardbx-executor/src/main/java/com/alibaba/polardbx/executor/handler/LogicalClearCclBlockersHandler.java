 ...
 */
public class LogicalClearCclBlockersHandler extends HandlerCommon {
    private static final Logger logger = LoggerFactory.getLogger(LogicalClearCclBlockersHandler.class);

    public LogicalClearCclBlockersHandler(IRepository repo) {
        super(repo);
    }
    ...
    public Cursor handle(RelNode logicalPlan, ExecutionContext executionContext) {
        ...
        try (Connection metaDbConn = MetaDbUtil.getConnection()) {
            ...
            List<CclBlockerRecord> cclBlockerRecordList = cclBlockerAccessor.query();
            if (CollectionUtils.isNotEmpty(cclBlockerRecordList)) {
                List<Integer> triggerPriorities = cclBlockerRecordList.stream().map((e) -> e.priority).collect(
                    Collectors.toList());
            ...
            int affectedRowCount = cclBlockerAccessor.deleteAll();
            ...
            throw new TddlRuntimeException(ErrorCode.ERR_CCL, " Failed to CLEAR CCL_BLOCKERS", e);
        }
    }
}
