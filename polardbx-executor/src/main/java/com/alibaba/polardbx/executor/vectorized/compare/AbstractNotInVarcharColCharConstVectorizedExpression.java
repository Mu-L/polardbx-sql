...
public abstract class AbstractNotInVarcharColCharConstVectorizedExpression extends AbstractVectorizedExpression {
    ...
    public AbstractNotInVarcharColCharConstVectorizedExpression(
        int outputIndex,
        ...
        if (!compatible && leftInputVectorSlot instanceof SliceBlock) {
            ...
            if (operandCount() == 1) {
                if (isSelectionInUse) {
                    for (int i = 0; i < batchSize; i++) {
                        int j = sel[i];
                        output[j] = sliceBlock.equals(j, (Slice) operandSortKeys[0]) ^ 1;
                    }
                } else {
                    for (int i = 0; i < batchSize; i++) {
                        output[i] = sliceBlock.equals(i, (Slice) operandSortKeys[0]) ^ 1;
                    }
                }
                ...
                if (isSelectionInUse) {
                    for (int i = 0; i < batchSize; i++) {
                        int j = sel[i];
                        output[j] = sliceBlock.anyMatch(j, (Slice) operandSortKeys[0], (Slice) operandSortKeys[1]) ^ 1;
                    }
                } else {
                    for (int i = 0; i < batchSize; i++) {
                        output[i] = sliceBlock.anyMatch(i, (Slice) operandSortKeys[0], (Slice) operandSortKeys[1]) ^ 1;
                    }
                }
                ...
                if (isSelectionInUse) {
                    for (int i = 0; i < batchSize; i++) {
                        int j = sel[i];
                        output[j] = sliceBlock.anyMatch(j, operandSortKeys) ^ 1;
                    }
                } else {
                    for (int i = 0; i < batchSize; i++) {
                        output[i] = sliceBlock.anyMatch(i, operandSortKeys) ^ 1;
                    }
                }
            }
        } else {
