...
public class FastAndLongCol4VectorizedExpression extends AbstractVectorizedExpression {
    ...
    private long[][] childResults = new long[4][];
    private boolean[][] childNulls = new boolean[4][];
    private boolean[] childHasNull = new boolean[4];

    public FastAndLongCol4VectorizedExpression(
        int outputIndex,
    ...
    private void doAndEval(EvaluationContext ctx) {
        ...
        if (isSelectionInUse) {
            for (int i = 0; i < batchSize; i++) {
                int j = sel[i];
                computeResult(j, childResults, childNulls, childHasNull, res, outputNulls, 4);
                outputVectorHasNull |= outputNulls[j];
            }
        } else {
            for (int i = 0; i < batchSize; i++) {
                computeResult(i, childResults, childNulls, childHasNull, res, outputNulls, 4);
                outputVectorHasNull |= outputNulls[i];
            }
        }
