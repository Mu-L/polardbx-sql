...
public class LogicalShowTablesFromCatalogHandlerTest {
    ...
    public void testHandleSuccess() {
        ...
        when(mockMetadata.listTables("db1")).thenReturn(Arrays.asList("table1", "table2"));
        ...
        try (MockedStatic<ExternalCatalogManager> mgrMock = mockStatic(ExternalCatalogManager.class);
            ...
            SqlShowTablesFromCatalog showNode = mock(SqlShowTablesFromCatalog.class);
            when(plan.getNativeSqlNode()).thenReturn(showNode);
            when(showNode.getCatalogName()).thenReturn("cat1");
            when(showNode.getExternalDbName()).thenReturn("DB1");
            when(showNode.isFull()).thenReturn(false);

            ExecutionContext ec = mock(ExecutionContext.class);
            IRepository repo = mock(IRepository.class);
            LogicalShowTablesFromCatalogHandler handler =
                new LogicalShowTablesFromCatalogHandler(repo);
    ...
    public void testHandleFull() {
        ...
        ConnectorMetadata mockMetadata = mock(ConnectorMetadata.class);
        when(mockMetadata.listTables("db1")).thenReturn(Arrays.asList("table1"));
