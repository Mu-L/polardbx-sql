...
public class FastAndLongCol3VectorizedExpressionTest {
    ...
    public void testAllTrue() {
        ...
        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3);
    ...
    public void testMixed() {
        ...
        LongBlock block3 = LongBlock.of(1L, 1L, 1L, 2L, 1L);

        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3);
        // Result: (1==1) AND (1==1) AND (1==1) = true
        //         (0==1) AND ... = false
        //         (1==1) AND (2==1) AND ... = false
        //         (1==1) AND (1==1) AND (2==1) = false
        //         (0==1) AND ... = false
    ...
    public void testWithNull() {
        ...
        LongBlock block3 = LongBlock.of(1L, 1L, 1L, 1L, null);

        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3);
        // Result: true AND true AND true = true
        //         null AND true AND true = null (no false, has null)
        //         true AND null AND true = null (no false, has null)
        //         false AND ... = false
        //         true AND true AND null = null (no false, has null)
    ...
    public void testWithSelection() {
        ...
        LongBlock block3 = LongBlock.of(1L, 1L, 1L, 2L, 1L, 1L);
    ...
    public void testNullWithFalse() {
        ...
        //         (1==1)=true AND (null==1)=null AND true = null (no false, has null)
    ...
    private void doTest(Chunk inputChunk, RandomAccessBlock expectBlock, int[] sel) {
        ...
        // Create FastAndLongCol3VectorizedExpression with 3 EQLongColCharConstVectorizedExpression children
        ...
        for (int i = 0; i < 3; i++) {
            children[i] = new EQLongColCharConstVectorizedExpression(
                3 + i, // output index for each child
        ...
        FastAndLongCol3VectorizedExpression condition = new FastAndLongCol3VectorizedExpression(
            6, // output index (after 3 input columns + 3 child outputs)
            children
        );

        // Placeholder for input and output blocks
        // Need slots for: 3 input columns + 3 child outputs + 1 final output
