...
public class FastAndLongCol6VectorizedExpressionTest {
    ...
    public void testAllTrue() {
        ...
        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3, block4, block5, block6);
    ...
    public void testMixed() {
        ...
        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3, block4, block5, block6);
        // Result: (1==1) AND (1==1) AND (1==1) AND (1==1) AND (1==1) AND (1==1) = true
        //         (0==1) AND ... = false
        //         (1==1) AND (2==1) AND ... = false
        //         (1==1) AND (1==1) AND (1==1) AND (2==1) AND ... = false
        //         (0==1) AND ... = false
    ...
    public void testWithNull() {
        ...
        Chunk inputChunk = new Chunk(block1.getPositionCount(), block1, block2, block3, block4, block5, block6);
        // Result: true AND true AND true AND true AND true AND true = true
        //         null AND true AND ... = null (no false, has null)
        //         true AND null AND ... = null (no false, has null)
        //         false AND ... = false
        //         true AND true AND true AND null AND ... = null (no false, has null)
    ...
    public void testNullWithFalse() {
        ...
        //         (1==1)=true AND (null==1)=null AND ... = null (no false, has null)
    ...
    private void doTest(Chunk inputChunk, RandomAccessBlock expectBlock, int[] sel) {
        ...
        // Create FastAndLongCol6VectorizedExpression with 6 EQLongColCharConstVectorizedExpression children
        // Each child compares a Long column with value 1 (i.e., col == 1)
        VectorizedExpression[] children = new VectorizedExpression[6];
        for (int i = 0; i < 6; i++) {
            children[i] = new EQLongColCharConstVectorizedExpression(
                6 + i, // output index for each child
        ...
        FastAndLongCol6VectorizedExpression condition = new FastAndLongCol6VectorizedExpression(
            12, // output index (after 6 input columns + 6 child outputs)
            children
        );

        // Placeholder for input and output blocks
        // Need slots for: 6 input columns + 6 child outputs + 1 final output
