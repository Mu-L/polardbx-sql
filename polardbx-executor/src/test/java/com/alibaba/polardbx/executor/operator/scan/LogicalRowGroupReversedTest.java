...
public class LogicalRowGroupReversedTest extends ScanTestBase {
    private int startGroupId;
    private int startPosition;
    private int positionCount;
    private int[] startPositionsForStripe1 = {9000, 19000, 29000, 39000}; // for each group.
    private int[] startPositionsForStripe2 = {249000, 259000, 269000, 279000}; // for each group.
    ...
    private void doTestChunkEnumerate(int stripeId) throws IOException {
        ...
        while (rgIterator.hasNext()) {
            ...
            startPosition = stripeId == 0 ? startPositionsForStripe1[rowGroupId] : startPositionsForStripe2[rowGroupId];

            // check logical row-group:
            System.out.println("group_id = " + logicalRowGroup.groupId());

            Chunk chunk;
            RowGroupReader<Chunk> rowGroupReader = logicalRowGroup.getReversedReader();
            while ((chunk = rowGroupReader.nextBatch()) != null) {
                ...
                Assert.assertTrue(batchRange[0] == startPosition);
                Assert.assertTrue(batchRange[1] == positionCount);
...
//        final int loadColumnIndex = 1; // column index to load.
        ...
        while (rgIterator.hasNext()) {
            ...
            System.out.println("rowGroupId = " + rowGroupId);
            startPosition = startPositionsForStripe1[rowGroupId];
