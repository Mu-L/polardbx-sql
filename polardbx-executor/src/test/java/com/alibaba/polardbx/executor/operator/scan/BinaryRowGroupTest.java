...
import org.apache.orc.impl.RowIndexDeserializeUtils;
import org.apache.orc.impl.RowIndexSerializeUtils;
...
public class BinaryRowGroupTest {
    ...
    protected static final String TEST_ORC_FILE_NAME = "binary_row_group_test.orc";
    ...
    public void prepareFile() throws Exception {
        TypeDescription schema = TypeDescription.createStruct()
            ...
            .addField("f2", TypeDescription.createVarchar())
            .addField("f3", TypeDescription.createDouble())
        ...
        try (Writer writer = OrcFile.createWriter(FILE_PATH,
            ...
            for (int rowNumber = 0; rowNumber < rowCount; rowNumber++) {
                // Populate the rowIdx
                ((LongColumnVector) batch.cols[0]).vector[batch.size] = rowNumber;
                ((BytesColumnVector) batch.cols[1]).setVal(batch.size, ("row: " + rowNumber).getBytes());
                ((DoubleColumnVector) batch.cols[2]).vector[batch.size] = rowNumber * 1.0d;
                ((LongColumnVector) batch.cols[3]).vector[batch.size] = rowNumber;
                ((LongColumnVector) batch.cols[4]).vector[batch.size] = rowNumber;
        ...
        for (int i = 0; i < redundantStripeMetadataList.size(); i++) {
            ...
            for (int index = 0; index < rowIndexWithColumnList.size(); index++) {
                ...
                OrcProto.BinaryRowIndexWithColumn binaryRowIndexWithColumn
                    = RowIndexSerializeUtils.transformToBinaryRowIndexWithColumn(rowIndexWithColumn);
                rowIndexWithColumn = RowIndexDeserializeUtils.transformToRowIndexWithColumn(binaryRowIndexWithColumn);

                int column = rowIndexWithColumn.getColumn();
                OrcProto.RowIndex rowIndex = rowIndexWithColumn.getRowIndex();

                OrcProto.BinaryRowIndex binaryRowIndex = RowIndexSerializeUtils.transformToBinaryRowIndex(rowIndex);

                OrcProto.RowIndex transformed = RowIndexDeserializeUtils.transformToRowIndex(binaryRowIndex);

                indexes[column] = transformed;
            }
