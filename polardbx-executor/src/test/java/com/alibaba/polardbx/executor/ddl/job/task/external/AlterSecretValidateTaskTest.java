...
public class AlterSecretValidateTaskTest {

    @Test
    public void testExecuteImplSecretExists() {
        ...
        try (MockedStatic<MetaDbUtil> metaDbMock = mockStatic(MetaDbUtil.class);
            ...
            AlterSecretValidateTask task = new AlterSecretValidateTask("secret1");
    ...
    @Test
    public void testExecuteImplMetaDbError() {
        ExecutionContext mockEc = mock(ExecutionContext.class);

        try (MockedStatic<MetaDbUtil> metaDbMock = mockStatic(MetaDbUtil.class)) {
            metaDbMock.when(MetaDbUtil::getConnection).thenThrow(new RuntimeException("metadb down"));
