...
public class ScanWorkWithEarlyStopTest extends ScanTestBase {
    ...
    public void prepare() throws IOException {
        ...
        earlyStopManager = new EarlyStopManagerImpl(
            ImmutableList.of(new OrderByOption(0, true, true)),
            ImmutableList.of("pk"),
            1000
        );

        scanOrderByOptions = ImmutableList.of(new OrderByOption(0, true, true));

        orderByInProjects = ImmutableList.of(0);

        globalTopNThreshold = Mockito.mock(GlobalTopNThreshold.class);
        Mockito.when(globalTopNThreshold.isInitialized()).thenReturn(false).thenReturn(false).thenReturn(true);
        Mockito.when(globalTopNThreshold.getTopNThresholdType()).thenReturn(GlobalTopNThreshold.TopNThresholdType.LONG);
        Mockito.when(globalTopNThreshold.getLongThreshold()).thenReturn(1L);
