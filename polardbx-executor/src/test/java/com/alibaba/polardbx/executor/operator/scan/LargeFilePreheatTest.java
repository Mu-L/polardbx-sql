...
public class LargeFilePreheatTest {
    ...
    protected static final String TEST_ORC_FILE_NAME = "redundant_file_test.orc";
    ...
    public void prepareFile() throws Exception {
        ...
        try (Writer writer = OrcFile.createWriter(FILE_PATH,
            ...
            for (int rowId = 0; rowId < totalRowCount / 2; rowId++) {
                // Populate the rowIdx
                ((LongColumnVector) batch.cols[0]).vector[batch.size] = rowId;
                ((BytesColumnVector) batch.cols[1]).setVal(batch.size, (rowId + " row: " + rowId).getBytes());
                ((LongColumnVector) batch.cols[2]).vector[batch.size] = rowId;
            ...
            for (int rowId = totalRowCount / 2; rowId < totalRowCount; rowId++) {
                // Populate the rowIdx
                ((LongColumnVector) batch.cols[0]).vector[batch.size] = rowId;
                ((BytesColumnVector) batch.cols[1]).setVal(batch.size, ("row 0000").getBytes());
                ((LongColumnVector) batch.cols[2]).vector[batch.size] = rowId;
                ((LongColumnVector) batch.cols[3]).vector[batch.size] = rowId;
                ((LongColumnVector) batch.cols[4]).vector[batch.size] = rowId;
