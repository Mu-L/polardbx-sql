...
public class DeadlockTest extends CrudBasedLockTestCase {
    ...
    @Test
    public void testRds80DisableDeadlockDetction() throws InterruptedException {
        if (!isMySQL80()) {
            return;
        }
        JdbcUtil.executeSuccess(tddlConnection, "set global ENABLE_DEADLOCK_DETECTION_80 = false");
        Thread.sleep(1000);
        String tableName = "testRds80DisableDeadlockDetction";
        String dropSql = "drop table if exists " + tableName;
        String createSql = "create table " + tableName + "(id int primary key) dbpartition by hash(id)";
        String insertSql = "insert into " + tableName + " values (0), (1)";
        ...
        try {
            ...
            JdbcUtil.executeUpdate(connections.get(0), "set innodb_lock_wait_timeout = 5");
            String sql = "select * from " + tableName + " where id = 0 for update";
            ...
            JdbcUtil.executeUpdate(connections.get(1), "set innodb_lock_wait_timeout = 5");
            ...
            futures.add(threadPool.submit(() -> {
                final String sql0 = "select * from " + tableName + " where id = 1 for update";
                try {
                    JdbcUtil.executeQuery(sql0, connections.get(0));
                } catch (Throwable e) {
                    ...
                    try {
                        // Try to commit
                        JdbcUtil.executeUpdate(connections.get(0), "commit");
                    } catch (Throwable e) {
                        // Ignore
                    }
                }
            ...
            futures.add(threadPool.submit(() -> {
                final String sql0 = "select * from " + tableName + " where id = 0 for update";
                try {
                    JdbcUtil.executeQuery(sql0, connections.get(1));
                } catch (Throwable e) {
                    if (e.getMessage().contains("Lock wait timeout exceeded")) {
                        return true;
                    } else {
                    ...
                    try {
                        // Try to commit
                        JdbcUtil.executeUpdate(connections.get(1), "commit");
                    } catch (Throwable e) {
                        // Ignore
                    }
                }
