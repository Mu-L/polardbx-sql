...
import com.alibaba.polardbx.druid.benchmark.TPCH;
import com.alibaba.polardbx.qatest.ReadBaseTestCase;
import com.alibaba.polardbx.qatest.dql.sharding.explain.ExplianAnalyzeExecuteTest;
...
public class ExplainExecuteTpchTest extends ReadBaseTestCase {
    ...
    public static Map<String, String> phyTableNameMap = new HashMap<>();

    public static List<String> tableNames =
        Arrays.asList("customer", "lineitem", "nation", "orders", "part", "partsupp", "region", "supplier");

    public static String dbName = "ExplainExecuteTpchTest_db";

    public static List<String> tpchSqlList = new ArrayList<>();
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `customer` (  `c_custkey` int(11) NOT NULL,  `c_name` varchar(25) NOT NULL,  `c_address` varchar(40) NOT NULL,  `c_nationkey` int(11) NOT NULL,  `c_phone` varchar(15) NOT NULL,  `c_acctbal` decimal(15, 2) NOT NULL,  `c_mktsegment` varchar(10) NOT NULL,  `c_comment` varchar(117) NOT NULL,  PRIMARY KEY (`c_custkey`)) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`c_custkey`) PARTITIONS "
                + partitionCount);
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `lineitem` (  `l_orderkey` bigint(20) NOT NULL,  `l_partkey` int(11) NOT NULL,  `l_suppkey` int(11) NOT NULL,  `l_linenumber` bigint(20) NOT NULL,  `l_quantity` decimal(15, 2) NOT NULL,  `l_extendedprice` decimal(15, 2) NOT NULL,  `l_discount` decimal(15, 2) NOT NULL,  `l_tax` decimal(15, 2) NOT NULL,  `l_returnflag` varchar(1) NOT NULL,  `l_linestatus` varchar(1) NOT NULL,  `l_shipdate` date NOT NULL,  `l_commitdate` date NOT NULL,  `l_receiptdate` date NOT NULL,  `l_shipinstruct` varchar(25) NOT NULL,  `l_shipmode` varchar(10) NOT NULL,  `l_comment` varchar(44) NOT NULL,  PRIMARY KEY (`l_orderkey`, `l_linenumber`)) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`l_orderkey`) PARTITIONS "
                + partitionCount);
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `nation` (  `n_nationkey` int(11) NOT NULL,  `n_name` varchar(25) NOT NULL,  `n_regionkey` int(11) NOT NULL,  `n_comment` varchar(152) DEFAULT NULL,  PRIMARY KEY (`n_nationkey`) ) BROADCAST ENGINE = InnoDB DEFAULT CHARSET = latin1;\n");
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `orders` (  `o_orderkey` bigint(20) NOT NULL,  `o_custkey` int(11) NOT NULL,  `o_orderstatus` varchar(1) NOT NULL,  `o_totalprice` decimal(15, 2) NOT NULL,  `o_orderdate` date NOT NULL,  `o_orderpriority` varchar(15) NOT NULL,  `o_clerk` varchar(15) NOT NULL,  `o_shippriority` bigint(20) NOT NULL,  `o_comment` varchar(79) NOT NULL,  PRIMARY KEY (`o_orderkey`) ) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`o_orderkey`) PARTITIONS  "
                + partitionCount);
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `part` (  `p_partkey` int(11) NOT NULL,  `p_name` varchar(55) NOT NULL,  `p_mfgr` varchar(25) NOT NULL,  `p_brand` varchar(10) NOT NULL,  `p_type` varchar(25) NOT NULL,  `p_size` int(11) NOT NULL,  `p_container` varchar(10) NOT NULL,  `p_retailprice` decimal(15, 2) NOT NULL,  `p_comment` varchar(23) NOT NULL,  PRIMARY KEY (`p_partkey`)) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`p_partkey`) PARTITIONS  "
                + partitionCount);
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `partsupp` (  `ps_partkey` int(11) NOT NULL,  `ps_suppkey` int(11) NOT NULL,  `ps_availqty` int(11) NOT NULL,  `ps_supplycost` decimal(15, 2) NOT NULL,  `ps_comment` varchar(199) NOT NULL,  PRIMARY KEY (`ps_partkey`, `ps_suppkey`) ) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`ps_partkey`) PARTITIONS  "
                + partitionCount);
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `region` (  `r_regionkey` int(11) NOT NULL,  `r_name` varchar(25) NOT NULL,  `r_comment` varchar(152) DEFAULT NULL,  PRIMARY KEY (`r_regionkey`)) BROADCAST ENGINE = InnoDB DEFAULT CHARSET = latin1;\n");
        ddl.add(
            "    CREATE TABLE IF NOT EXISTS `supplier` (  `s_suppkey` int(11) NOT NULL,  `s_name` varchar(25) NOT NULL,  `s_address` varchar(40) NOT NULL,  `s_nationkey` int(11) NOT NULL,  `s_phone` varchar(15) NOT NULL,  `s_acctbal` decimal(15, 2) NOT NULL,  `s_comment` varchar(101) NOT NULL,  PRIMARY KEY (`s_suppkey`)) ENGINE = InnoDB DEFAULT CHARSET = latin1 PARTITION BY KEY (`s_suppkey`) PARTITIONS  "
                + partitionCount);
        for (String sql : ddl) {
        ...
        for (String tableName : tableNames) {
            for (String phyTableName : getALLPhyTbNames(connection, tableName)) {
                phyTableNameMap.put(phyTableName, tableName);
            }
        }
    ...
    @Test
    public void testExplainExecuteTpch() throws Exception {
        Connection connection = getPolardbxConnection(dbName);
        for (String sql : tpchSqlList) {
            String explainShardingSql = "explain sharding " + sql;
            ResultSet explainShardingRs = JdbcUtil.executeQuery(explainShardingSql, connection);
            Map<String, Integer> phyTableShardingMap = new HashMap<>();
            while (explainShardingRs.next()) {
                phyTableShardingMap.put(explainShardingRs.getString("LOGICAL_TABLE"),
                    explainShardingRs.getInt("SHARD_COUNT"));
            }

            String explainExecuteSql = "/*+TDDL:EXPLAIN_EXECUTE_PHYTB_LEVEL=2*/explain execute " + sql;
            ResultSet explainExecuteRs = JdbcUtil.executeQuery(explainExecuteSql, connection);
            Map<String, Set<String>> explainExecuteMap = new HashMap<>();
            while (explainExecuteRs.next()) {
                String phyTableName = explainExecuteRs.getString("table");
                if (phyTableNameMap.containsKey(phyTableName)) {
                    String logicalTable = phyTableNameMap.get(phyTableName);
                    explainExecuteMap.putIfAbsent(logicalTable, new HashSet<>());
                    explainExecuteMap.get(logicalTable).add(phyTableName);
                }
            }

            for (String logicalTable : explainExecuteMap.keySet()) {
                Assert.assertEquals((int) phyTableShardingMap.get(logicalTable),
                    (int) explainExecuteMap.get(logicalTable).size());
            }
        }
    }

    public static Set<String> getALLPhyTbNames(Connection connection, String tableName) throws Exception {
        ...
        while (topologyRs.next()) {
            ...
            allPhyTableNames.add(realPhyTableName);
        }
    ...
    @Test
    public void testExplainAnalyzeExecuteTpch() throws Exception {
        if (!isMySQL80()){
            return;
        }
        ...
        for (String sql : tpchSqlList) {
            ...
            String explainExecuteSql = "/*+TDDL:EXPLAIN_EXECUTE_PHYTB_LEVEL=2*/explain analyze_execute " + sql;
            ResultSet explainExecuteRs = JdbcUtil.executeQuery(explainExecuteSql, connection);
            Map<String, Set<String>> explainExecuteMap = new HashMap<>();
            while (explainExecuteRs.next()) {
                List<String> phyTableNames = ExplianAnalyzeExecuteTest.removeBracket(explainExecuteRs.getString(1));
    ...
    @Test
    public void testExplainTreeExecuteTpch() throws Exception {
        if (!isMySQL80()){
            return;
        }
        ...
        for (String sql : tpchSqlList) {
            ...
            String explainExecuteSql = "/*+TDDL:EXPLAIN_EXECUTE_PHYTB_LEVEL=2*/explain tree_execute " + sql;
            ResultSet explainExecuteRs = JdbcUtil.executeQuery(explainExecuteSql, connection);
            Map<String, Set<String>> explainExecuteMap = new HashMap<>();
            while (explainExecuteRs.next()) {
                List<String> phyTableNames = ExplianAnalyzeExecuteTest.removeBracket(explainExecuteRs.getString(1));
    ...
    @Test
    public void testExplainJsonExecuteTpch() throws Exception {
        if (!isMySQL80()){
            return;
        }
        ...
        for (String sql : tpchSqlList) {
            ...
            String explainExecuteSql = "/*+TDDL:EXPLAIN_EXECUTE_PHYTB_LEVEL=2*/explain json_execute " + sql;
            ResultSet explainExecuteRs = JdbcUtil.executeQuery(explainExecuteSql, connection);
            Map<String, Set<String>> explainExecuteMap = new HashMap<>();
            while (explainExecuteRs.next()) {
                List<String> phyTableNames = ExplianAnalyzeExecuteTest.removeBracket(explainExecuteRs.getString(1));
