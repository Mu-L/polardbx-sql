...
public class AutoPartitionColumnTest extends AutoPartitionTestBase {

    private final static String CREATE_TABLE_TEMPLATE = "CREATE PARTITION TABLE `{0}` (\n"
        ...
        + "  UNIQUE CLUSTERED INDEX `{2}` using btree (`order_id`) dbpartition by hash(`order_id`)\n"
        + ");";
    private static final String CREATE_TABLE_NO_PK_TEMPLATE = "CREATE PARTITION TABLE `{0}` (\n"
        + "  `c0` int,\n"
        + "  `t` timestamp default current_timestamp,\n"
        + "  `order_id` int DEFAULT NULL,\n"
        + "  `seller_id` int DEFAULT NULL,\n"
    ...
    public void addColumnPlanTest() {
        ...
        final String beforeExplain = getExplainResult(tddlConnection, sql);
        Assert.assertTrue(beforeExplain.contains("IndexScan"));
        final String beforeNoExplain = getExplainResult(tddlConnection, sqlNo);
        ...
        final String afterExplain = getExplainResult(tddlConnection, sql);
        Assert.assertTrue(afterExplain.contains("IndexScan") && afterExplain.contains("`c1`"));
        final String afterNoExplain = getExplainResult(tddlConnection, sqlNo);
    ...
    @Test
    @Ignore
    public void testAddColumnRollbackWithPartSuccess() throws Exception {

        final Random random = new Random();
        final String[] badTables = {TABLE_NAME, CGSI_NAME, UCGSI_NAME};
        for (String badTable : badTables) {
            // Note: This try 2 times to ensure correct sync of table meta and get the same error and rollback correctly.
            for (int i = 0; i < 2; ++i) {
                final String addedColumn = "addedy";

                // Make a existing column on physical table.
                final String phyTableName; // Auto partition only have tbpartition, so all phyName is same.
                try (ResultSet rs = JdbcUtil
                    .executeQuery(MessageFormat.format("show topology `{0}`", badTable), tddlConnection)) {
                    List<List<String>> res = JdbcUtil.getAllStringResult(rs, false, null);
                    Assert.assertEquals(4, res.size());
                    Assert.assertEquals(4, res.get(0).size());
                    phyTableName = res.get(0).get(2);
                    Assert.assertEquals(0, phyTableName.indexOf(badTable));
                }

                // Make an existing col on one partition.
                final int badPartition = random.nextInt(4);
                JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat
                    .format("/*+TDDL: node({0})*/alter table `{1}` add column `{2}` int",
                        badPartition, phyTableName, addedColumn));

                final String alter =
                    MessageFormat.format("alter table `{0}` add column `{1}` int", TABLE_NAME, addedColumn);
                JdbcUtil
                    .executeUpdateFailed(tddlConnection, alter,
                        "Not all physical DDLs have been executed successfully");

                // Assert that no column on phy table except the made one.
                for (int paritionId = 0; paritionId < 4; ++paritionId) {
                    try (ResultSet rs = JdbcUtil
                        .executeQuery(
                            MessageFormat
                                .format("/*+TDDL: node({0})*/show create table `{1}`", paritionId, phyTableName),
                            tddlConnection)) {
                        List<List<String>> res = JdbcUtil.getAllStringResult(rs, false, null);
                        Assert.assertEquals(1, res.size());
                        Assert.assertEquals(2, res.get(0).size());
                        if (paritionId == badPartition) {
                            Assert.assertTrue(res.get(0).get(1).contains(addedColumn));
                        } else {
                            Assert.assertFalse(res.get(0).get(1).contains(addedColumn));
                        }
                    }
                }

                // Restore the bad partition.
                JdbcUtil.executeUpdateSuccess(tddlConnection, MessageFormat
                    .format("/*+TDDL: node({0})*/alter table `{1}` drop column `{2}`",
                        badPartition, phyTableName, addedColumn));
