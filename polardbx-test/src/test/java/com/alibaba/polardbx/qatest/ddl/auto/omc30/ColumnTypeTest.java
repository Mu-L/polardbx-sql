    ...
    public void convertToNotNullableColumnTest() throws SQLException {
        ...
        try (Connection conn = getPolardbxConnection()) {
            ...
            String alterSql = String.format(
                "/*+TDDL:cmd_extra(FORCE_USING_OMC_30 = true)*/alter table %s modify column b int not null", tableName);

            String sqlMode = JdbcUtil.getSqlMode(conn);
            setSqlMode("STRICT_TRANS_TABLES", conn);
            JdbcUtil.executeUpdateSuccess(conn, hint + alterSql + USE_OMC_ALGORITHM);
            setSqlMode("", conn);
            JdbcUtil.executeUpdateSuccess(conn, hint + alterSql + USE_OMC_ALGORITHM);
