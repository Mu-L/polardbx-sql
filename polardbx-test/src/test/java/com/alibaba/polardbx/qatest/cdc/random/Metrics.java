...
public class Metrics {
    ...
    private AtomicLong alterCharsetSuccess = new AtomicLong();
    private AtomicLong alterCharsetFail = new AtomicLong();
    private AtomicLong addIndexSuccess = new AtomicLong();
    private AtomicLong addIndexFail = new AtomicLong();
    private AtomicLong dropIndexSuccess = new AtomicLong();
    private AtomicLong dropIndexFail = new AtomicLong();
    ...
    void check() {
        ...
        Assert.assertTrue(addColumnSuccess.get() > 0);
        Assert.assertTrue(dropColumnSuccess.get() > 0);
        Assert.assertTrue(modifyColumnSuccess.get() > 0);
        Assert.assertTrue(alterCharsetSuccess.get() > 0);
        Assert.assertTrue(addIndexSuccess.get() > 0);
        Assert.assertTrue(dropIndexSuccess.get() > 0);
    }
}
