...
public class ReturningReplaceTest extends DDLBaseNewDBTestCase {
    ...
    private static String buildReturningReplace(String insert) {
        return
            buildCmdExtra(ENABLE_OPTIMIZE_REPLACE_BY_RETURNING,
                DISABLE_DML_GET_DUP_FOR_LOCAL_UK_WITH_FULL_TABLE_SCAN)
                + insert;
    }
    ...
    public void tableWithPkNoUkWithGsi() throws SQLException {
        ...
        // 已知问题，根据mysql官方文档 affected rows应返回3，但在dn80和mysql80验证实际返回的是6
...
//        final List<Pair<String, String>> topology = JdbcUtil.getTopology(tddlConnection, tableName);
    ...
    public void tableWithPkNoUkWithMultiGsi_partitionByPk() throws SQLException {
        ...
        final String createTable = "CREATE TABLE IF NOT EXISTS `" + tableName + "` (\n"
            ...
            + "(`c1`) COVERING(`c2`,`c5`) PARTITION BY HASH(`c1`) PARTITIONS 3,\n"
    ...
    public void tableWithPkWithUkWithGsi_partitionByPk() throws SQLException {
        ...
        // 3 replace for primary , 2 replace for ug
    ...
    public void tableWithPkNoUkWithUgsi_multiDuplicateRow_usingGsi() throws SQLException {
        ...
        executeOnMysqlAndTddl(mysqlConnection, tddlConnection, replace, "trace " + returningReplace, null, true);
        final List<List<String>> trace = getTrace(tddlConnection);
    ...
    public void testReplaceUGSI() throws SQLException {
        ...
        String returingReplaceSql = buildReturningReplace(replaceSql);
        JdbcUtil.executeUpdateSuccess(tddlConnection, returingReplaceSql);
    ...
    @Test
    public void testReplaceReturningStats() {
        // for code coverage
        String log = GlobalReplaceReturningStatsSingleton.getInstance().log();
    ...
    public void testCoveringIndexReplace() throws SQLException {
        ...
        final String createTable = "CREATE TABLE IF NOT EXISTS `" + tableName + "` (\n"
            ...
            + "(`b`) COVERING(`c`,`d`) PARTITION BY KEY(`b`) PARTITIONS 3,\n"
            + "  UNIQUE GLOBAL INDEX " + gsiName2
            + "(`d`,`c`) COVERING(`b`) PARTITION BY KEY(`d`,`c`) PARTITIONS 3\n"
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4\n";
        final String partitionDef = " PARTITION BY KEY(`a`) PARTITIONS 3";
    ...
    public void testNotCoveringIndexReplace() throws SQLException {
        ...
        final String createTable = "CREATE TABLE IF NOT EXISTS `" + tableName + "` (\n"
            ...
            + "  PRIMARY KEY (`a`),\n"
            + "  UNIQUE GLOBAL INDEX " + gsiName1
            + "(`b`) PARTITION BY KEY(`b`) PARTITIONS 3,\n"
            + "  UNIQUE GLOBAL INDEX " + gsiName2
            + "(`c`) PARTITION BY KEY(`c`) PARTITIONS 3\n"
            + ") ENGINE=InnoDB DEFAULT CHARSET=utf8mb4\n";
        final String partitionDef = " PARTITION BY KEY(`a`) PARTITIONS 3";
    ...
    /**
     * 测试binlog对replace returning的重排序机制
     * 构造了包含对三种fix delete处理方式都包含的case，详见设计文档
     */
    @Test
    public void testBinlogConflict() throws SQLException {
        if (!isMySQL80() || !useXproto()) {
            return;
        }

        final String tableName = "replace_returning_test_binlog_conflict";
        dropTableIfExists(tableName);
        dropTableIfExistsInMySql(tableName);

        final String gsiName = "gic";
        final String createTable = String.format("CREATE TABLE `%s` (\n"
            + "\t`a` int NOT NULL,\n"
            + "\t`b` int DEFAULT NULL,\n"
            + "\t`c` int DEFAULT NULL,\n"
            + "\tPRIMARY KEY (`a`),\n"
            + "\tUNIQUE GLOBAL INDEX `%s` (`c`) COVERING(`b`)\n"
            + "\t\tPARTITION BY LIST(`c`)\n"
            + "      (\n"
            ...
            + "  PARTITION p3 VALUES IN (DEFAULT)\n"
            + ")", tableName, gsiName);
        final String mysqlCreateTable = String.format("CREATE TABLE `%s` (\n"
            + "\t`a` int NOT NULL,\n"
            + "\t`b` int DEFAULT NULL,\n"
            + "\t`c` int DEFAULT NULL,\n"
            + "\tPRIMARY KEY (`a`),\n"
            + "UNIQUE INDEX `%s` (`c`)\n"
            + ") ENGINE = InnoDB DEFAULT CHARSET = utf8mb4", tableName, gsiName);

        JdbcUtil.executeUpdateSuccess(tddlConnection, createTable);
        JdbcUtil.executeUpdateSuccess(mysqlConnection, mysqlCreateTable);

        String insert = "insert into " + tableName + " values(1,1,1),(2,1,2)";
        JdbcUtil.executeUpdateSuccess(tddlConnection, insert);
        JdbcUtil.executeUpdateSuccess(mysqlConnection, insert);

        String mysqlReplace = "replace into " + tableName + " values(1,2,1),(1,2,2),(1,2,1),(2,1,1),(2,2,1)";
        String replace = buildReturningReplace(mysqlReplace);

        JdbcUtil.executeUpdateSuccess(tddlConnection, replace);
        JdbcUtil.executeUpdateSuccess(mysqlConnection, mysqlReplace);
        final List<List<String>> trace = getTrace(tddlConnection);
        Assert.assertFalse("Should not use returning path when batch values have PK/UK conflicts",
            isReturningPath(trace));
        selectContentSameAssert("select * from " + tableName, null, mysqlConnection, tddlConnection);
