...
public abstract class LocalityTestBase extends BaseTestCase {
    ...
    public static List<StorageNodeBean> getStorageInfo(Connection tddlConnection) {
        final String showStorageSql = "SHOW STORAGE";

        List<StorageNodeBean> res = new ArrayList<>();
        try (ResultSet result = JdbcUtil.executeQuerySuccess(tddlConnection, showStorageSql)) {
            while (result.next()) {
                String instance = result.getString("STORAGE_INST_ID");
                String instKind = result.getString("INST_KIND");
                String status = result.getString("STATUS");
                boolean deletable = BooleanUtils.toBoolean(result.getString("DELETABLE"));
                StorageNodeBean
                    storageNode = new StorageNodeBean(instance, instKind, status, deletable);
                res.add(storageNode);
            }
    ...
    public static String chooseDatanode(Connection tddlConnection) {
        return chooseDatanode(tddlConnection, false);
    }

    public static String chooseDatanode(Connection tddlConnection, Boolean firstDn) {
