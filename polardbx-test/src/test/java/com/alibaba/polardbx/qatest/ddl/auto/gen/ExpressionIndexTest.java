...
public class ExpressionIndexTest extends DDLBaseNewDBTestCase {
    ...
    public void testCreateFailed() {
        ...
        String create = String.format(
            "create table %s (a int primary key, b int, c int, d varchar(64), e varchar(64)) partition by hash(a)",
            tableName);
        ...
        indexColumns = "(a+1),b";
        ddl = generatedDdl(tableName, "global", indexName, indexColumns, "partition by hash(b)");
    ...
    public void testSpecialIndexName() {
        ...
        for (String indexName : indexNames) {
            testCreateExprIndexInternal(tableName, indexName, create, "partition by hash(a)", indexColumns,
                columnInfos, false);
            testCreateExprIndexInternal(tableName, indexName, create, "partition by hash(a)", indexColumns,
                columnInfos, true);
        }
    ...
    public void testSpecialColumnName() {
        ...
        testCreateExprIndexInternal(tableName, indexName, create, "partition by hash(a)", indexColumns, columnInfos,
            false);
        testCreateExprIndexInternal(tableName, indexName, create, "partition by hash(a)", indexColumns, columnInfos,
            true);
    ...
    @Test
    public void testAutoPartitionTable() {
        if (isMySQL80()) {
            return;
        }
        String tableName = "expr_auto_part_tbl";

        String create = String.format(
            "create table %s (a int primary key, b int, c int, d varchar(64), e varchar(64))",
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, create);
        String hint = buildCmdExtra(ENABLE_CREATE_EXPRESSION_INDEX, ENABLE_UNIQUE_KEY_ON_GEN_COL);
        ...
        String indexName = tableName + "_idx_1";
        String indexColumns = "(a+1)";
        List<String> ddl = generatedDdl(tableName, "", indexName, indexColumns, "");
    ...
    public void testMultipleAlterTable() {
        ...
        String create = String.format(
            "create table %s (a int primary key, b int, c varchar(20), d varchar(20)) partition by hash(a)",
            tableName);
        ...
        Assert.assertTrue(showCreateTable.equalsIgnoreCase(trueCreateSql));
