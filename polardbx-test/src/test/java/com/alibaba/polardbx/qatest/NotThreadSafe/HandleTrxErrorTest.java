...
@NotThreadSafe
public class HandleTrxErrorTest extends BaseTestCase {
    @Test
    public void testHandleStatementErrorFail()
        throws SQLException, ExecutionException, InterruptedException, TimeoutException {
        try (final Connection conn = this.getPolardbxConnection()) {
            JdbcUtil.executeUpdateSuccess(conn, "set global ENABLE_CLOSE_CONNECTION_WHEN_TRX_FATAL = false");
