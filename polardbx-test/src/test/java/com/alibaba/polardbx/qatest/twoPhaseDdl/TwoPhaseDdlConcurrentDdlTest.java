...
public class TwoPhaseDdlConcurrentDdlTest extends DDLBaseNewDBTestCase {
    ...
    public TwoPhaseDdlConcurrentDdlTest(boolean crossSchema) {
    ...
    // two_phase_ddl_test1.online_ddl_pause_concurrent.{0..3}, 16 * 100W
    @Test
    public void testConcurrentAlterTableAddDdlPauseBeforePrepare() throws Exception {
        ...
        for (int i = 0; i < concurrent; i++) {
            String tableName = schemaPrefix + "online_ddl_pause_concurrent" + String.valueOf(i);
            tableNames.add(tableName);
            prepareData(tddlConnection, schemaName, tableName, 1_000_000);
        }
        for (int i = 0; i < concurrent; i++) {
    ...
    // two_phase_ddl_test1.modify_column_pause_concurrent.{0..3}, 16 * 100W
    @Test
    public void testConcurrentAlterTableModifyColumnDdlPauseBeforePrepare() throws Exception {
        ...
        int concurrent = 4;
        for (int i = 0; i < concurrent; i++) {
            String tableName = schemaPrefix + "modify_column_pause_concurrent" + String.valueOf(i);
            tableNames.add(tableName);
            prepareData(tddlConnection, schemaName, tableName, 1_000_000);
        }
        for (int i = 0; i < concurrent; i++) {
