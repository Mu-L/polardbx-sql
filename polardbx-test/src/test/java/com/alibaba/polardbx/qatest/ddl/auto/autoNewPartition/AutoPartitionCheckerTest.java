 ...
 * @version 1.0
 */
public class AutoPartitionCheckerTest extends BaseAutoPartitionNewPartition {
    ...
    public void testCheckHint() throws Exception {
        ...
        final String addGsiSql =
            "/*+TDDL: cmd_extra(GSI_BACKFILL_USE_FASTCHECKER=false)*/alter table " + TABLE_NAME
                + " add clustered index `" + indexName + "` (x) partition by hash(x) partitions 3;";
        JdbcUtil.executeUpdateSuccess(tddlConnection, addGsiSql);

        checkGsi(tddlConnection, getRealGsiName(tddlConnection, TABLE_NAME, indexName));

        //将gsi某列设置为write_only
        updateColumnStatus(tddlDatabase1, getRealGsiName(tddlConnection, TABLE_NAME, indexName),
            ImmutableList.of("order_id"), 2);

        checkGsi(tddlConnection, "/*+TDDL: cmd_extra(GSI_BACKFILL_USE_FASTCHECKER=false)*/",
            getRealGsiName(tddlConnection, TABLE_NAME, indexName));
    ...
    private void updateColumnStatus(String schemaName, String tableName, List<String> columnNames, int status) {
        try {
            ...
            String refresh = String.format("reload table %s.%s", schemaName, tableName);
