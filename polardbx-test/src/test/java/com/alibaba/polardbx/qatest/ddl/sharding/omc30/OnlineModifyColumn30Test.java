...
public class OnlineModifyColumn30Test extends DDLBaseNewDBTestCase {
    ...
    public void testOnlineModifyColumnOnTableWithGsi() {
        ...
        String sql = String.format(
            "create table %s (a int primary key, b int, c int) dbpartition by hash(a) tbpartition by hash(a)",
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);

        sql = String.format("create global clustered index `%s` on %s (`b`) dbpartition by hash(`b`)", indexName,
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);
    ...
    @Test
    public void testOnlineModifyColumnOnTableWithGsiSuccess() {
        ...
        sql = String.format("create global index `%s` on %s (`b`) covering (`c`) dbpartition by hash(`b`)", indexName,
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);
    ...
    @Test
    public void testOnlineModifyColumnOnTableWithGsiForce() {
    ...
    public void testOnlineModifyColumnLocalIndexFailed() {
        ...
        String sql =
            String.format(
                "create table %s (a int primary key, b varchar(255), c int) dbpartition by hash(a) tbpartition by hash(a)",
                tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);
        ...
        sql = hint + String.format("alter table %s modify column b text", tableName)
            + USE_OMC_ALGORITHM;
        ...
        sql = hint + String.format("alter table %s modify column b blob(10)",
    ...
    public void testOnlineModifyColumnRollback() {
        ...
        dropTableIfExists(tableName);
        String sql = String.format(
            "create table %s (a int primary key, b bigint, c bigint, d bigint, e bigint) single",
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);
        ...
        sql = hint + String.format("alter table %s modify column b tinyint,",
            tableName) + USE_OMC_ALGORITHM;
        JdbcUtil.executeUpdateFailed(tddlConnection, sql, "");

        sql = hint + String.format("alter table %s modify column c smallint,",
            tableName) + USE_OMC_ALGORITHM;
        JdbcUtil.executeUpdateFailed(tddlConnection, sql, "");

        sql = hint + String.format("alter table %s modify column d mediumint,",
            tableName) + USE_OMC_ALGORITHM;
        execDdlWithRetry(tddlDatabase1, tableName, sql, tddlConnection);
        sql = hint + String.format("alter table %s modify column e int,", tableName)
            + USE_OMC_ALGORITHM;
        execDdlWithRetry(tddlDatabase1, tableName, sql, tddlConnection);
    ...
    public void testOnlineModifyColumnLocalIndexRollback() {
        ...
        sql = hint + String.format("alter table %s modify column b varchar(0),",
            tableName) + USE_OMC_ALGORITHM;
        JdbcUtil.executeUpdateFailed(tddlConnection, sql, "");
    ...
    public void testOnlineModifyColumnFulltextIndex() {
        ...
        dropTableIfExists(tableName);
        String sql = String.format(
            "create table %s (a int primary key, b int, c int, d int, e int, f varchar(64)) dbpartition by hash(a) tbpartition by hash(a)",
            tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, sql);
        ...
        sql = hint + String.format("alter table %s modify column d bigint,",
            tableName) + USE_OMC_ALGORITHM;
        execDdlWithRetry(tddlDatabase1, tableName, sql, tddlConnection);
        sql = hint + String.format("alter table %s modify column e bigint,",
            tableName) + USE_OMC_ALGORITHM;
        execDdlWithRetry(tddlDatabase1, tableName, sql, tddlConnection);
    ...
    public void testOnlineModifyColumnSimpleCheckerTest() {
        ...
        sql = hint + String.format("alter table %s modify column b bigint,", tableName)
            + USE_OMC_ALGORITHM;
        execDdlWithRetry(tddlDatabase1, tableName, sql, tddlConnection);
    ...
    public void testShowTable() throws Exception {
        ...
        testShowTableInternal(tableName,
            hint + String.format("alter table %s modify column b int,", tableName) + USE_OMC_ALGORITHM);
        testShowTableInternal(tableName,
            hint + String.format("alter table %s change column b b int,", tableName) + USE_OMC_ALGORITHM);
    }
