...
public class GeneratedColumnDMLTest extends DDLBaseNewDBTestCase {
    ...
    private static final String UDF_NAME = "auto_gen_add";

    private static final String CREATE_UDF = "create function auto_gen_add(num1 int, num2 int) \n"
        + "  returns int\n"
        + "begin\n"
        + "  return num1 + num2;\n"
        + "end;";
    ...
    private static final String[] COLUMN_DEFS =
        {"d tinyint as (a+id*20) logical, c int as (auto_gen_add(id,a)) logical", "c int as (a-id) logical"};

    private static final String[] PART_DEFS =
        {"dbpartition by hash(a)", "dbpartition by hash(c)", "single", "broadcast"};
    ...
    public void initSelectTable() {
        ...
        dropFunctionIfExists(UDF_NAME);
        JdbcUtil.executeUpdateSuccess(tddlConnection, CREATE_UDF);
    }
    ...
    public void testGeneratedColumnUpdate() throws SQLException {
        ...
        if (strictMode) {
            // In MySQL, update will return ER_WARN_DATA_OUT_OF_RANGE even if it's not the row updated that is out
            // of range.
            return;
        }
    ...
    private void testGeneratedColumnDmlInternal(String tableName, boolean withGsi, boolean withData, String[] dml,
        ...
        if (withGsi) {
            ...
            String createGsi1 =
                String.format("create global index %s on %s(c) dbpartition by hash(c)", gsiName1, tableName);
            String createGsi2 =
                String.format("create global clustered index %s on %s(a) dbpartition by hash(a)", gsiName2, tableName);
            String createGsi3 =
                String.format("create global index %s on %s(id) covering(a) dbpartition by hash(id)", gsiName3,
                    tableName);
            ...
            if (writeOnly) {
                updateColumnStatus(tddlDatabase1, gsiName2, 2);
            }
            ...
            if (writeOnly) {
                updateColumnStatus(tddlDatabase1, gsiName2, 1);
                checkGsi(tddlConnection, gsiName2);
            }
        }
    }
