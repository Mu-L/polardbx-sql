...
@NotThreadSafe
public class AlterTableExtractPartitionByHotValueTest extends AlterTableReorgBaseTest {
    ...
    * */
    static List<PartitionRuleInfo> partitionRuleInfos = new ArrayList<>(Arrays
        .asList(
            new PartitionRuleInfo(PartitionStrategy.KEY,
                ...
                "alter table " + tableName + " extract to partition hp by hot value(80)",
                true, 1, 10, "p2", 0, 0, 0, 0),
            new PartitionRuleInfo(PartitionStrategy.KEY,
                2,
                PARTITION_BY_INT_KEY, "alter table " + tableName + " extract to partition hp by hot value(100)",
                true, 2, 10, "p2", 0, 0, 0, 0),
            new PartitionRuleInfo(PartitionStrategy.KEY,
                2,
                PARTITION_BY_INT_BIGINT_KEY,
                "alter table " + tableName + " extract to partition hp by hot value(100,50)",
    ...
    final static String logicalDatabase = "AlterTableExtractTest";

    public AlterTableExtractPartitionByHotValueTest(PartitionRuleInfo curPartitionRuleInfo) {
    ...
    public void testUpdate() {
        ...
        if (!partitionRuleInfo.isPkIsPartCol()) {
            ...
            if (!isPublic) {
                ...
                Assert.assertThat(trace.toString(), updateTrace, lessThanOrEqualTo(topology.size()));
                Assert.assertThat(topology.size(), is(5));
