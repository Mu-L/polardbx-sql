...
public class AutoPartitionWithIndexColumnTypeTest extends BaseAutoPartitionNewPartition {
    ...
    private void insertAndAssertSame(final String indexPrefix) throws Exception {
        ...
        // Assert correct plan.
        Assert.assertFalse(getExplainResult(selectPrimary).contains("IndexScan"));
    ...
    public void explicitPkIndexToGlobalTest() throws Exception {
        ...
        final String res = showFullCreateTable(tddlConnection, TABLE_NAME);
    ...
    public void explicitPkAddGlobalIndexAutoPartitionTest() throws Exception {
        ...
        // Assert that 1 index(only global)
        String res = showFullCreateTable(tddlConnection, TABLE_NAME);
        System.out.println(res);
        Assert.assertFalse(res
            .contains(MessageFormat.format("KEY `{0}` ({1})", INDEX_NAME, indexColumn)));
        Assert.assertTrue(res.contains("GLOBAL INDEX "));
        ...
        JdbcUtil.executeUpdateSuccess(tddlConnection,
            MessageFormat.format("delete from `{0}` where 1=1", TABLE_NAME));
        ...
        // Assert that 1 index(only global)
        res = showFullCreateTable(tddlConnection, TABLE_NAME);
        System.out.println(res);
        Assert.assertFalse(res
            .contains(MessageFormat.format("UNIQUE KEY `{0}` ({1})", INDEX_NAME, indexColumn)));
        Assert.assertTrue(res.contains("UNIQUE GLOBAL INDEX "));
