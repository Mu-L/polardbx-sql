...
public class AlterCciDropPartitionTest extends AlterCciPartitionBaseTest {
    static List<PartitionRuleInfo> partitionRuleInfos = new ArrayList<>(Arrays
        .asList(new PartitionRuleInfo(PartitionStrategy.RANGE,
                1,
                PARTITION_BY_BIGINT_RANGE, "alter table " + tableName + "." + cciName + " drop partition p2"),
            new PartitionRuleInfo(PartitionStrategy.RANGE_COLUMNS,
                2,
                PARTITION_BY_INT_BIGINT_RANGE_COL, "alter table " + tableName + "." + cciName + " drop partition p2"),
            new PartitionRuleInfo(PartitionStrategy.LIST,
                1,
                PARTITION_BY_BIGINT_LIST, "alter table " + tableName + "." + cciName + " drop partition p2"),
            new PartitionRuleInfo(PartitionStrategy.LIST_COLUMNS,
                4,
                PARTITION_BY_INT_BIGINT_LIST, "alter table " + tableName + "." + cciName + " drop partition p2"))
    );
