...
public class UpdateTest extends DDLBaseNewDBTestCase {
    ...
    public void updateBlobTest() {
        ...
        for (String colDef : colDefs) {
            ...
            final String createGsi = "CREATE UNIQUE GLOBAL INDEX `" + gsiName
                + "` ON `"
                + tableName
                + "`(`id`) COVERING(`userId`, `zoneId`) dbpartition by hash(`id`) tbpartition by hash(`id`) tbpartitions 3";
    ...
    public void updateGsiTimestampTest() throws SQLException {
        ...
        final String createGsi = "CREATE UNIQUE GLOBAL INDEX `" + gsiName
            + "` ON `"
            + tableName
            + "`(`id`) COVERING(`c`)  DBPARTITION BY HASH(`id`)";
        JdbcUtil.executeUpdateSuccess(tddlConnection, createGsi);
    ...
    public void updateGsiBackfillTest() throws Exception {
        updateGsiBackfillTestInternal("update_gsi_backfill_test_shard_tb", "dbpartition by hash(`b`)");
    ...
    public void testUpdateCurrentTimestamp() throws SQLException {
        ...
        create =
            String.format("create global index %s on %s(c) covering (b) dbpartition by hash(c)", gsiName, tableName);
        JdbcUtil.executeUpdateSuccess(tddlConnection, create);
    ...
    public void testUpdateBinaryFunc1() throws SQLException {
        ...
        for (int i = 0; i < binaryValues.length; i++) {
            String update =
                String.format("update %s set a=a+1000, b=%s where b=%s", tableName, binaryValues[i], binaryValues[i]);
            JdbcUtil.executeUpdateSuccess(tddlConnection, update);
            JdbcUtil.executeUpdateSuccess(mysqlConnection, update);
        ...
        for (int i = 0; i < binaryValues.length; i++) {
            ...
            String update = hint + String.format("update %s set a=a+1000, b=%s where b=%s", tableName, binaryValues[i],
                binaryValues[i]);
            JdbcUtil.executeUpdateSuccess(tddlConnection, update);
            JdbcUtil.executeUpdateSuccess(mysqlConnection, update);
