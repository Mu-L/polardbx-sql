...
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isBit;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isBoolean;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isCharacterSetSensitive;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isCompatibleTime;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isFloatingType;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isFloatingWithZeroFraction;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isGeometry;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isJson;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isNumberic;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isTextOrBlob;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isTime;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.isVarCharOrBinary;
import static com.alibaba.polardbx.qatest.cdc.random.ColumnTypeUtil.parseLength;
...
public class DdlSqlBuilder {
    ...
    Pair<String, String> buildCreateIndexSql() {
        boolean flag = RandomUtils.nextBoolean();
        Optional<String> columnNameOptional = columnSeeds.COLUMN_NAME_COLUMN_TYPE_MAPPING.keySet().stream().findAny();
        if (columnNameOptional.isPresent()) {
            String columnName = columnNameOptional.get();
            String indexName = columnName + "_index";

            if (flag) {
                return Pair.of(indexName,
                    String.format("alter table `%s` add index `%s`(`%s`)", tableName, indexName, columnName));
            } else {
                return Pair.of(indexName,
                    String.format("create index `%s` on `%s`(`%s`)", indexName, tableName, columnName));
            }
        }
    ...
    Pair<String, String> buildDropIndexSql() {
        boolean flag = RandomUtils.nextBoolean();
        Optional<String> indexNameOption = columnSeeds.INDEX_SET.stream().findAny();
        if (indexNameOption.isPresent()) {
            String indexName = indexNameOption.get();
            if (flag) {
                return Pair.of(indexName, String.format("alter table `%s` drop index `%s`", tableName, indexName));
            } else {
                return Pair.of(indexName, String.format("drop index `%s` on `%s`", indexName, tableName));
            }
        }
    ...
    Pair<Pair<String, String>, String> buildModifyColumnSql() {
        ...
        if (isNumberic(columnSeed.getValue())) {
            ...
            throw new RuntimeException("invalid column type for modify " + columnSeed.getValue());
        }
