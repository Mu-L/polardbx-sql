...
//            LOG.info("getLocalityInfo" + res);
    ...
    public static void flushStorageLabel(List<String> storageNodeBeanList, Connection tddlConnection) {
        for (int i = 0; i < storageNodeBeanList.size(); i++) {
            String storageNodeBean = storageNodeBeanList.get(i);
            String flushStorageLabelSql = String.format(
                "UPDATE metadb.storage_info SET extras = JSON_SET(extras, '$.storageInstLabel', 'set%s') WHERE storage_master_inst_id = '%s'",
                i + 1, storageNodeBean);
            JdbcUtil.executeSuccess(tddlConnection, flushStorageLabelSql);
        }
        String updateDataIdVersion =
            "update metadb.config_listener set op_version = op_version + 1 where data_id like 'polardbx.storage.info%'";
        JdbcUtil.executeSuccess(tddlConnection, updateDataIdVersion);
    }
