...
public class LocalIndexTest extends DDLBaseNewDBTestCase {
    ...
    public void testUniqueKeyRollback() throws SQLException {
        ...
        dropTableIfExists(tableName);
        String createSql =
            String.format("create table %s (a int primary key, b varchar(20) unique key, c int) single",
                tableName);
        ...
        try {
            setSqlMode("", conn);
            String alterSql = String.format("alter table %s modify column b bigint, algorithm=omc", tableName);
