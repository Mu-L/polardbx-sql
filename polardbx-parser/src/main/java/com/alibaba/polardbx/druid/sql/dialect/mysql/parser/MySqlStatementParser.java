...
import static com.alibaba.polardbx.druid.sql.parser.Token.AS;
...
import static com.alibaba.polardbx.druid.sql.parser.Token.CREATE;
...
import static com.alibaba.polardbx.druid.sql.parser.Token.FULL;
...
public class MySqlStatementParser extends SQLStatementParser {
    ...
    private static final String COLUMNAR = "COLUMNAR";
...
//        accept(Token.CREATE);
        if (lexer.identifierEquals("REGISTER")) {
            lexer.nextToken();
        } else {
            accept(Token.CREATE);
        }
        ...
        if (lexer.identifierEquals("SECRET")) {
            lexer.reset(mark);
            return parseCreateSecret();
        }
        ...
        if (lexer.identifierEquals("TENANT")) {
            ...
            if (!lexer.identifierEquals("STORAGE")) {
                throw new RuntimeException(
                    String.format("parse failed! stmt %s not supported yet! only CREATE TENANT STORAGE POOL supported!",
                        lexer.text.toString()));
            }
        }
        ...
        if (lexer.token() == Token.UNIQUE
            ...
            || lexer.identifierEquals(FnvHash.Constants.VECTOR)
            ...
            || lexer.identifierEquals(FnvHash.Constants.COLUMNAR)) {
            if (replace) {
                lexer.reset(mark);
            }
...
//            Lexer.SavePoint savePoint = lexer.mark();
        ...
        if (lexer.identifierEquals(FnvHash.Constants.ROUTING_RULE)) {
            lexer.reset(mark);
            return parseCreateRoutingRule();
        }
        ...
        if (isEnabled(SQLParserFeature.DrdsCCL) && lexer.identifierEquals(FnvHash.Constants.CCL_BLOCKER)) {
            lexer.reset(mark);
            return parseCreateCclBlocker();
        }
        ...
        if (this.lexer.identifierEquals("Security")) {
            ...
            return this.parseCreateSecurity();
        }

        throw new ParserException("TODO " + this.lexer.info());
    }

    private SQLStatement parseCreateSecurity() {
        if (Token.CREATE == lexer.token()) {
            this.accept(Token.CREATE);
        }
        this.acceptIdentifier("Security");
        if (lexer.identifierEquals("LABEL")) {
            lexer.nextToken();
            if (lexer.identifierEquals("COMPONENT")) {
                lexer.nextToken();
                DrdsCreateSecurityLabelComponentStatement componentStatement =
                    new DrdsCreateSecurityLabelComponentStatement();
                SQLExpr componentName = new SQLIdentifierExpr(lexer.stringVal());
                lexer.nextToken();
                SQLExpr componentType = new SQLIdentifierExpr(lexer.stringVal());
                lexer.nextToken();
                SQLExpr componentContent = exprParser.expr();

                componentStatement.setComponentName(componentName);
                componentStatement.setComponentType(componentType);
                componentStatement.setComponentContent(componentContent);
                return componentStatement;
            } else {
                DrdsCreateSecurityLabelStatement labelStatement = new DrdsCreateSecurityLabelStatement();
                SQLPropertyExpr propertyExpr = (SQLPropertyExpr) exprParser.expr();
                SQLExpr policyName = new SQLIdentifierExpr(propertyExpr.getOwnerName());
                SQLExpr labelName = new SQLIdentifierExpr(propertyExpr.getName());
                SQLExpr labelContent = exprParser.expr();

                labelStatement.setLabelName(labelName);
                labelStatement.setPolicyName(policyName);
                labelStatement.setLabelContent(labelContent);
                return labelStatement;
            }
        } else if (lexer.identifierEquals("POLICY")) {
            lexer.nextToken();
            DrdsCreateSecurityPolicyStatement policyStatement = new DrdsCreateSecurityPolicyStatement();
            SQLExpr policyName = new SQLIdentifierExpr(lexer.stringVal());
            lexer.nextToken();
            this.acceptIdentifier("COMPONENTS");
            SQLExpr policyComponents = exprParser.expr();

            policyStatement.setPolicyName(policyName);
            policyStatement.setPolicyComponents(policyComponents);
            return policyStatement;
        } else if (lexer.identifierEquals("ENTITY")) {
            lexer.nextToken();
            SQLCreateLBACSecurityEntityStatement stmt = new SQLCreateLBACSecurityEntityStatement();
            SQLName entityType = exprParser.name();
            SQLName entityKey = exprParser.name();
            SQLName entityAttr = exprParser.name();
            stmt.setEntityType(entityType);
            stmt.setEntityKey(entityKey);
            stmt.setEntityAttr(entityAttr);
            return stmt;
        }
    ...
    public SQLCreateIndexStatement parseCreateIndex(boolean acceptCreate) {
        ...
        if (stmt.getIndexDefinition().getDbName() != null) {
            stmt = new SQLCreateIndexInDatabaseStatement(stmt.getIndexDefinition().getDbName(),
                stmt.getIndexDefinition().isColumnar(), stmt.getIndexDefinition().isIn());
        }
    ...
    public SQLStatement parseCreateUser() {
        ...
        MySqlCreateUserStatement.UserAccountType accountType;
        if (lexer.token() == Token.USER) {
            accountType = MySqlCreateUserStatement.UserAccountType.NORMAL;
            lexer.nextToken();
        } else if (lexer.token() == Token.DBA_USER) {
            accountType = MySqlCreateUserStatement.UserAccountType.DBA;
            lexer.nextToken();
        } else {
            throw new ParserException("Unknown user type: " + lexer.info());
        }

        MySqlCreateUserStatement stmt = new MySqlCreateUserStatement();
        stmt.setAccountType(accountType);
        ...
        for (; ; ) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.IDENTIFIED)) {
                ...
                if (lexer.token() == Token.BY) {
                    ...
                    if (lexer.token() == Token.BY ||
                        lexer.token() == AS) {
                        userSpec.setPluginAs(lexer.token() == AS);
                        lexer.nextToken();
    ...
    public MySqlOptimizeStatement parseOptimize() {
        ...
        if (lexer.identifierEquals("LOCAL")) {
            lexer.nextToken();
            stmt.setLocal(true);
        } else if (lexer.identifierEquals("NO_WRITE_TO_BINLOG")) {
            lexer.nextToken();
    ...
    public SQLStatement parseReset() {
        ...
        if (lexer.identifierEquals("SLAVE") || lexer.identifierEquals("REPLICA")) {
            MySqlResetSlaveStatement stmt = new MySqlResetSlaveStatement();
            lexer.nextToken();
            if (lexer.token() == Token.ALL) {
                lexer.nextToken();
                stmt.setAll(true);
            ...
            if (lexer.token() == FOR) {
                ...
                } else if (lexer.identifierEquals("SUB_CHANNEL")) {
                    lexer.nextToken();
                    SQLCharExpr subChannel = this.exprParser.charExpr();
                    stmt.setSubChannel(subChannel);
                } else if (lexer.identifierEquals("DDL_LOAD")) {
                    lexer.nextToken();
                    stmt.setDdlLoad(true);
                } else {
                    setErrorEndPos(lexer.pos());
                    throw new ParserException("syntax error, expect channel or sub channel" + ", actual "
                        + lexer.token() + ", " + lexer.info());
                }
            }
            return stmt;
        } else if (lexer.identifierEquals("MASTER")) {
            return parseResetMaster();
        }
    ...
    public boolean parseStatementListDialect(List<SQLStatement> statementList) {
        ...
        if (lexer.identifierEquals("START")) {
            ...
            if (lexer.identifierEquals("TRANSACTION")) {
                ...
                MySqlStartMasterStatement stmt = parseStartMaster();
                statementList.add(stmt);
            }
            return true;
        }
        ...
        if (lexer.identifierEquals("STOP")) {
            ...
            if (lexer.identifierEquals("SLAVE") || lexer.identifierEquals("REPLICA")) {
                MySqlStopSlaveStatement stmt = parseStopSlave();
                statementList.add(stmt);
            } else if (lexer.identifierEquals("MASTER")) {
                MySqlStopMasterStatement stmt = parseStopMaster();
                statementList.add(stmt);
            }
            return true;
        }

        if (lexer.identifierEquals("RESTART")) {
            acceptIdentifier("RESTART");
            if (lexer.identifierEquals("MASTER")) {
                MySqlRestartMasterStatement stmt = parseRestartMaster();
                statementList.add(stmt);
            }
            return true;
        }
        ...
        if (lexer.identifierEquals("CLEAR")) {
            ...
            if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("DDL")) {
                ...
                statementList.add(stmt);
                return true;
            } else if (isEnabled(SQLParserFeature.DrdsCCL) && lexer.identifierEquals("CCL_BLOCKERS")) {
                lexer.nextToken();
                DrdsClearCclBlockersStatement stmt = new DrdsClearCclBlockersStatement();
                statementList.add(stmt);
                return true;
                ...
                SQLName name = this.exprParser.name();
                DrdsClearFileStorageStatement stmt = new DrdsClearFileStorageStatement();
                stmt.setName(name);
                statementList.add(stmt);
                return true;
            }
        ...
        if (lexer.identifierEquals("REPLICA")) {
            SQLStatement stmt = parseReplica();
            statementList.add(stmt);
            return true;
        }
        ...
        if (lexer.identifierEquals("SKIP")) {
            SQLStatement stmt = parseSkipSubjob();
            statementList.add(stmt);
            return true;
        }
        ...
        if (lexer.identifierEquals(FnvHash.Constants.IMPORT)) {
            ...
            if (lexer.token() == Token.TABLE) {
                ...
                SQLStatement stmt = parseImportSequence();
                statementList.add(stmt);
            }
            return true;
        }
        ...
        if (lexer.identifierEquals("RELOAD")) {
            SQLStatement stmt = parseReload();
            statementList.add(stmt);
            return true;
        }
        ...
        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.token() == CONTINUE) {
            ...
            if (identifierEquals("DDL")) {
                ...
                statementList.add(stmt);
                return true;
            } else if (identifierEquals("REBALANCE")) {
                acceptIdentifier("REBALANCE");
                return parseGenericDdlCommand(new DrdsContinueDDLJob(), statementList);
            }
        }
        ...
        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals(FnvHash.Constants.PAUSE)) {
            ...
            if (identifierEquals("DDL")) {
                ...
                acceptIdentifier("REBALANCE");
                return parseGenericDdlCommand(new DrdsPauseRebalanceJob(), statementList);
            }
        }
        ...
        if (lexer.identifierEquals("INSPECT")) {
            ...
            if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("DDL")) {
                ...
                // INSPECT SEQUENCE RANGE FOR <seq_name>
                lexer.nextToken();
                ...
                DrdsInspectSeqRangeStatement stmt = new DrdsInspectSeqRangeStatement();
                stmt.setName(name);
                statementList.add(stmt);
                return true;
            } else if (lexer.token() == Token.INDEX) {
                //inspect index
                DrdsInspectIndexStatement stmt = new DrdsInspectIndexStatement();
                lexer.nextToken();
                if (full) {
                    stmt.setFull(true);
                }

                //inspect index from table
                if (lexer.token() == FROM) {
                    lexer.nextToken();
                    SQLName tableName = exprParser.name();
                    stmt.setFromTable(true);
                    stmt.setTableName(tableName);
                } else {
                    stmt.setFromTable(false);
                }

                if (lexer.identifierEquals(FnvHash.Constants.MODE)) {
                    lexer.nextToken();
                    accept(EQ);
                    if (lexer.identifierEquals("STATIC")) {
                        lexer.nextToken();
                        stmt.setMode("STATIC");
                    } else if (lexer.identifierEquals("DYNAMIC")) {
                        lexer.nextToken();
                        stmt.setMode("DYNAMIC");
                    } else if (lexer.identifierEquals("MIXED")) {
                        lexer.nextToken();
                        stmt.setMode("MIXED");
                    } else {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.CONVERT)) {
            ...
            if (lexer.token() == CREATE) {
                /**
                 * 内部语法: convert create table tb0(xxx) dbpartition by hash(xxx)
                 * 用于把drds模式的建表语句转换成auto模式
                 * */
                SQLCreateTableStatement createTableStatement = parseCreateTable();
                createTableStatement.setOnlyConvertTableMode(true);
                statementList.add(createTableStatement);
                return true;
            } else {
                //convert sequence
                accept(ALL);
        ...
        if (isEnabled(SQLParserFeature.DRDSBaseline) && lexer.identifierEquals("BASELINE")) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.GRAY_RATIO)) {
                lexer.nextToken();
                int grayRatio = lexer.integerValue().intValue();
                stmt.setGrayRatio(grayRatio);
                lexer.nextToken();
            }

            if (lexer.identifierEquals(FnvHash.Constants.EXPR)) {
                lexer.nextToken();
                SQLExpr expr = this.exprParser.expr();
                stmt.setExpr(expr);
            }
        ...
        if (isEnabled(SQLParserFeature.DrdsMisc)) {
            if (lexer.identifierEquals("UPGRADE")) {
                ...
                lexer.nextToken();
                if (lexer.identifierEquals(FnvHash.Constants.EXTERNAL)) {
                    SQLStatement stmt = parseRefreshExternal();
                    statementList.add(stmt);
                    return true;
                }
                // REFRESH LOCAL RULES
                ...
                statementList.add(stmt);
                return true;
            } else if (lexer.identifierEquals("WARMUP")) {

                // case1. warmup(cron) /*HINT*/ select ...
                // case2. warmup /*HINT*/ select ...
                // case3. warmup delete/resume/suspend id
                // case4. warmup delete/resume/suspend all
                // case5. warmup(cron) {select ...; select ...;}
                lexer.nextToken();

                // warmup delete/resume/suspend id
                // warmup delete/resume/suspend all
                if (lexer.token() == Token.DELETE
                    || lexer.identifierEquals("RESUME")
                    || lexer.identifierEquals("SUSPEND")) {

                    String tokenName = lexer.stringVal();

                    lexer.nextToken();

                    ColumnarWarmupControlStatement columnarWarmupControlStatement
                        = new ColumnarWarmupControlStatement(tokenName);
                    if (lexer.token() == Token.ALL) {
                        columnarWarmupControlStatement.setAll(true);
                    } else {
                        Number taskId = lexer.integerValue();
                        if (taskId instanceof Long || taskId instanceof Integer) {
                            long taskIdLongVal = taskId.longValue();
                            columnarWarmupControlStatement.setTaskId(taskIdLongVal);
                        } else {
                            throw new ParserException();
                        }
                    }
                    ...
                    statementList.add(columnarWarmupControlStatement);
                    return true;
                }

                // warmup(cron) /*HINT*/ select ...
                // warmup /*HINT*/ select ...
                ColumnarWarmupStatement stmt = new ColumnarWarmupStatement();
                if (lexer.token() == LPAREN) {
                    // warmup(cron expression)
                    lexer.nextToken();
                    String cronExpression = lexer.stringVal();
                    stmt.setCronExpression(cronExpression);
                    lexer.nextToken();
                    accept(RPAREN);
                }

                if (lexer.token() == Token.LBRACE) {
                    List<SQLCommentHint> hintsForAllSelects = new ArrayList<>();
                    while (lexer.token() == Token.LBRACE) {
                        ...
                        // Parse select.
                        SQLCommentHint hintOfSelect = new SQLCommentHint();
                        if (lexer.token() == Token.HINT) {
                            List<SQLCommentHint> hints = this.exprParser.parseHints();
                            if (hints != null && hints.size() > 0) {
                                hintOfSelect = hints.get(0);
                            }
                        }
                        hintsForAllSelects.add(hintOfSelect);
                        ...
                        lexer.setCommentHandler(null);

                        accept(Token.RBRACE);
                    }
                    stmt.setHeadHints(hintsForAllSelects);
                } else {
                    // Parse select.
                    SQLCommentHint hint = new SQLCommentHint();
                    if (lexer.token() == Token.HINT) {
                        List<SQLCommentHint> hints = this.exprParser.parseHints();
                        if (hints != null && hints.size() > 0) {
                            hint = hints.get(0);
                    ...
                    MySqlSelectParser selectParser = createSQLSelectParser();
                    stmt.setSelect(selectParser.select());
                    stmt.setHeadHints(Lists.newArrayList(hint));
                }
        ...
        if (lexer.identifierEquals("REBALANCE")) {
            lexer.nextToken();
            if (lexer.identifierEquals("MASTER")) {
                MySqlRebalanceMasterStatement stmt = parseRebalanceMaster();
                statementList.add(stmt);
            } else {
        ...
        if (lexer.token() == Token.HINT) {
            ...
            case IDENTIFIER:
                acceptHint = lexer.hash_lower() == FnvHash.Constants.DUMP
                    ...
                    || lexer.identifierEquals("SKIP")
                    || lexer.identifierEquals("UNARCHIVE")
                    || lexer.identifierEquals("IMPORT")
                    || lexer.identifierEquals("COLLECT")
                    || lexer.identifierEquals("REGISTER")
                    || lexer.identifierEquals("UNREGISTER")
                    || lexer.identifierEquals("MODIFY")
                    || lexer.identifierEquals("FIRE")
                ;
                break;
            default:
                break;
        ...
        if (lexer.token() == Token.CHECK) {
            ...
            if (lexer.identifierEquals("PHYSICAL")) {
                physical = true;
                lexer.nextToken();
            }
            if (lexer.token() == Token.TABLE) {
                ...
                if (lexer.identifierEquals(FnvHash.Constants.ROUTING)) {
                    /**
                     * <pre>
                     *     check table routing `xxx_db`.`xxx_tb` [partition(`xxx_p1`)] [explain = true]
                     * </pre>
                     */
                    lexer.nextToken();
                    DrdsCheckTableRouting stmt = new DrdsCheckTableRouting();
                    SQLName table = this.exprParser.name();
                    stmt.setTableName(table);
                    if (lexer.token() == Token.PARTITION) {
                        lexer.nextToken();
                        accept(Token.LPAREN);
                        this.exprParser.names(stmt.getPartitions(), table);
                        accept(Token.RPAREN);
                    }
                    ...
                    if (lexer.token() == Token.EXPLAIN) {
                        ...
                        if (lexer.token() == TRUE) {
                            accept(TRUE);
                            stmt.setExplain(true);
                        } else {
                            accept(FALSE);
                            stmt.setExplain(false);
                        }
                    }
                ...
                MySqlCheckTableGroupStatement stmt = new MySqlCheckTableGroupStatement();
                if (lexer.token() != Token.EOF
                    && lexer.token() == Token.IDENTIFIER) {
                    if (collectTableName) {
                        for (; ; ) {
                            SQLName tableGroup = this.exprParser.name();
                            stmt.addTableGroup(tableGroup);
                ...
                statementList.add(stmt);
            } else if (isEnabled(SQLParserFeature.DrdsGSI) && lexer.token() == Token.INDEX) {
                accept(Token.INDEX);
                if (lexer.identifierEquals(FnvHash.Constants.ROUTING)) {
                    parseAndAddCheckRoutingStmt(statementList);
                } else {
                    setErrorEndPos(lexer.pos());
            ...
            } else if (isEnabled(SQLParserFeature.DrdsGSI) && lexer.identifierEquals(FnvHash.Constants.COLUMNAR)) {
                lexer.nextToken();
                if (lexer.token() == Token.INDEX) {
                    accept(Token.INDEX);

                    DrdsCheckColumnarIndex checkColumnarIndex = new DrdsCheckColumnarIndex();
                    checkColumnarIndex.setIndexName(exprParser.name());
                    if (ON == lexer.token()) {
                        lexer.nextToken();
                        checkColumnarIndex.setTableName(exprParser.name());
                    }
                ...
                } else if (lexer.token() == Token.PARTITION || lexer.identifierEquals("PARTITION")) {
                    lexer.nextToken();
            ...
            } else if (lexer.identifierEquals("REPLICA")) {
                lexer.nextToken();
                accept(TABLE);
                SQLName dbName = new SQLIdentifierExpr(lexer.stringVal(), 0);
                lexer.nextToken();
                SQLName tbName = null;
                if (lexer.token() == Token.DOT) {
                    lexer.nextToken();
                    tbName = new SQLIdentifierExpr(lexer.stringVal(), 0);
                    lexer.nextToken();
                }
                ...
                if (lexer.identifierEquals("CHANNEL")) {
                    ...
                    lexer.nextToken();
                    SQLPauseReplicaCheckTableStatement stmt = new SQLPauseReplicaCheckTableStatement();
                    stmt.setDbName(dbName);
                    stmt.setTableName(tbName);
                    statementList.add(stmt);
                } else if (lexer.token() == CONTINUE) {
                    lexer.nextToken();
                    SQLContinueReplicaCheckTableStatement stmt = new SQLContinueReplicaCheckTableStatement();
                    stmt.setDbName(dbName);
                    stmt.setTableName(tbName);
                    statementList.add(stmt);
                } else if (lexer.identifierEquals("CANCEL")) {
                    lexer.nextToken();
                    SQLCancelReplicaCheckTableStatement stmt = new SQLCancelReplicaCheckTableStatement();
                    stmt.setDbName(dbName);
                    stmt.setTableName(tbName);
                    statementList.add(stmt);
                } else if (lexer.identifierEquals("RESET")) {
                    lexer.nextToken();
                    SQLResetReplicaCheckTableStatement stmt = new SQLResetReplicaCheckTableStatement();
                    stmt.setDbName(dbName);
                    stmt.setTableName(tbName);
                    statementList.add(stmt);
                } else if (lexer.token() == Token.SHOW) {
                    lexer.nextToken();
                    if (lexer.identifierEquals("PROGRESS")) {
                        lexer.nextToken();
                        SQLShowReplicaCheckProgressStatement stmt = new SQLShowReplicaCheckProgressStatement();
                        stmt.setDbName(dbName);
                        stmt.setTableName(tbName);
                        statementList.add(stmt);
                    } else if (lexer.identifierEquals("DIFF")) {
                        lexer.nextToken();
                        SQLShowReplicaCheckDiffStatement stmt = new SQLShowReplicaCheckDiffStatement();
                        stmt.setDbName(dbName);
                        stmt.setTableName(tbName);
                        statementList.add(stmt);
                    }
    ...
    private void parseAndAddCheckRoutingStmt(List<SQLStatement> statementList) {
        /**
         * <pre>
         *     check index routing  `xxx_idx` [partition(`xxx_p1`)] on `xxx_db`.`xxx_tb` [explain = true]
         * </pre>
         */
        lexer.nextToken();
        DrdsCheckTableRouting stmt = new DrdsCheckTableRouting();
        stmt.setCheckIndexRouting(true);
        ...
        if (lexer.token() == Token.PARTITION) {
            lexer.nextToken();
            accept(Token.LPAREN);
            this.exprParser.names(stmt.getPartitions(), indexName);
            accept(Token.RPAREN);
        }
        accept(Token.ON);
        SQLName tableName = this.exprParser.name();
    ...
    private SQLStatement parseRebalance(boolean logicalDdl) {
        ...
        if (lexer.token() == Token.TABLE) {
        ...
        } else if (lexer.identifierEquals("TENANTDB")) {
            lexer.nextToken();
            stmt.setRebalanceTenantDb();
            SQLName storagePoolName = this.exprParser.name();
            stmt.setTableSource(new SQLExprTableSource(storagePoolName));
        } else if (lexer.identifierEquals("TENANT")) {
            lexer.nextToken();
            stmt.setRebalanceTenant();
            SQLName storagePoolName = this.exprParser.name();
            stmt.setTableSource(new SQLExprTableSource(storagePoolName));
        } else if (lexer.token() == Token.DATABASE) {
            lexer.nextToken();
            ...
            throw new ParserException("only REBALANCE DATABASE/TABLE/CLUSTER/TABLEGROUP/TENANT_DB supported");
        }
    ...
    private SQLStatement parseCancelJob() {
        ...
        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("REBALANCE")) {
            // CANCEL REBALANCE { ALL | <job_id> [, <job_id>] ... }
            lexer.nextToken();
            DrdsCancelRebalanceJob stmt = new DrdsCancelRebalanceJob();
        ...
        if (lexer.identifierEquals("COLLECT")) {
            lexer.nextToken();
            acceptIdentifier("STATISTIC");
            DrdsCancelCollectStatisticStatement stmt = new DrdsCancelCollectStatisticStatement();
            List<SQLExpr> connectionIds = null;
            if (lexer.token() == Token.LITERAL_INT) {
                connectionIds = new ArrayList<>();
                connectionIds.add(this.exprParser.integerExpr());
                while (lexer.token() == Token.COMMA) {
                    lexer.nextToken();
                    connectionIds.add(this.exprParser.integerExpr());
                }
            }
            stmt.setConnectionIds(connectionIds);
            return stmt;
        }
    ...
    private SQLStatement parseSkipSubjob() {
        lexer.nextToken();

        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("REBALANCE")) {
            // SKIP REBALANCE SUBJOB { ALL | <job_id> [, <job_id>] ... }
            lexer.nextToken();
            if (lexer.identifierEquals("SUBJOB")) {
                lexer.nextToken();
                DrdsSkipRebalanceSubjob stmt = new DrdsSkipRebalanceSubjob();
                parseGenericDdlCommand(stmt);
                return stmt;
            }
        }

        throw new ParserException(String.format("unexpect '%s' after skip", lexer.stringVal()));
    }
    ...
    protected SQLStatement parseImportDB() {
        ...
        if (lexer.token() == Token.IF) {
            accept(Token.IF);
            accept(Token.EXISTS);
            stmt.setExistStillImportTag(true);
        }

        SQLName srcPhyDb = this.exprParser.name();
        SQLName logicalDb = null;
        if (lexer.token() == Token.AS) {
            accept(AS);
            logicalDb = this.exprParser.name();
        } else {
            logicalDb = srcPhyDb.clone();
        }
        ...
        stmt.setSrcPhyDb(srcPhyDb);
        stmt.setDstLogicalDb(logicalDb);
...
//        }
    ...
    protected SQLStatement parseImportSequence() {
        accept(Token.SEQUENCE);
        accept(FROM);
        SQLName logicalDb = this.exprParser.name();

        SQLImportSequenceStatement stmt = new SQLImportSequenceStatement();
        stmt.setLogicalDatabase(logicalDb);
    ...
    public SQLStatement parseReload() {
        acceptIdentifier("RELOAD");

        if (lexer.token() == Token.TABLE) {
            DrdsReloadTableStatement stmt = new DrdsReloadTableStatement();
            ...
            if (lexer.identifierEquals("PREEMPTIVE")) {
                lexer.nextToken();
                stmt.setPreemptive(true);
            }

            return stmt;
        }

        throw new RuntimeException(String.format("parse failed! stmt %s not supported yet!", lexer.text.toString()));
    }
    ...
    public SQLStatement parseFlush() {
        ...
        for (; ; ) {
            if (lexer.token() == Token.BINARY || lexer.identifierEquals("BINARY")) {
                ...
                lexer.nextToken();
                MySqlFlushLogsStatement flushLogsStatement = new MySqlFlushLogsStatement();
                if (lexer.token() == WITH) {
                    lexer.nextToken();
                    flushLogsStatement.setWith(this.exprParser.expr());
                }
                return flushLogsStatement;
    ...
    public SQLStatement parseDescribe() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.EXTERNAL)) {
            Lexer.SavePoint mark = lexer.mark();
            lexer.nextToken();
            if (lexer.identifierEquals("CATALOG")) {
                lexer.nextToken();
                // DESC EXTERNAL CATALOG <name>: the token after CATALOG must start a name.
                // Otherwise (e.g. EOF) this is DESC <table "external"> <column "catalog">,
                // a valid plain DESCRIBE — fall back via reset.
                Token t = lexer.token();
                if (t == Token.IDENTIFIER || t == Token.LITERAL_ALIAS
                    || t == Token.LITERAL_CHARS || t == Token.VARIANT) {
                    MySqlDescribeExternalCatalogStatement stmt = new MySqlDescribeExternalCatalogStatement();
                    stmt.setName(this.exprParser.name());
                    return stmt;
                }
            }
    ...
    private MySqlExplainStatement parseExplain(MySqlExplainStatement explain) {
        ...
        if (lexer.token() == Token.ANALYZE) {
            ...
            if (lexer.identifierEquals("EXECUTE")) {
                lexer.nextToken();
                explain.setType("ANALYZE_EXECUTE");
            }
        } else if (lexer.identifierEquals("DIFF_EXECUTE")) {
            lexer.nextToken();
            explain.setType("DIFF_EXECUTE");
        } else if (lexer.identifierEquals("ALL_EXECUTE")) {
            lexer.nextToken();
            explain.setType("ALL_EXECUTE");
        } else if (lexer.identifierEquals("ANALYZE_EXECUTE")) {
            lexer.nextToken();
            explain.setType("ANALYZE_EXECUTE");
        } else if (lexer.identifierEquals("JSON_EXECUTE")) {
            lexer.nextToken();
            explain.setType("JSON_EXECUTE");
        } else if (lexer.identifierEquals("TREE_EXECUTE")) {
            lexer.nextToken();
            explain.setType("TREE_EXECUTE");
            ...
            lexer.nextToken();
            if (lexer.identifierEquals("DETAIL")) {
                lexer.nextToken();
                explain.setType("OPTIMIZER_DETAIL");
            } else {
        ...
        } else if (lexer.identifierEquals("ROUTING")) {
            lexer.nextToken();
            explain.setType("ROUTING");
        } else if (lexer.identifierEquals("KEYWORD")) {
            lexer.nextToken();
            explain.setType("KEYWORD");
        ...
        } else if (lexer.identifierEquals("COST_TRACE")) {
            lexer.nextToken();
        ...
        } else if (lexer.identifierEquals("ONLINE_DDL")) {
            lexer.nextToken();
            explain.setType("ONLINE_DDL");
        } else if (lexer.identifierEquals("SCHEDULE")) {
            lexer.nextToken();
            explain.setType("SCHEDULE");
        } else if (lexer.identifierEquals("DDL_DAG")) {
            lexer.nextToken();
            explain.setType("DDL_DAG");
        }
        ...
        if (lexer.token() == Token.IDENTIFIER) {
            ...
            } else if (hash == FnvHash.Constants.RENAME) {
                // do nothing
            } else if (hash == FnvHash.Constants.CONVERT) {
                // do nothing
            } else if (hash == FnvHash.Constants.MOVE) {
                // do nothing
            } else {
    ...
    public SQLStatement parseShow() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.EXTERNAL)) {
            Lexer.SavePoint externalMark = lexer.mark();
            lexer.nextToken();
            if (lexer.identifierEquals("CATALOGS")) {
                lexer.nextToken();
                MySqlShowExternalCatalogsStatement stmt = new MySqlShowExternalCatalogsStatement();
                if (lexer.token() == Token.LIKE) {
                    lexer.nextToken();
            ...
            lexer.reset(externalMark);
        }

        if (lexer.identifierEquals("SECRETS")) {
            lexer.nextToken();
            MySqlShowSecretsStatement stmt = new MySqlShowSecretsStatement();
            if (lexer.token() == Token.LIKE) {
                lexer.nextToken();
                stmt.setLike(this.exprParser.expr());
            }
        ...
        if (lexer.identifierEquals("CONNECTORS")) {
            lexer.nextToken();
            MySqlShowConnectorsStatement stmt = new MySqlShowConnectorsStatement();
            if (lexer.token() == Token.LIKE) {
                lexer.nextToken();
                stmt.setLike(this.exprParser.expr());
            }
        ...
        if (lexer.token() == Token.CREATE) {
            Lexer.SavePoint createMark = lexer.mark();
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.EXTERNAL)) {
                lexer.nextToken();
                acceptIdentifier("CATALOG");
                MySqlShowCreateExternalCatalogStatement stmt = new MySqlShowCreateExternalCatalogStatement();
                stmt.setName(this.exprParser.name());
                return stmt;
            }
            if (lexer.identifierEquals("SECRET")) {
                lexer.nextToken();
                MySqlShowCreateSecretStatement stmt = new MySqlShowCreateSecretStatement();
                stmt.setName(this.exprParser.name());
                return stmt;
            }
            lexer.reset(createMark);
        }
        ...
        if (full && lexer.identifierEquals("CONNECTORS")) {
            lexer.nextToken();
            MySqlShowConnectorsStatement stmt = new MySqlShowConnectorsStatement(true);
            if (lexer.token() == Token.LIKE) {
                lexer.nextToken();
                stmt.setLike(this.exprParser.expr());
            }
        ...
        if (lexer.identifierEquals(FnvHash.Constants.STATS)) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.LIST)) {
                showStats.setList(true);
                lexer.nextToken();
            }
            return showStats;
        }

        if (lexer.identifierEquals(FnvHash.Constants.ROUTING_RULES)) {
            lexer.nextToken();
            SQLShowRoutingRulesStatement stmt = new SQLShowRoutingRulesStatement();
        ...
        if (isEnabled(SQLParserFeature.DrdsCCL) && lexer.identifierEquals(FnvHash.Constants.CCL_BLOCKERS)) {
            lexer.nextToken();
            DrdsShowCclBlockerStatement showCclBlockers = new DrdsShowCclBlockerStatement();
            showCclBlockers.setAll(true);
            return showCclBlockers;
        }

        if (isEnabled(SQLParserFeature.DrdsCCL) && lexer.identifierEquals(FnvHash.Constants.CCL_BLOCKER)) {
            lexer.nextToken();
            DrdsShowCclBlockerStatement showCclBlocker = new DrdsShowCclBlockerStatement();
            SQLName sqlName = exprParser.name();
            List<SQLName> blockerNames = Lists.newArrayList();
            blockerNames.add(sqlName);
            while (lexer.token() == Token.COMMA) {
                lexer.nextToken();
                blockerNames.add(exprParser.name());
            }
            showCclBlocker.setblockerNames(blockerNames);
            return showCclBlocker;
        }
        ...
        if (lexer.identifierEquals(COLUMNAR)) {
            lexer.nextToken();
            if ((Token.INDEX == lexer.token() || lexer.identifierEquals("INDEXES"))) {
                lexer.nextToken();

                DrdsShowColumnarIndex stmt = new DrdsShowColumnarIndex();
                if (Token.FROM == lexer.token()) {
                    lexer.nextToken();
                    stmt.setTableName(this.exprParser.name());
                }
                ...
                if (lexer.token() == Token.AS) {
                    ...
                    if (lexer.token() == Token.OF) {
                        ...
                        if (lexer.identifierEquals("TSO")) {
                            ...
                            stmt.setTso(this.exprParser.expr());
                        }
                    }
                }
                return stmt;
            } else if (lexer.identifierEquals("TRANS")) {
                lexer.nextToken();
                DrdsShowTransStatement stmt = new DrdsShowTransStatement();
                stmt.setColumnar(true);
                return stmt;
            }
        }

        if (lexer.identifierEquals("EXPAND")) {
            lexer.nextToken();
            acceptIdentifier(STATUS);

            DrdsShowExpandStatus stmt = new DrdsShowExpandStatus();
        ...
        if (lexer.identifierEquals(CDC)) {
            lexer.nextToken();
            if (lexer.identifierEquals("STORAGE")) {
                lexer.nextToken();
                return new MySqlShowCdcStorageStatement();
            }
        }

        if (lexer.token() == Token.BINARY) {
            lexer.nextToken();
            if (lexer.identifierEquals("STREAMS")) {
                lexer.nextToken();
                MySqlShowBinaryStreamsStatement stmt = new MySqlShowBinaryStreamsStatement();
                stmt.setFull(full);
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.expr());
                }
                return stmt;
            } else {
                acceptIdentifier("LOGS");
                MySqlShowBinaryLogsStatement stmt = new MySqlShowBinaryLogsStatement();
                stmt.setFull(full);
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.expr());
        ...
        if (lexer.identifierEquals("MASTER")) {
            ...
            if (lexer.identifierEquals("LOGS")) {
                lexer.nextToken();
                MySqlShowMasterLogsStatement stmt = new MySqlShowMasterLogsStatement();
                stmt.setFull(full);
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.expr());
                ...
                acceptIdentifier(STATUS);
                MySqlShowMasterStatusStatement stmt = new MySqlShowMasterStatusStatement();
                stmt.setFull(full);
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.expr());
        ...
        if (lexer.identifierEquals(BINLOG)) {
            ...
            if (lexer.identifierEquals(EVENTS)) {
                ...
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.primary());
                }
                ...
                acceptIdentifier(STATUS);
                MySqlShowBinlogDumpStatusStatement stmt = new MySqlShowBinlogDumpStatusStatement();
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    stmt.setWith(this.exprParser.primary());
                }
        ...
        if (lexer.token() == Token.JAVA || lexer.identifierEquals("JAVA")) {
            lexer.nextToken();
            if (lexer.identifierEquals("FUNCTIONS")) {
                lexer.nextToken();
                MySqlShowJavaFunctionsStatement stmt = new MySqlShowJavaFunctionsStatement();
                if (lexer.token() == Token.LIKE) {
                    lexer.nextToken();
                    stmt.setLike(this.exprParser.expr());
                }
            ...
            throw new ParserException("syntax error, expect FUNCTIONS, actual " + lexer.token() + " " + lexer.info());
        }
        ...
        if (lexer.token() == Token.CREATE) {
            ...
            if (lexer.token() == Token.JAVA || lexer.identifierEquals("JAVA")) {
                ...
                MySqlShowCreateJavaFunctionStatement stmt = new MySqlShowCreateJavaFunctionStatement();
                stmt.setName(this.exprParser.name());
                return stmt;
            }
            ...
            if (lexer.identifierEquals("TABLEGROUP")) {
                lexer.nextToken();
                DrdsShowCreateTableGroup stmt = new DrdsShowCreateTableGroup();
...
//        }
        ...
        // SHOW AI FUNCTION [FROM xxx] | SHOW AI MODEL [FROM xxx]
        if (lexer.identifierEquals("AI")) {
            lexer.nextToken();
            if (lexer.token() == Token.FUNCTION) {
                lexer.nextToken();
                DrdsShowAiFunctionStatement stmt = new DrdsShowAiFunctionStatement();
                if (lexer.token() == Token.FROM) {
                    lexer.nextToken();
                    stmt.setFunctionName(lexer.stringVal());
                    lexer.nextToken();
                }
                return stmt;
            }
            if (lexer.identifierEquals("MODEL")) {
                lexer.nextToken();
                DrdsShowAiModelStatement stmt = new DrdsShowAiModelStatement();
                if (lexer.token() == Token.FROM) {
                    lexer.nextToken();
                    stmt.setModelName(lexer.stringVal());
                    lexer.nextToken();
                }
                return stmt;
            }
            throw new ParserException("syntax error, expect FUNCTION or MODEL after AI, actual "
                + lexer.token() + ", " + lexer.info());
        }
        ...
        if (lexer.identifierEquals("TRANS")) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.STATS)) {
                lexer.nextToken();
                return new DrdsShowTransStatsStatement();
            }
        ...
        if (lexer.token() == Token.TABLE) {
            ...
            if (lexer.identifierEquals("REPLICATE")) {
                lexer.nextToken();

                if (lexer.identifierEquals(STATUS)) {
                    acceptIdentifier(STATUS);

                    DrdsShowTableReplicate stmt = new DrdsShowTableReplicate();
                    stmt.setFull(full);
                    if (lexer.token() == Token.WHERE) {
                        lexer.nextToken();
                        SQLExpr where = exprParser.expr();
        ...
        if (lexer.identifierEquals("PARTITIONS")) {
            ...
            if (lexer.identifierEquals("META")) {
                lexer.nextToken();
                stmt.setShowPartMeta(true);
            }

            if (lexer.token() == Token.OF) {

                /*
                 * SHOW PARTITIONS [META] OF INDEX xxx_idx FROM xxx_tbl
                 *
                 * */
                ...
                if (lexer.token() != Token.INDEX) {
                    throw new ParserException(
                        "syntax error, expected 'of index [index_name]'");
                }
                lexer.nextToken();
                SQLName indexName = exprParser.name();
                stmt.setShowIndexPartMeta(true);
                stmt.setIndexName(indexName);
            }
        ...
        if (lexer.identifierEquals("DDL")) {
            ...
            if (isPhysical) {
                DrdsShowPhysicalDdl drdsShowPhysicalDdl = new DrdsShowPhysicalDdl();
                drdsShowPhysicalDdl.setFull(true);
                if (lexer.identifierEquals("STATUS")) {
                    lexer.nextToken();
                    drdsShowPhysicalDdl.setStatus(true);
                }
        ...
        if (lexer.identifierEquals("PRUNE")) {
            lexer.nextToken();
            lexer.nextToken();
            MySqlShowPruneTraceStatement stmt = new MySqlShowPruneTraceStatement();
        ...
        if (lexer.identifierEquals(FnvHash.Constants.HOTKEY)) {
            lexer.nextToken();

            MySQLShowHotkeyStatement stmt = parseShowHotkey();
            if (stmt == null) {
                throw new ParserException(
                    "syntax error, expected 'from [table] partition([partition])'");
            }
        ...
        if (lexer.identifierEquals("STORAGE") && isEnabled(SQLParserFeature.DrdsMisc)) {
            lexer.nextToken();

            boolean showReplicas = false;
            if (lexer.identifierEquals("REPLICAS")) {
                lexer.nextToken();
                showReplicas = true;
            }

            DrdsShowStorage stmt = new DrdsShowStorage();
            stmt.setFull(full);
            stmt.setShowReplicas(showReplicas);
        ...
        if (lexer.identifierEquals("ChangeSet")) {
            lexer.nextToken();
            DrdsShowChangeSet stmt = new DrdsShowChangeSet();
            stmt.setFull(full);
            if (lexer.token() == Token.WHERE) {
                lexer.nextToken();
                SQLExpr where = exprParser.expr();
    ...
    public MySqlChangeMasterStatement parseChangeMaster() {
        ...
        if (lexer.token() == Token.TO) {
            ...
            lexer.nextToken();
            if (lexer.identifierEquals("DDL_LOAD")) {
                lexer.nextToken();
                MySqlChangeMasterStatement stmt = new MySqlChangeMasterStatement();
                stmt.setDdlLoad(true);
                return stmt;
            } else {
                throw new ParserException("syntax error, expect ddl_load after for ...");
            }
        }
        return null;
    }
    ...
    public MySqlStartMasterStatement parseStartMaster() {
        lexer.nextToken();
        MySqlStartMasterStatement stmt = new MySqlStartMasterStatement();
        if (lexer.token() == WITH) {
            lexer.nextToken();
            stmt.setWith(this.exprParser.expr());
        }
    ...
    public MySqlStopMasterStatement parseStopMaster() {
        lexer.nextToken();
        MySqlStopMasterStatement stmt = new MySqlStopMasterStatement();
        if (lexer.token() == WITH) {
            lexer.nextToken();
            stmt.setWith(this.exprParser.expr());
        }
    ...
    public MySqlRestartMasterStatement parseRestartMaster() {
        lexer.nextToken();
        MySqlRestartMasterStatement stmt = new MySqlRestartMasterStatement();
        if (lexer.token() == WITH) {
            lexer.nextToken();
            stmt.setWith(this.exprParser.expr());
        }
    ...
    public MySqlRebalanceMasterStatement parseRebalanceMaster() {
        lexer.nextToken();
        MySqlRebalanceMasterStatement stmt = new MySqlRebalanceMasterStatement();
        if (lexer.token() == WITH) {
            lexer.nextToken();
            stmt.setWith(this.exprParser.expr());
        }
    ...
    public SQLStatement parseReplica() {
        acceptIdentifier("REPLICA");
        if (lexer.identifierEquals("HASHCHECK")) {
            return parseReplicaHashcheck();
        } else {
            throw new ParserException("parse replica error");
        }
    }

    /**
     * replica hashcheck * from dbname.tbname { where condition }
     * condition: sub_condition [ and sub_condition ]
     * sub_condition: primary key comparator value | ( primary key1, priamry key2...) comparator ( v1, v2...)
     * comparator: > | <=
     */
    public SQLReplicaHashCheckStatement parseReplicaHashcheck() {
        SQLReplicaHashCheckStatement stmt = new SQLReplicaHashCheckStatement();
        acceptIdentifier("HASHCHECK");
        accept(Token.STAR);
        accept(FROM);
        SQLName name = this.exprParser.name();
        if (name instanceof SQLPropertyExpr) {
            ...
            setErrorEndPos(lexer.pos());
            throw new ParserException(
                "syntax error, no db or table name, " + lexer
                    .info());
        }
        ...
        if (lexer.token() == WHERE) {
            ...
            SQLExpr where = exprParser.expr();
            stmt.setWhere(where);
            parseReplicaHashCheckWhere(stmt, where);
        }

        return stmt;
    }

    private void parseReplicaHashCheckWhere(SQLReplicaHashCheckStatement stmt, SQLExpr where) {
        if (!(where instanceof SQLBinaryOpExpr)) {
            setErrorEndPos(lexer.pos());
            throw new ParserException(
                "syntax error, where must be binary operator, " + lexer
                    .info());
        }

        SQLBinaryOpExpr whereExpr = (SQLBinaryOpExpr) where;
        SQLBinaryOperator operator = whereExpr.getOperator();
        if (operator == SQLBinaryOperator.BooleanAnd) {
            parseReplicaHashCheckWhere(stmt, whereExpr.getLeft());
            parseReplicaHashCheckWhere(stmt, whereExpr.getRight());
            return;
        }

        // 为了parse简单，只支持 pk1 <= v1 或者 pk1 > v1
        SQLExpr primaryKeysExpr = whereExpr.getLeft();
        SQLExpr boundValuesExpr = whereExpr.getRight();
        List<Object> boundValues = new ArrayList<>();
        if (primaryKeysExpr instanceof SQLIdentifierExpr) {
            if (!(boundValuesExpr instanceof SQLValuableExpr)) {
                setErrorEndPos(lexer.pos());
                throw new ParserException(
                    "syntax error, unexpected where, " + lexer
                        .info());
            }
            boundValues.add(((SQLValuableExpr) boundValuesExpr).getValue());
        } else if (primaryKeysExpr instanceof SQLListExpr) {
            if (!(boundValuesExpr instanceof SQLListExpr)) {
                setErrorEndPos(lexer.pos());
                throw new ParserException(
                    "syntax error, unexpected where, " + lexer
                        .info());
            ...
            for (SQLExpr value : ((SQLListExpr) boundValuesExpr).getItems()) {
                ...
                boundValues.add(((SQLValuableExpr) value).getValue());
            }
        } else {
            setErrorEndPos(lexer.pos());
            throw new ParserException(
        ...
        if (operator == SQLBinaryOperator.LessThanOrEqual) {
            stmt.setUpperBounds(boundValues);
        } else if (operator == SQLBinaryOperator.GreaterThan) {
            stmt.setLowerBounds(boundValues);
        } else {
            setErrorEndPos(lexer.pos());
            throw new ParserException(
                "syntax error, where operator muse be <= or >, " + lexer
                    .info());
        }
    }

    public MySqlResetMasterStatement parseResetMaster() {
        lexer.nextToken();
        MySqlResetMasterStatement stmt = new MySqlResetMasterStatement();
        if (lexer.token() == WITH) {
            lexer.nextToken();
            stmt.setWith(this.exprParser.expr());
        }
    ...
    public MySqlStopSlaveStatement parseStopSlave() {
        lexer.nextToken();
    ...
    public SQLStatement parseTerminate() {
        acceptIdentifier("TERMINATE");

        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("REBALANCE")) {
            // TERMINATE REBALANCE { ALL | <job_id> [, <job_id>] ... }
            lexer.nextToken();
            DrdsTerminateRebalanceJob stmt = new DrdsTerminateRebalanceJob();
            parseGenericDdlCommand(stmt);
            return stmt;
        }
    ...
    public SQLStatement parseResume() {
        ...
        if (isEnabled(SQLParserFeature.DRDSAsyncDDL) && lexer.identifierEquals("REBALANCE")) {
            // RESUME REBALANCE { ALL | <job_id> [, <job_id>] ... }
            lexer.nextToken();
    ...
    public SQLStatement parseSet() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.PASSWORD)) {
            ...
            return stmt;
        } else if (lexer.identifierEquals("CDC")) {
            lexer.nextToken();
            acceptIdentifier(GLOBAL);
            MySqlSetCdcGlobalStatement stmt = new MySqlSetCdcGlobalStatement();
    ...
    public SQLStatement parseAlter() {
        Lexer.SavePoint mark = lexer.mark();
//        accept(Token.ALTER);
        if (lexer.identifierEquals("MODIFY")) {
            lexer.nextToken();
        } else {
            accept(Token.ALTER);
        }
        ...
        if (lexer.identifierEquals(FnvHash.Constants.EXTERNAL)) {
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.CATALOG)) {
                return parseAlterExternalCatalog();
            }
            throw new ParserException("TODO " + lexer.info());
        }

        if (lexer.identifierEquals("SECRET")) {
            return parseAlterSecret();
        }
...
//            accept(Token.FULLTEXT);
    ...
    protected SQLStatement parseAlterTableGroup() {
        ...
        if (lexer.token() == Token.BY) {
            ...
            /**
             * e.g alter tablegroup
             *  by table xxx_db.xxx_tbl
             *
             * e.g alter tablegroup
             *  by index idx_xxx on table xxx_db.xxx_tbl
             */
            accept(Token.BY);
            if (lexer.token() == Token.TABLE) {
                accept(Token.TABLE);
        ...
        if (alterIndexTg) {
            ...
            tblNameOfIndex = this.exprParser.name();
        }

        SQLAlterTableGroupStatement stmt = new SQLAlterTableGroupStatement(getDbType());
        stmt.setName(name);
        stmt.setAlterByTable(alterByTable);
        stmt.setAlterIndexTg(alterIndexTg);
        stmt.setTblNameOfIndex(tblNameOfIndex);
        parseAlterTablePartitionItem(stmt);
        parseAsyncOption(stmt);
        parseDryRunOption(stmt);
        return stmt;
    }

    private void parseAlterTablePartitionItem(SQLAlterTableGroupStatement stmt) {
        if (lexer.identifierEquals("SPLIT")) {
            acceptIdentifier("SPLIT");
            if (lexer.token() == Token.INTO) {
                DrdsSplitHotKey item = parseSplitHotKey(false);
                stmt.setItem(item);
            } else {
            ...
            SQLAlterTableReorgPartition item = parseReorgPartition();
            stmt.setItem(item);
        } else if (lexer.identifierEquals("ADD")) {
            lexer.nextToken();
            if (lexer.identifierEquals("TABLES")) {
                ...
                SQLAlterTableAddPartition addSubPartition = parseAddSubPartition(null);
                stmt.setItem(addSubPartition);
            } else {
                SQLAlterTableAddPartition addPartition = parseAddPartition();
                stmt.setItem(addPartition);
            }
        } else if (lexer.token() == Token.DROP) {
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                SQLAlterTableDropPartition dropSubPartition = parseDropSubPartition(null);
                stmt.setItem(dropSubPartition);
            } else if (lexer.token() == Token.PARTITION) {
            ...
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                // MODIFY SUBPARTITION sub_part_name ADD|DROP VALUES()
                List<SQLAlterTableModifyPartitionValues> modifySubPartitionItems = parseModifySubPartitionValues(null);
                stmt.setItem(modifySubPartitionItems.get(0));
            } else {
                // MODIFY PARTITION part_name ADD|DROP VALUES() | ADD|DROP|MODIFY SUBPARTITION ...
                parseModifyPartition(stmt);
            }
        } else if (lexer.token() == Token.TRUNCATE) {
            // TRUNCATE PARTITION {partition_names | ALL}
            SQLAlterTableTruncatePartition item = parseTruncatePartition();
            stmt.setItem(item);
    ...
    protected SQLAlterTableTruncatePartition parseTruncatePartition() {
...
//            }
    ...
    protected SQLAlterTableAddPartition parseAddSubPartition(SQLName partitionName) {
        lexer.nextToken();
        ...
        addPartition.addPartition(partition);
        addPartition.setSubPartition(true);

        accept(Token.LPAREN);

        this.getExprParser().parseSubPartitions(partition);
        ...
        if (lexer.identifierEquals("ALGORITHM")) {
            ...
            accept(Token.EQ);
            String algorithm = lexer.stringVal();
            addPartition.setAlgorithm(algorithm);
            lexer.nextToken();
        }
    ...
    protected SQLAlterTableDropPartition parseDropSubPartition(SQLName partitionName) {
        SQLAlterTableDropPartition dropPartition = parseAlterTableDropPartition(false);
        dropPartition.setSubPartition(true);
        dropPartition.setPartitionName(partitionName);
        return dropPartition;
    }

    // parse MODIFY PARTITION [(] part_name ADD|DROP VALUES(),part_name ADD|DROP VALUES(), ... [)]
    protected List<SQLAlterTableModifyPartitionValues> parseModifyPartitionValues() {
        // skip lexer 'PARTITION'
        lexer.nextToken();
        boolean paren = false;
        if (lexer.token() == Token.LPAREN) {
            paren = true;
        ...
        List<SQLAlterTableModifyPartitionValues> alterItems = new ArrayList<>();
        for (; ; ) {
            SQLAlterTableModifyPartitionValues item = this.getExprParser().parseModifyPartitionValues(null);
            if (lexer.identifierEquals("ALGORITHM")) {
                lexer.nextToken();
                accept(Token.EQ);
                String algorithm = lexer.stringVal();
    ...
    protected DrdsAlterTableModifyTtlOptions parseModifyTtlOptions() {
        ...
        // skip lexer 'TTL'
        lexer.nextToken();
        // make sure next lexer must be set
        if (lexer.token() == Token.SET) {
            lexer.nextToken();
        }
        ...
        for (; ; ) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_ENABLE)) {
                ...
                modifyTtlOptions.setTtlEnable(ttlEnableExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_EXPR)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlExpr = MySqlCreateTableParser.parseTimeToLiveExpr(exprParser, lexer);
                modifyTtlOptions.setTtlExpr(ttlExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_JOB)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlJobExpr = MySqlCreateTableParser.parseTimeToLiveJobExpr(exprParser, lexer);
                modifyTtlOptions.setTtlJob(ttlJobExpr);
                findOptions = true;
                continue;
            }

            if (lexer.identifierEquals(FnvHash.Constants.TTL_COL_ENCODER)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlColEncoderExpr = exprParser.expr();
                modifyTtlOptions.setTtlColEncoder(ttlColEncoderExpr);
                findOptions = true;
                continue;
            }

            if (lexer.identifierEquals(FnvHash.Constants.TTL_COL_DECODER)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlColDecoderExpr = exprParser.expr();
                modifyTtlOptions.setTtlColDecoder(ttlColDecoderExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_CLEANUP)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlCleanup = exprParser.charExpr();
                modifyTtlOptions.setTtlCleanup(ttlCleanup);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_PART_INTERVAL)) {
                ...
                modifyTtlOptions.setTtlPartInterval(ttlPartInterval);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.TTL_FILTER)) {
                ...
                SQLExpr ttlFilterExpr = null;
                if (lexer.token() == Token.RPAREN) {
                    exprParser.accept(RPAREN);
                    ttlFilterExpr = new SQLBooleanExpr(true);
                } else {
                    ttlFilterExpr = exprParser.expr();
                    exprParser.accept(RPAREN);
                }
                modifyTtlOptions.setTtlFilter(ttlFilterExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TYPE)) {
                ...
                modifyTtlOptions.setArchiveKind(archiveTypeExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_SCHEMA)) {
                ...
                modifyTtlOptions.setArchiveTableSchema(archiveTableSchemaExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_NAME)) {
                ...
                modifyTtlOptions.setArchiveTableName(archiveTableNameExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_PRE_ALLOCATE)) {
                ...
                modifyTtlOptions.setArcPreAllocate(arcPreAllocateExpr);
                findOptions = true;
                continue;
            }
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_POST_ALLOCATE)) {
                ...
                modifyTtlOptions.setArcPostAllocate(arcPostAllocateExpr);
                findOptions = true;
                continue;
            }

            if (lexer.identifierEquals(FnvHash.Constants.TTL_REF_COL_LIST)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlRefColListExpr = exprParser.expr();
                modifyTtlOptions.setTtlRefColList(ttlRefColListExpr);
                findOptions = true;
                continue;
            }

            if (lexer.identifierEquals(FnvHash.Constants.TTL_HYBRID)) {
                lexer.nextToken();
                exprParser.accept(Token.EQ);
                SQLExpr ttlHybridExpr = exprParser.expr();
                modifyTtlOptions.setTtlHybrid(ttlHybridExpr);
                findOptions = true;
                continue;
            }
        ...
        return modifyTtlOptions;
    }

    // parse MODIFY SUBPARTITION [(] sub_part_name ADD|DROP VALUES(),sub_part_name ADD|DROP VALUES(), ... [)]
    protected List<SQLAlterTableModifyPartitionValues> parseModifySubPartitionValues(SQLName partName) {
        // skip lexer 'SUBPARTITION'
        lexer.nextToken();

        SQLPartition partition = null;
        if (partName != null) {
        ...
        for (; ; ) {
            ...
            SQLAlterTableModifyPartitionValues sqlAlterTableModifySubPartitionValues =
                this.getExprParser().parseModifySubPartitionValues(partition);
            alterItems.add(sqlAlterTableModifySubPartitionValues);
    ...
    protected void parseModifyPartition(SQLAlterStatement alterStatement) {
        SavePoint sp = this.getExprParser().getLexer().mark();
        accept(Token.PARTITION);
        boolean isAddSubPartition = false, isDropSubPartition = false, isModifySubPartition = false;
        SQLName name = this.getExprParser().name();
        boolean addDropPartValOnly = false;
        boolean isSplitByHotValue = false;
        boolean isExtractByHotValue = false;
        if (lexer.identifierEquals(FnvHash.Constants.ADD)) {
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                isAddSubPartition = true;
            } else {
                addDropPartValOnly = true;
            }
        } else if (lexer.token() == Token.DROP) {
            lexer.nextToken();
            if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                isDropSubPartition = true;
            } else {
                addDropPartValOnly = true;
            }
        } else if (lexer.identifierEquals("SPLIT")) {
            isSplitByHotValue = true;
        } else if (lexer.identifierEquals("EXTRACT")) {
            isExtractByHotValue = true;
        } else {
            acceptIdentifier("MODIFY");
            if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                isModifySubPartition = true;
            }
        }
        if (addDropPartValOnly) {
            /**
             * Put the lexer cursor back to the keyword "partition" of "->partition pxxx add values (...)",
             * so the method of parseModifyPartitionValues will parse the following format expr
             * "partition [(] p1 add values (...), p2 add values (...),...  [)]" successfully
             */
            this.getExprParser().getLexer().reset(sp);
        }

        boolean isAlterTableGroup = alterStatement instanceof SQLAlterTableGroupStatement;

        if (isAddSubPartition) {
            SQLAlterTableAddPartition addPartition = parseAddSubPartition(name);
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(addPartition);
            } else {
                ((SQLAlterTableStatement) alterStatement).addItem(addPartition);
            }
        } else if (isDropSubPartition) {
            SQLAlterTableDropPartition dropPartition = parseDropSubPartition(name);
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(dropPartition);
            } else {
                ((SQLAlterTableStatement) alterStatement).addItem(dropPartition);
            }
        } else if (isModifySubPartition) {
            List<SQLAlterTableModifyPartitionValues> items = parseModifySubPartitionValues(name);
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(items.get(0));
            } else {
                items.stream().forEach(i -> ((SQLAlterTableStatement) alterStatement).addItem(i));
            }
        } else if (isSplitByHotValue) {
            acceptIdentifier("SPLIT");
            DrdsSplitHotKey splitHotKey = parseSplitHotKey(true);
            splitHotKey.setModifyPartitionName(name);
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(splitHotKey);
            } else {
                ((SQLAlterTableStatement) alterStatement).addItem(splitHotKey);
            }
        } else if (isExtractByHotValue) {
            DrdsExtractHotKey extractHotkey = parseExtractHotKey();
            if (!extractHotkey.isExtractSubPartition()) {
                throw new ParserException("syntax error, expect subpartition, actual partition");
            }
            SQLPartition partition = new SQLPartition();
            partition.setName(name);
            extractHotkey.getPartitions().add(partition);
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(extractHotkey);
            } else {
                ((SQLAlterTableStatement) alterStatement).addItem(extractHotkey);
            }
        } else {
            List<SQLAlterTableModifyPartitionValues> items = parseModifyPartitionValues();
            if (isAlterTableGroup) {
                ((SQLAlterTableGroupStatement) alterStatement).setItem(items.get(0));
            } else {
                items.stream().forEach(i -> ((SQLAlterTableStatement) alterStatement).addItem(i));
    ...
    protected DrdsMergePartition parseMergePartition() {
        ...
        if (identifierEquals(PARTITIONS)) {
            ...
            acceptIdentifier("SUBPARTITIONS");
            item.setSubPartitionsMerge(true);
        }
        while (true) {
            SQLName partitionName = this.exprParser.name();
    ...
    protected DrdsArchivePartition parseArchivePartition() {
        DrdsArchivePartition item = new DrdsArchivePartition();
        lexer.nextToken();

        if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITIONS)) {
            item.setSubPartitionsArchive(true);
            lexer.nextToken();
        } else {
            acceptIdentifier("PARTITIONS");
        }
    ...
    protected DrdsMovePartition parseMovePartition() {
        ...
        if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITIONS)) {
            item.setSubPartitionsMoved(true);
            lexer.nextToken();
        } else {
            acceptIdentifier("PARTITIONS");
        }
        ...
        Map<SQLName, SQLExpr> localities = new HashMap<>();
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (lexer.identifierEquals("LOCALITY")) {
                        ...
                        localities.put(partitionName, locality);
                    }
                    if (lexer.token() != Token.COMMA) {
                        break;
                    }
                ...
                instPartitions.computeIfAbsent(storageId, k -> new ArrayList<>()).addAll(partNames);
                if (lexer.token() != Token.COMMA) {
                    break;
                }
                accept(Token.COMMA);
        ...
        item.setLocalities(localities);

        return item;
    }

    protected SQLAlterTableReorgPartition parseReorgPartition() {
        SQLAlterTableReorgPartition item = new SQLAlterTableReorgPartition();
        ...
        for (; ; ) {
            SQLObject partition;
            if (item.isSubPartition()) {
                acceptIdentifier("SUBPARTITION");
                partition = this.getExprParser().parseSubPartition();
            } else {
    ...
    protected DrdsExtractHotKey parseExtractHotKey() {
        ...
        if (lexer.token() == PARTITION) {
            accept(Token.PARTITION);
            item.setExtractSubPartition(false);
        } else {
            acceptIdentifier("SUBPARTITION");
            item.setExtractSubPartition(true);
        }
        if (lexer.token() != Token.BY) {
            SQLName partitionName = this.exprParser.name();
        ...
        if (lexer.identifierEquals("LOCALITY")) {
            lexer.nextToken();
            accept(EQ);
            SQLExpr localityItem = this.exprParser.expr();
            item.setLocality(localityItem);
        }
        accept(Token.BY);
    ...
    protected DrdsRenamePartition parseRenamePartition() {
        ...
        if (lexer.token() == PARTITION) {
            ...
            acceptIdentifier("SUBPARTITION");
            item.setSubPartitionsRename(true);
        }
        do {
            if (!firstPart) {
                accept(Token.COMMA);
    ...
    protected DrdsSplitPartition parseSplitPartition() {
        ...
        if (lexer.token() == PARTITION) {
            ...
            acceptIdentifier("SUBPARTITION");
            item.setSubPartitionsSplit(true);
        }
        SQLName partitionName = this.exprParser.name();
        item.setSplitPartitionName(partitionName);
        // parse comma-separated additional partition names for multi-partition split
        while (lexer.token() == Token.COMMA) {
            lexer.nextToken();
            SQLName additionalPartName = this.exprParser.name();
            item.addSplitPartitionName(additionalPartName);
        }
        boolean isMultiPartitionSplit = item.getSplitPartitionNames().size() > 1;
        if (isMultiPartitionSplit && lexer.identifierEquals("AT")) {
            throw new ParserException("AT clause is not supported with multi-partition split");
        }
        if (lexer.identifierEquals("AT")) {
            ...
            if (lexer.token() == Token.LPAREN) {
                if (isMultiPartitionSplit) {
                    throw new ParserException(
                        "Explicit partition definitions are not supported with multi-partition split. "
                            + "Use INTO PARTITIONS N instead.");
                }
                accept(Token.LPAREN);
                for (; ; ) {
                    SQLObject partition;
                    if (item.isSubPartitionsSplit()) {
                        acceptIdentifier("SUBPARTITION");
                        partition = this.getExprParser().parseSubPartition();
                    } else {
                        partition = this.getExprParser().parsePartition();
                ...
                if (!item.isSubPartitionsSplit()) {
                    if (!lexer.identifierEquals("partitions")) {
                        SQLName newPartitionPrefix = this.exprParser.name();
                        item.setNewPartitionNamePrefix(newPartitionPrefix);
                    }
                    acceptIdentifier("PARTITIONS");
                } else {
                ...
                SQLIntegerExpr newPartitionNum = this.exprParser.integerExpr();
                item.setNewPartitionNum(newPartitionNum);
            }
        }
        return item;
    }

    protected DrdsSplitHotKey parseSplitHotKey(boolean onlyAllowSubpartition) {
        DrdsSplitHotKey item = new DrdsSplitHotKey();
        accept(Token.INTO);
        if (!lexer.identifierEquals("PARTITIONS") && !lexer.identifierEquals("SUBPARTITIONS")) {
            SQLName partitionName = this.exprParser.name();
            item.setHotKeyPartitionName(partitionName);
        }
        if (lexer.identifierEquals("LOCALITY")) {
        ...
        if (!onlyAllowSubpartition && lexer.identifierEquals("PARTITIONS")) {
            acceptIdentifier("PARTITIONS");
        } else {
            acceptIdentifier("SUBPARTITIONS");
            item.setSubPartitionsSplit(true);
    ...
    protected SQLStatement parseAlterSystem() {
        ...
        if (lexer.token() == Token.SET) {
            ...
            SQLAlterSystemRefreshStorageStatement stmt = new SQLAlterSystemRefreshStorageStatement();

            SQLExpr dnId = this.exprParser.expr();
            stmt.setTargetStorage(dnId);
            ...
            for (; ; ) {
                ...
                accept(Token.EQ);
                SQLExpr value = this.exprParser.expr();
                stmt.getAssignItems().add(new SQLAssignItem(key, value));
                if (lexer.token() == Token.EOF) {
                    break;
                }
                if (lexer.token() == (Token.SEMI)) {
            ...
            lexer.nextToken();
            acceptIdentifier("STORAGE");
            SQLAlterSystemReloadStorageStatement stmt = new SQLAlterSystemReloadStorageStatement();
            if (lexer.token() == (Token.SEMI)) {
                lexer.nextToken();
            }
            if (lexer.token() != Token.EOF) {
            ...
            return stmt;
        } else if (lexer.identifierEquals("LEADER")) {
            lexer.nextToken();
            SQLAlterSystemLeaderStatement stmt = new SQLAlterSystemLeaderStatement();
            ...
            if (lexer.token() != Token.EOF) {
                for (; ; ) {
                    SQLExpr nodeId = this.exprParser.expr();
                    stmt.setNodeId(nodeId);
                    if (lexer.token() == Token.EOF) {
                        break;
                    }
                    if (lexer.token() == (Token.SEMI)) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.ADD)) {
                ...
                case IDENTIFIER:
                    if (lexer.token() == Token.FULLTEXT
                        ...
                        if (lexer.token() == Token.HINT) {
                            String execComment = lexer.stringVal();
                            item.getIndexDefinition().getOptions().setExecComment(execComment);
                            lexer.nextToken();
                        }
                        stmt.addItem(item);
                ...
                case CHECK:
                    ...
                    if (lexer.token() == Token.FOREIGN) {
                        ...
                        if (lexer.token() == Token.NOT) {
                            ...
                            if (lexer.identifierEquals(FnvHash.Constants.ENFORCED)) {
                                acceptIdentifier("ENFORCED");
                                check.setWithEnforced(true);
                            }
...
//                    // Why do we have [FIRST column] syntax? It is not supported by MySQL.
                ...
                if (lexer.token() == Token.PARTITION) {
                    ...
                    // MODIFY SUBPARTITION [(]subpart_name ADD|DROP VALUES(),subpart_name ADD|DROP VALUES()[)]
                    List<SQLAlterTableModifyPartitionValues> items = parseModifySubPartitionValues(null);
                    for (int i = 0; i < items.size(); i++) {
                        stmt.addItem(items.get(i));
                    }

                    return true;
                } else if (lexer.identifierEquals("TTL")) {
                    /**
                     * <pre>
                     * MODIFY TTL
                     * SET
                     ...
                     * </pre>
                     */
                    DrdsAlterTableModifyTtlOptions modifyTtlOptions = parseModifyTtlOptions();
                    stmt.addItem(modifyTtlOptions);
                    return true;
                } else {
...
//                    throw new ParserException("we don't support rename column in this version, you can use change column instead or upgrade to later version!");
                    return true;
                }
                case TO:
                case AS:
                ...
                stmt.addItem(item);
                return true;
            } else if (lexer.identifierEquals("EXPAND")) {
                lexer.nextToken();
                boolean subPartitions = false;
                if (lexer.identifierEquals("SUBPARTITIONS")) {
                    lexer.nextToken();
                    subPartitions = true;
                } else {
                ...
                accept(Token.TO);
                SQLIntegerExpr targetCount = this.exprParser.integerExpr();
                DrdsExpandPartitions item = new DrdsExpandPartitions();
                item.setSubPartitions(subPartitions);
                item.setTargetCount(targetCount);
                stmt.addItem(item);
                return true;
            } else if (lexer.identifierEquals("CANCEL")) {
                lexer.nextToken();
                acceptIdentifier("EXPAND");
                DrdsAlterTableCancelExpand item = new DrdsAlterTableCancelExpand();
                stmt.addItem(item);
                return true;
                ...
                stmt.addItem(item);
                return true;
            } else if (lexer.identifierEquals("CLEANUP")) {
                lexer.nextToken();
                acceptIdentifier("EXPIRED");
                acceptIdentifier("DATA");
                DrdsAlterTableCleanupExpiredData item = new DrdsAlterTableCleanupExpiredData();

                if (lexer.token() == WITH) {
                    accept(WITH);
                    while (true) {
                        boolean findNewOptions = false;

                        if (lexer.identifierEquals(FnvHash.Constants.TTL_CLEANUP)) {
                            lexer.nextToken();
                            exprParser.accept(Token.EQ);
                            SQLExpr ttlCleanup = exprParser.expr();
                            item.setTtlCleanup(ttlCleanup);
                            findNewOptions = true;
                            continue;
                        }

                        if (lexer.identifierEquals(FnvHash.Constants.TTL_CLEANUP_BOUND)) {
                            lexer.nextToken();
                            exprParser.accept(Token.EQ);
                            SQLExpr ttlCleanupBound = exprParser.expr();
                            item.setTtlCleanupBound(ttlCleanupBound);
                            findNewOptions = true;
                            continue;
                        }

                        if (lexer.identifierEquals(FnvHash.Constants.TTL_CLEANUP_POLICY)) {
                            lexer.nextToken();
                            exprParser.accept(Token.EQ);
                            SQLExpr ttlCleanupPolicy = exprParser.expr();
                            item.setTtlCleanupPolicy(ttlCleanupPolicy);
                            findNewOptions = true;
                            continue;
                        }
                        ...
                        if (lexer.identifierEquals(FnvHash.Constants.TTL_PART_INTERVAL)) {
                            ...
                            exprParser.accept(Token.EQ);
                            SQLExpr ttlPartInterval = exprParser.expr();
                            item.setTtlPartInterval(ttlPartInterval);
                            findNewOptions = true;
                            continue;
                        }
                        ...
                        if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_PRE_ALLOCATE)) {
                            lexer.nextToken();
                            exprParser.accept(Token.EQ);
                            SQLExpr archiveTablePreAllocateCnt = exprParser.expr();
                            item.setArchiveTablePreAllocateCount(archiveTablePreAllocateCnt);
                            findNewOptions = true;
                            continue;
                        }
                        ...
                        if (lexer.identifierEquals(FnvHash.Constants.ARCHIVE_TABLE_POST_ALLOCATE)) {
                            lexer.nextToken();
                            exprParser.accept(Token.EQ);
                            SQLExpr archiveTablePostAllocateCnt = exprParser.expr();
                            item.setArchiveTablePostAllocateCount(archiveTablePostAllocateCnt);
                            findNewOptions = true;
                            continue;
                        }
                ...
                for (; ; ) {
                    SQLName partition = this.exprParser.name();
                    item.addSrcPartition(partition);
                    if (lexer.token() == Token.COMMA) {
                        lexer.nextToken();
                        continue;
                    }
                ...
                if (!subPartition) {
                    if (lexer.token() == Token.PARTITION) {
                        ...
                        for (; ; ) {
                            SQLName partition = this.exprParser.name();
                            item.addTargetPartition(partition);
                            if (lexer.token() == Token.COMMA) {
                                lexer.nextToken();
                                continue;
                            }
                ...
                item.setSubPartition(subPartition);
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    acceptIdentifier("VALIDATION");
                ...
                lexer.nextToken();
                if (lexer.identifierEquals("CLEANUP")) {
                    lexer.nextToken();
                    accept(Token.WHERE);
                    DrdsAlterTableRebuildCleanup item = new DrdsAlterTableRebuildCleanup();
                    item.setCleanupPredicate(this.exprParser.expr());
                    if (lexer.identifierEquals("DRY")) {
                        lexer.nextToken();
                        acceptIdentifier("RUN");
                        item.setDryRun(true);
                    }
                    stmt.addItem(item);
                } else if (lexer.identifierEquals(FnvHash.Constants.COLUMNAR)
                    || lexer.identifierEquals(FnvHash.Constants.CLUSTERED)) {
                    // REBUILD COLUMNAR INDEX
                    SQLAlterTableRebuildIndex item = new SQLAlterTableRebuildIndex();
                    this.exprParser.parseIndex(item.getIndexDefinition());
                    if (lexer.token() == Token.HINT) {
                        String execComment = lexer.stringVal();
                        item.getIndexDefinition().getOptions().setExecComment(execComment);
                ...
                if (lexer.identifierEquals(FnvHash.Constants.LOCAL)) {
                     ...
                     */
                    lexer.nextToken();
                    DrdsAlterTableRemoveTtlOptions removeTtlOptions = new DrdsAlterTableRemoveTtlOptions();
                    stmt.addItem(removeTtlOptions);
                    return true;
                } else if (lexer.identifierEquals("AUTO")) {
                    acceptIdentifier("AUTO");
                    accept(PARTITION);
                    stmt.setRemoveAutoPartition(true);
                    return true;
                }
                acceptIdentifier("PARTITIONING");
            ...
            //
                ...
                if (lexer.identifierEquals(FnvHash.Constants.DBPARTITIONS)) {
                    ...
                    item.setDbPartitions(dbPartitoins);
                }
                ...
                item.setCount(exprParser.integerExpr());
                stmt.addItem(item);
                return true;
            } else if (lexer.identifierEquals(FnvHash.Constants.SUBPARTITION)) {
                // Caution: Not in MySql documents.
                ...
                stmt.addItem(item);
                return true;
            } else if (lexer.identifierEquals("SECONDARY_ENGINE_ATTRIBUTE")) {
                SQLAlterTableSecondaryEngineAttribute item = new SQLAlterTableSecondaryEngineAttribute();
                lexer.nextToken();
                if (lexer.token() == Token.EQ) {
                    accept(Token.EQ);
                }
                if (lexer.token() == Token.NULL) {
                    ...
                    if (expr instanceof SQLCharExpr) {
                        item.setVal((SQLCharExpr) expr);
                    } else {
                        throw new ParserException("SECONDARY_ENGINE_ATTRIBUTE expects a string value or NULL");
                    }
                }
                stmt.addItem(item);
                return true;
        ...
        case ALTER: {
            ...
            if (lexer.token() == Token.INDEX) {
                ...
                //ALTER TABLE xx ALTER INDEX yy INVISIBLE;
                if (lexer.identifierEquals("INVISIBLE") || lexer.identifierEquals("VISIBLE")) {
                    if (lexer.identifierEquals("INVISIBLE")) {
                        alterIndex.setIndexVisibility("INVISIBLE");
                    } else {
                        alterIndex.setIndexVisibility("VISIBLE");
                    }
                    lexer.nextToken();
                ...
                if (lexer.token() == Token.SET) {
                    ...
                    accept(Token.DEFAULT);
                    if (lexer.token() == LPAREN) {
                        alterColumn.setDefaultExprHasLp(true);
                    }
        ...
        case DISABLE:
            ...
            if (lexer.token() == Token.CONSTRAINT) {
                ...
                stmt.addItem(item);
            } else if (lexer.identifierEquals("FULL_SCAN")) {
                // disable full table scan.
                lexer.nextToken();
                SQLAlterTableToggleFullScan item = new SQLAlterTableToggleFullScan();
        ...
        case OPTIMIZE: {
            ...
            SQLAlterTableOptimizePartition item = parseOptimizePartition();
...
//            }
        ...
        case SET:
            ...
            if (lexer.identifierEquals(FnvHash.Constants.RULE)) {
                ...
                setOption.addOption(item);
                setOption.setAlterTableGroup(true);
                if (lexer.identifierEquals("IMPLICIT")) {
                    setOption.setImplicit(true);
                    lexer.nextToken();
                }
                if (lexer.identifierEquals("FORCE")) {
        ...
        case PARTITION: {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.LIFECYCLE)) {
                ...
                item.setLifecycle(exprParser.integerExpr());
                stmt.addItem(item);
                return true;
            } else {
                lexer.reset(mark);
    ...
    protected SQLStatement parseAlterInstance() {
        ...
        if (lexer.identifierEquals("ROTATE")) {
            ...
            accept(Token.EQ);
            SQLExpr value = this.exprParser.expr();
            final MySQLInstanceReadonlyItem readonlyItem = new MySQLInstanceReadonlyItem();
            readonlyItem.getOptions().add(new SQLAssignItem(key, value));
            stmt.setItem(readonlyItem);
        }
        return stmt;
    }
    ...
    protected SQLStatement parseAlterTable(boolean ignore, boolean online, boolean offline) {
        ...
        if (lexer.identifierEquals("ARCHIVE")) {
            DrdsArchivePartition archivePartition = parseArchivePartition();
            stmt.setDrdsArchivePartition(archivePartition);
        }
        ...
        if (Token.PARTITION == lexer.token()) {
            ...
            if (1 == stmt.getItems().size() && stmt.getItems().get(0) instanceof SQLAlterTableRename) {
                ...
                parseAsyncOption(renameStmt);
                parseDryRunOption(stmt);
                return renameStmt;
            }
        }

        if (stmt.getItems().size() >= 1 || (stmt.getItems().size() == 0 && stmt.getPartition() != null)
            || stmt.isRemovePatiting()) {
            SQLAlterTableItem sqlAlterTableItem = stmt.getItems().size() >= 1 ? stmt.getItems().get(0) : null;
            boolean partitionReorg =
                sqlAlterTableItem instanceof DrdsSplitPartition
                    || sqlAlterTableItem instanceof DrdsMergePartition
                    ...
                    || sqlAlterTableItem instanceof SQLAlterTableReorgPartition
                    || sqlAlterTableItem instanceof DrdsAlterTableSingle
                    || sqlAlterTableItem instanceof DrdsAlterTableBroadcast
                    || sqlAlterTableItem instanceof SQLAlterTablePartitionCount
                    || sqlAlterTableItem instanceof SQLAlterTableModifyPartitionValues
                    || sqlAlterTableItem instanceof SQLAlterTableDropPartition
                    || sqlAlterTableItem instanceof SQLAlterTableDropSubpartition
                    || sqlAlterTableItem instanceof MySqlAlterTableModifyColumn
                    || stmt.getPartition() != null
                    || stmt.isRemovePatiting();
            if (!partitionReorg && stmt.getItems().size() > 1) {
                partitionReorg = stmt.getItems().stream().anyMatch(o -> o instanceof MySqlAlterTableModifyColumn);
            }
            if (partitionReorg) {
                if (lexer.token() == Token.WITH) {
                    lexer.nextToken();
                    if (lexer.identifierEquals(FnvHash.Constants.TABLEGROUP)) {
                        ...
                        SQLName tableGroup = this.exprParser.name();
                        acceptIdentifier("IMPLICIT");
                        stmt.setTargetImplicitTableGroup(tableGroup);
                    }
                }
                while (lexer.token() == COMMA) {
                    lexer.nextToken();
                    ...
                    acceptIdentifier("TABLEGROUP");
                    accept(EQ);
                    SQLName tgName = this.exprParser.name();
                    acceptIdentifier("IMPLICIT");
                    stmt.addIndexTableGroupPair(indexName, tgName);
                }
            }
        }
        ...
        parseAsyncOption(stmt);
        parseDryRunOption(stmt);
        parsePerfModeOption(stmt);
        return stmt;
    }

    private void parseDryRunOption(SQLStatementImpl stmt) {
        if (lexer.identifierEquals("DRYRUN")) {
            lexer.nextToken();
            accept(EQ);
            if (lexer.token() == FALSE) {
                stmt.setDryrun(Boolean.FALSE);
            } else {
                stmt.setDryrun(Boolean.TRUE);
            }
            lexer.nextToken();
        }
    }

    private void parseAsyncOption(SQLStatementImpl stmt) {
        if (lexer.identifierEquals("ASYNC")) {
            lexer.nextToken();
            accept(EQ);
            if (lexer.token() == FALSE) {
                stmt.setAsync(Boolean.FALSE);
            } else {
                stmt.setAsync(Boolean.TRUE);
            }
            lexer.nextToken();
        }
    }

    private void parsePerfModeOption(SQLStatementImpl stmt) {
        if (lexer.identifierEquals("PERF_MODE")) {
            lexer.nextToken();
            accept(EQ);
            // 接受字符串值：'boost' 或 'default'
            if (lexer.token() == Token.LITERAL_CHARS || lexer.token() == Token.LITERAL_ALIAS) {
                String perfMode = lexer.stringVal();
                perfMode = org.apache.commons.lang.StringUtils.strip(perfMode, "'\"");

                if ("boost".equalsIgnoreCase(perfMode) || "default".equalsIgnoreCase(perfMode)) {
                    stmt.setPerfMode(perfMode.toLowerCase());
                } else {
                    throw new ParserException("PERF_MODE value must be 'boost' or 'default'");
                }
                lexer.nextToken();
            } else {
                throw new ParserException("PERF_MODE value must be a string");
            }
        }
    }
...
//        FnvHash.Constants
        ...
        boolean alterIndexPartition = false;
        if (lexer.token() == Token.ON) {
            accept(Token.ON);
            alterIndexPartition = true;
            if (lexer.token() == Token.TABLE) {
                accept(Token.TABLE);
            }
            stmt.setTable(this.exprParser.name());
        }
        if (alterIndexPartition) {

            Token opToken = lexer.token();
            String opName = opToken.name();
            if (lexer.identifierEquals(FnvHash.Constants.ADD)
                || lexer.identifierEquals(FnvHash.Constants.MODIFY)
                || lexer.identifierEquals(FnvHash.Constants.SPLIT)
                || lexer.identifierEquals(FnvHash.Constants.MERGE)
                || lexer.identifierEquals(FnvHash.Constants.MOVE)
                || lexer.identifierEquals(FnvHash.Constants.REORGANIZE)
                || lexer.identifierEquals(FnvHash.Constants.EXTRACT)
                || lexer.identifierEquals(FnvHash.Constants.RENAME)
                || lexer.identifierEquals(FnvHash.Constants.SET)
                || lexer.identifierEquals(FnvHash.Constants.DROP)
                || lexer.identifierEquals(FnvHash.Constants.TRUNCATE)
                || lexer.identifierEquals(FnvHash.Constants.OPTIMIZER)
                || (opName.equalsIgnoreCase("SPLIT")
                || opName.equalsIgnoreCase("MERGE") || opName.equalsIgnoreCase("MOVE")
                || opName.equalsIgnoreCase("ADD") || opName.equalsIgnoreCase("MODIFY")
                || opName.equalsIgnoreCase("REORGANIZE") || opName.equalsIgnoreCase("RENAME")
                || opName.equalsIgnoreCase("SET") || opName.equalsIgnoreCase("EXTRACT")
                || opName.equalsIgnoreCase("DROP") || opName.equalsIgnoreCase("TRUNCATE") || opName.equalsIgnoreCase(
                "OPTIMIZE")
            )
            ) {

                SQLAlterTableStatement alterTblStmt = new SQLAlterTableStatement(getDbType());
                alterTblStmt.setTableSource(stmt.getTable());
                alterTblStmt.setAlterIndexName(stmt.getName());
                alterTblStmt.setFromAlterIndexPartition(true);

                parseAlterSpecification(alterTblStmt);
                List<SQLAlterTableItem> allItems = alterTblStmt.getItems();
                for (int i = 0; i < allItems.size(); i++) {
                    SQLAlterTableItem item = allItems.get(i);
                    if (!(item instanceof SQLAlterTableGroupItem)) {
                        throw new ParserException(
                            "syntax error, 'alter index on table' only support the operations of alter (sub)partitions");
                    }
                    if ((item instanceof SQLAlterTableDropPartition)
                        || (item instanceof SQLAlterTableTruncatePartition)) {
                        throw new ParserException(
                            "'alter index on table' does not support drop/truncate (sub)partitions");
                    } else if (item instanceof SQLAlterTableModifyPartitionValues) {
                        boolean isDropVals = ((SQLAlterTableModifyPartitionValues) item).isDrop();
                        if (isDropVals) {
                            throw new ParserException(
                                "'alter index on table' does not support modify (sub)partitions drop values");
                        }
                    }
                }
                if (allItems.size() == 1) {
                    SQLAlterTableItem sqlAlterTableItem = allItems.get(0);
                    boolean partitionReorg =
                        sqlAlterTableItem instanceof DrdsSplitPartition
                            || sqlAlterTableItem instanceof DrdsMergePartition
                            || sqlAlterTableItem instanceof DrdsExtractHotKey
                            ...
                            || sqlAlterTableItem instanceof SQLAlterTableDropPartition
                            || sqlAlterTableItem instanceof SQLAlterTableDropSubpartition;
                    if (partitionReorg && lexer.token() == Token.WITH) {
                        lexer.nextToken();
                        if (lexer.identifierEquals(FnvHash.Constants.TABLEGROUP)) {
                            lexer.nextToken();
                            if (lexer.token() == Token.EQ) {
                            ...
                            SQLName tableGroup = this.exprParser.name();
                            acceptIdentifier("IMPLICIT");
                            alterTblStmt.setTargetImplicitTableGroup(tableGroup);
                        }
                    }
                }
                return alterTblStmt;
...
//            accept(Token.FULLTEXT);
    ...
    public SQLStatement parseCreateRoutingRule() {
        accept(Token.CREATE);
        acceptIdentifier("ROUTING_RULE");
        DrdsCreateRoutingRuleStatement stmt = new DrdsCreateRoutingRuleStatement();
        ...
        stmt.setRuleName(exprParser.name());
        accept(TO);
        stmt.setUserName(exprParser.name());

        //filter options
        if (lexer.identifierEquals("FILTER")) {
            lexer.nextToken();
            accept(Token.BY);
            if (lexer.identifierEquals("TEMPLATE")) {
                ...
                stmt.setTemplateId((SQLCharExpr) template);
            } else if (lexer.identifierEquals("KEYWORD")) {
                lexer.nextToken();
                ...
                for (SQLExpr item : ((SQLListExpr) keywords).getItems()) {
                ...
                setErrorEndPos(lexer.pos());
                throw new ParserException(
                    "syntax error, Failed to parse values of FILTER, " + lexer.info());
            }
        }

        //with options
        accept(Token.WITH);
        stmt.setWith(this.exprParser.parseAssignItem());
    ...
    public SQLStatement parseCreateCclBlocker() {
        if (lexer.token() == Token.CREATE) {
            lexer.nextToken();
        }

        DrdsCreateCclBlockerStatement stmt = new DrdsCreateCclBlockerStatement();

        acceptIdentifier("CCL_BLOCKER");
        ...
        if (lexer.token() == TO) {
            lexer.nextToken();
            if (lexer.token() == Token.LITERAL_CHARS) {
                SQLName user = exprParser.name();
                if (user instanceof MySqlUserName) {
                    stmt.setUser(user);
                }
            }
        }
    ...
    public SQLStatement parseCreateSchedule() {
        ...
        if (identifierEquals("LOCAL_PARTITION")) {
            ...
            stmt.setExecutorType("LOCAL_PARTITION");
            stmt.setForLocalPartition(true);
        } else if (identifierEquals("AUTO_SPLIT_TABLE_GROUP")) {
            acceptIdentifier("AUTO_SPLIT_TABLE_GROUP");
            stmt.setExecutorType("AUTO_SPLIT_TABLE_GROUP");
            stmt.setForAutoSplitTableGroup(true);
        } else if (lexer.identifierEquals("REFRESH_MATERIALIZED_VIEW")) {
            acceptIdentifier("REFRESH_MATERIALIZED_VIEW");
            stmt.setExecutorType("REFRESH_MATERIALIZED_VIEW");
        } else {
    ...
    public SQLStatement parseCreateDatabase() {
        ...
        //parse 'create database as/like'
        boolean isCreateDatabaseAsLikeStatement = false;
        if (lexer.token() == Token.LIKE || lexer.token() == Token.AS) {
            isCreateDatabaseAsLikeStatement = true;
            if (lexer.token() == Token.LIKE) {
                stmt.setLike(true);
                lexer.nextToken();
            } else {
            ...
            stmt.setSourceDatabase(this.exprParser.name());
        }
        ...
        for (; ; ) {
            if (lexer.identifierEquals(FnvHash.Constants.CHARACTER)) {
                ...
                stmt.setCharacterSet(charset);
            } else if (lexer.identifierEquals("ENCRYPTION")) {
                lexer.nextToken();
                if (lexer.token() == Token.EQ) {
                    lexer.nextToken();
                }
            ...
            } else if (lexer.identifierEquals(FnvHash.Constants.DEFAULT_SINGLE)) {
                lexer.nextToken();
                if (lexer.token() == Token.EQ) {
                    lexer.nextToken();
                }
                stmt.setDefaultSingle(this.exprParser.expr());
            } else if (lexer.token() == Token.LOCK) {
                accept(Token.LOCK);
                if (!isCreateDatabaseAsLikeStatement) {
                    throw new ParserException("INVALID TOKEN");
                }
                accept(EQ);
                if (lexer.token() == TRUE) {
                    accept(TRUE);
                    stmt.setWithLock(true);
                } else {
                    accept(FALSE);
                    stmt.setWithLock(false);
                }
            } else if (lexer.identifierEquals("DRY_RUN")) {
                lexer.nextToken();
                if (!isCreateDatabaseAsLikeStatement) {
                    throw new ParserException("INVALID TOKEN");
                }
                ...
                if (lexer.token() == TRUE) {
                    accept(TRUE);
                    stmt.setDryRunCreateAs(true);
                } else {
                    accept(FALSE);
                    stmt.setDryRunCreateAs(false);
                }
            } else if (lexer.identifierEquals("DRYRUN")) {
                // for register meta only
                lexer.nextToken();
                accept(EQ);
                if (lexer.token() == TRUE) {
                    accept(TRUE);
                    ...
                    accept(FALSE);
                    stmt.setDryrun(false);
                }
            } else if (lexer.identifierEquals("CREATE_TABLES")) {
                lexer.nextToken();
                if (!isCreateDatabaseAsLikeStatement) {
                    throw new ParserException("INVALID TOKEN");
                }
                ...
                if (lexer.token() == TRUE) {
                    accept(TRUE);
                    stmt.setCreateTables(true);
                } else {
                    accept(FALSE);
                    stmt.setCreateTables(false);
                }
            }
            //ads 特殊支持
        ...
        if (lexer.identifierEquals("INCLUDE")) {
            ...
            lexer.nextToken();
            accept(EQ);
            stmt.getIncludeTables().addAll(parseCreateDatabaseIncludeExcludeTables());
            ...
            lexer.nextToken();
            accept(EQ);
            stmt.getExcludeTables().addAll(parseCreateDatabaseIncludeExcludeTables());
        }
    ...
    protected List<SQLName> parseCreateDatabaseIncludeExcludeTables() {
        List<SQLName> result = new ArrayList<>();
        ...
        for (; ; ) {
            ...
            if (lexer.identifierEquals("IDENTIFIED")) {
                ...
                if (lexer.token() == Token.BY) {
                    ...
                    lexer.nextToken();
                    SQLCharExpr authPluginString = this.exprParser.charExpr();
                    lexer.nextToken(); // by or as
                    SQLCharExpr authString = this.exprParser.charExpr();
                }
            }
            ...
            if (lexer.identifierEquals("ACCOUNT")) {
                lexer.nextToken();
                if (Token.LOCK == lexer.token() || lexer.identifierEquals("LOCK")
                    || Token.UNLOCK == lexer.token() || lexer.identifierEquals("UNLOCK")) {
                    alterUser.setLock(Token.LOCK == lexer.token() || lexer.identifierEquals("LOCK"));
                    lexer.nextToken();
                }
            }

            if (lexer.identifierEquals("REQUIRE")) {
                lexer.nextToken();
                do {
                    if (lexer.identifierEquals("NONE") || lexer.identifierEquals("SSL") || lexer.identifierEquals(
                        "X509")) {
                        lexer.nextToken();
                    } else if (lexer.identifierEquals("CIPHER") || lexer.identifierEquals("ISSUER")
                        || lexer.identifierEquals("SUBJECT")) {
                        lexer.nextToken();
                        lexer.nextToken();
                    }
                } while (Objects.equals(lexer.token().name, "AND"));
            }

            if (lexer.token() == WITH) {
                lexer.nextToken();
                if (lexer.token() == GRANT) {
                    lexer.nextToken();
                    lexer.nextToken();
                } else if (lexer.identifierEquals("MAX_QUERIES_PER_HOUR")) {
                    lexer.nextToken();
                    SQLIntegerExpr maxQueriesPerHour = this.exprParser.integerExpr();
//                    alterUser.setMaxQueriesPerHour(maxQueriesPerHour);
                } else if (lexer.identifierEquals("MAX_UPDATES_PER_HOUR")) {
                    lexer.nextToken();
                    SQLIntegerExpr maxQueriesPerHour = this.exprParser.integerExpr();
                } else if (lexer.identifierEquals("MAX_CONNECTIONS_PER_HOUR")) {
                    lexer.nextToken();
                    SQLIntegerExpr maxQueriesPerHour = this.exprParser.integerExpr();
                } else if (lexer.identifierEquals("MAX_USER_CONNECTIONS")) {
                    lexer.nextToken();
                    SQLIntegerExpr maxQueriesPerHour = this.exprParser.integerExpr();
                }
            }

            if (lexer.identifierEquals("READ_STRATEGY")) {
                lexer.nextToken();
                if (lexer.identifierEquals("NONE")) {
                    alterUser.setReadStrategy(MySqlAlterUserStatement.ReadStrategy.NONE);
                    lexer.nextToken();
                } else if (lexer.identifierEquals("FOLLOWER")) {
                    alterUser.setReadStrategy(MySqlAlterUserStatement.ReadStrategy.FOLLOWER);
                    lexer.nextToken();
                } else if (lexer.identifierEquals("STALE")) {
                    alterUser.setReadStrategy(MySqlAlterUserStatement.ReadStrategy.STALE);
                    lexer.nextToken();
                }
            }
...
//                }
    ...
    public MySqlDeclareHandlerStatement parseDeclareHandler() {
        ...
        for (; ; ) {
            ...
            if (tokenName.equalsIgnoreCase("NOT")) {//for 'NOT FOUND'
                ...
                throw new ParserException(
                    "exception handler for sqlstate not support, currently support not found/error code/condition name");
            } else if (lexer.identifierEquals("SQLEXCEPTION")) { //for SQLEXCEPTION
                throw new ParserException(
                    "exception handler for sql exception not support, currently support not found/error code/condition name");
            } else if (lexer.identifierEquals("SQLWARNING")) { //for SQLWARNING
                throw new ParserException(
                    "exception handler for sql warning not support, currently support not found/error code/condition name");
    ...
    protected DrdsFireScheduleStatement parseFireSchedule() {
        ...
        boolean foundBy = lexer.token() == Token.BY;

        boolean useScheduleByTableName = false;
        boolean useScheduleByScheduleName = false;
        if (foundBy) {
            lexer.nextToken();
            if (lexer.token() == Token.TABLE) {
                useScheduleByTableName = true;
                lexer.nextToken();
            } else if (lexer.token().toString().equalsIgnoreCase("NAME")) {
                useScheduleByScheduleName = true;
                lexer.nextToken();
            }
        }
        if (useScheduleByTableName || useScheduleByScheduleName) {
            SQLExpr tarExpr = this.exprParser.expr();
            DrdsFireScheduleStatement stmt = new DrdsFireScheduleStatement();
            stmt.setDbType(dbType);
            stmt.setByScheduleName(useScheduleByScheduleName);
            stmt.setByTableName(useScheduleByTableName);
            stmt.setTargetExpr(tarExpr);
            return stmt;
        }
        ...
        stmt.setDbType(dbType);
        stmt.setScheduleId(lexer.integerValue().longValue());
        stmt.setByScheduleName(false);
        stmt.setByTableName(false);
        accept(Token.LITERAL_INT);
        return stmt;
    }
    ...
    // -------------------------------------------------------------------------
    // External Catalog / Secret parsing methods
    // -------------------------------------------------------------------------

    private SQLStatement parseAlterExternalCatalog() {
        acceptIdentifier("CATALOG");
        MySqlAlterExternalCatalogStatement stmt = new MySqlAlterExternalCatalogStatement();
        stmt.setName(this.exprParser.name());
        ...
        if (lexer.token() == Token.SET) {
            ...
            for (; ; ) {
                ...
                stmt.getSetProperties().put(key.toLowerCase(Locale.ROOT), value);
                if (lexer.token() == Token.COMMA) {
                    lexer.nextToken();
                    continue;
                }
        ...
        if (lexer.token() == Token.COMMENT) {
            lexer.nextToken();
            stmt.setComment(parseCommentValue());
            hasSomething = true;
        }
        if (!hasSomething) {
            throw new ParserException("ALTER EXTERNAL CATALOG requires SET or COMMENT clause. " + lexer.info());
        }

        return stmt;
    }

    private SQLStatement parseAlterSecret() {
        acceptIdentifier("SECRET");
        MySqlAlterSecretStatement stmt = new MySqlAlterSecretStatement();
        stmt.setName(this.exprParser.name());
    ...
    @Override
    protected SQLStatement parseRefreshExternal() {
        acceptIdentifier("EXTERNAL");
        MySqlRefreshExternalCatalogStatement stmt = new MySqlRefreshExternalCatalogStatement();

        if (lexer.identifierEquals(FnvHash.Constants.CATALOG)) {
            lexer.nextToken();
            stmt.setTable(false);
            stmt.setCatalogName(this.exprParser.name());
        } else if (lexer.token() == Token.TABLE) {
            lexer.nextToken();
            stmt.setTable(true);
            SQLName name = this.exprParser.name();
            SQLPropertyExpr tablePart = name instanceof SQLPropertyExpr ? (SQLPropertyExpr) name : null;
            SQLPropertyExpr dbPart = tablePart != null && tablePart.getOwner() instanceof SQLPropertyExpr
                ? (SQLPropertyExpr) tablePart.getOwner() : null;
            if (dbPart == null || !(dbPart.getOwner() instanceof SQLIdentifierExpr)) {
                throw new ParserException("syntax error, expect catalog.database.table after REFRESH EXTERNAL TABLE, "
                    + lexer.info());
            }
            stmt.setCatalogName((SQLIdentifierExpr) dbPart.getOwner());
            stmt.setDbName(new SQLIdentifierExpr(dbPart.getName()));
            stmt.setTableName(new SQLIdentifierExpr(tablePart.getName()));
        } else {
            throw new ParserException("syntax error, expect CATALOG or TABLE after REFRESH EXTERNAL, actual "
                + lexer.token() + ", " + lexer.info());
        }
    ...
    private SQLStatement parseCreateSecret() {
        accept(Token.CREATE);
        MySqlCreateSecretStatement stmt = new MySqlCreateSecretStatement();
        ...
        for (; ; ) {
            ...
            accept(Token.EQ);
            String value = parsePropertyValue();
            stmt.getProperties().put(key.toLowerCase(Locale.ROOT), value);
            if (lexer.token() == Token.COMMA) {
                lexer.nextToken();
                continue;
            }
    ...
    @Override
    protected SQLStatement parseDropSecret() {
        acceptIdentifier("SECRET");
        MySqlDropSecretStatement stmt = new MySqlDropSecretStatement();
