...
public class RunLengthIntegerReaderV2 implements IntegerReader {
  ...
  public void setMemoryCounter(ORCProfile memoryCounter) {
    ...
    // The fixed memory overhead for this object.
  ...
  private void readPatchedBaseValues(int firstByte) throws IOException {
    ...
    if (memoryCounter != null) {
      memoryCounter.update(len);
      memoryCounter.update(pl);
    }
  ...
  private int getRLEBlockSize(int len, int bitWidth) {
    return ((len) * (bitWidth) + 7) / 8;
  }

  private int skipValues(long numValues, boolean ignoreEof) throws IOException {
    // read the first 2 bits and determine the encoding type
    isRepeating = false;
    int firstByte = input.read();
    if (firstByte < 0) {
    ...
    currentEncoding = encodings[(firstByte >>> 6) & 0x03];
    switch (currentEncoding) {
    case SHORT_REPEAT: return skipShortRepeatValues(numValues, firstByte);
    case DIRECT: return skipDirectValues(numValues, firstByte);
    case PATCHED_BASE: return skipPatchedBaseValues(numValues, firstByte);
    case DELTA: return skipDeltaValues(numValues, firstByte);
    default: throw new IOException("Unknown encoding " + currentEncoding);
    }
  }

  private int skipDeltaValues(long numValues, int firstByte) throws IOException {
  ...
  private int skipPatchedBaseValues(long numValues, int firstByte) throws IOException {
    ...
    // skip the entire block whenever possible
    if (numValues >= len) {
      // skip Next Block
      final int skipSize = getRLEBlockSize(len, fb);
      input.skip(skipSize);

      // need to skip the patch list if necessary
      if (pl > 0) {
        final int cfb = utils.getClosestFixedBits(pw + pgw);
        final int skipPatchListSize = getRLEBlockSize(pl, cfb);
        input.skip(skipPatchListSize);
      }
  ...
  private int skipDirectValues(long numValues, int firstByte) throws IOException {
  ...
  private int skipShortRepeatValues(long numValues, int firstByte) throws IOException {
  ...
  public void skip(long numValues) throws IOException {
    while (numValues > 0) {
      if (used == numLiterals) {
        numLiterals = 0;
        used = 0;
        int skipSize = skipValues(numValues, false);
        numValues -= skipSize;
      }
