...
public class CheckChainAllProcedure extends BaseInnerProcedure {
    @Override
    public void execute(ServerConnection c, SQLCallStatement statement, ArrayResultCursor cursor) {
        ...
        if (params.size() != 2) {
            throw new IllegalArgumentException("Expects two parameters: schema_name and table_name."
                + " Example: call polardbx.check_chain_all('db1', 'tb1')");
        }
        final String schemaName = params.get(0).toString();
        final String tableName = params.get(1).toString();

        String result;
        StringBuilder sb = new StringBuilder();
        if (GlobalChainUtils.checkAll(schemaName, tableName, sb)) {
            result = "OK";
        } else {
            result = "FAIL";
        }
