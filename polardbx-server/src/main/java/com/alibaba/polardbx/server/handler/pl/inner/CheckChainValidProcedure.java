...
public class CheckChainValidProcedure extends BaseInnerProcedure {
    @Override
    public void execute(ServerConnection c, SQLCallStatement statement, ArrayResultCursor cursor) {
        ...
        if (params.size() != 2) {
            throw new IllegalArgumentException("Expects two parameters: schema_name and table_name."
                + " Example: call polardbx.check_chain_valid('db1', 'tb1')");
        }
        ...
        if (GlobalChainUtils.checkChain(schemaName, tableName, sb)) {
            result = "OK";
        } else {
            result = "FAIL";
        }
