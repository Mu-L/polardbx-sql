...
public class ColumnarUnignoreProcedure extends BaseInnerProcedure {
    @Override
    public void execute(ServerConnection c, SQLCallStatement statement, ArrayResultCursor cursor) {
        ...
        /**
         * 支持参数：
         * columnar_unignore(indexId1,indexId2,indexId3...)，通过列存tableId匹配
         */
        List<Long> tableIds = parseTableIds(statement.getParameters(), statement);

        if (!DynamicConfig.getInstance().enableColumnarIgnore()) {
            cursor.addRow(new Object[] {"FAIL: NOT ALLOW TO UN-IGNORE CCI"});
            return;
        }

        //返回结果
        try {
            ExecUtils.columnarUnIgnore(tableIds);
