...
import com.alibaba.polardbx.server.handler.pl.inner.CheckChainGlobalProcedure;
 ...
 * @author lijiu.lzw
 */
public class CheckChainGlobalProcedureTest {
    @Test
    public void test() {
        try (MockedStatic<GlobalChainUtils> ignored = mockStatic(GlobalChainUtils.class)) {
            SQLCallStatement statement =
                (SQLCallStatement) FastsqlUtils.parseSql("call polardbx.check_chain_global(10)",
                    SQLParserFeature.IgnoreNameQuotes).get(0);
            CheckChainGlobalProcedure procedure = new CheckChainGlobalProcedure();
            ...
            statement =
                (SQLCallStatement) FastsqlUtils.parseSql("call polardbx.check_chain_global(db, table)",
                    SQLParserFeature.IgnoreNameQuotes).get(0);
            when(GlobalChainUtils.checkGlobal(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(true);
            procedure.execute(null, statement, cursor);
            Assert.assertEquals(1, cursor.getRows().size());
            Assert.assertEquals("OK", cursor.getRows().get(0).getObject(0));
            when(GlobalChainUtils.checkGlobal(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(false);
