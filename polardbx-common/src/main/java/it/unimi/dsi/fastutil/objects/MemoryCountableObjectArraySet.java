 ...
 */
public class MemoryCountableObjectArraySet<K> extends AbstractObjectSet<K>
    implements java.io.Serializable, Cloneable, MemoryCountable {
    private static final int INSTANCE_SIZE = ClassLayout.parseClass(MemoryCountableObjectArraySet.class).instanceSize();
    ...
    private long elementsMemoryUsage = 0;

    @FieldMemoryCounter(value = false)
    private final MemoryCounter<K> elementMemoryCounter;

    @Override
    public long getMemoryUsage() {
        return INSTANCE_SIZE
            + VMSupport.align((int) SizeOf.sizeOf(a))
            + elementsMemoryUsage;
    }
     ...
     */
    public MemoryCountableObjectArraySet(final K[] a, final MemoryCounter<K> elementMemoryCounter) {
        this.a = a;
        size = a.length;
        this.elementMemoryCounter = Objects.requireNonNull(elementMemoryCounter);
    }

    /**
     * Creates a new empty array set.
     */
    public MemoryCountableObjectArraySet(final MemoryCounter<K> elementMemoryCounter) {
     ...
     */
    public MemoryCountableObjectArraySet(final int capacity, final MemoryCounter<K> elementMemoryCounter) {
     ...
     */
    public MemoryCountableObjectArraySet(ObjectCollection<K> c, final MemoryCounter<K> elementMemoryCounter) {
        this(c.size(), elementMemoryCounter);
        addAll(c);
    }
     ...
     * @param c a collection.
     */
    public MemoryCountableObjectArraySet(final Collection<? extends K> c, final MemoryCounter<K> elementMemoryCounter) {
        this(c.size(), elementMemoryCounter);
        addAll(c);
    }
     ...
     * @param c a collection.
     */
    public MemoryCountableObjectArraySet(ObjectSet<K> c, final MemoryCounter<K> elementMemoryCounter) {
        this(c.size(), elementMemoryCounter);
        int i = 0;
        for (K x : c) {
            elementsMemoryUsage += elementMemoryCounter.getMemoryUsage(x);
            a[i] = x;
            i++;
        }
        size = i;
     ...
     * @param c a collection.
     */
    public MemoryCountableObjectArraySet(final Set<? extends K> c, final MemoryCounter<K> elementMemoryCounter) {
        this(c.size(), elementMemoryCounter);
        int i = 0;
        for (K x : c) {
            elementsMemoryUsage += elementMemoryCounter.getMemoryUsage(x);
     ...
     */
    public MemoryCountableObjectArraySet(final K[] a, final int size, final MemoryCounter<K> elementMemoryCounter) {
        this.a = a;
        this.size = size;
        this.elementMemoryCounter = Objects.requireNonNull(elementMemoryCounter);
        if (size > a.length) {
    ...
    public ObjectIterator<K> iterator() {
        return new ObjectIterator<K>() {
            ...
            public void remove() {
                ...
                elementsMemoryUsage -= elementMemoryCounter.getMemoryUsage(a[size]);
                a[size] = null;
            }
    ...
    public boolean add(final K k) {
        ...
        if (size == a.length) {
            final K[] b = (K[]) new Object[size == 0 ? 2 : size * 2];
            for (int i = size; i-- != 0; ) {
                b[i] = a[i];
            }
            a = b;
        }
        elementsMemoryUsage += elementMemoryCounter.getMemoryUsage(k);
        a[size++] = k;
        return true;
    }
    ...
    @Override
    @SuppressWarnings("unchecked")
    public MemoryCountableObjectArraySet<K> clone() {
        MemoryCountableObjectArraySet<K> c;
        try {
            c = (MemoryCountableObjectArraySet<K>) super.clone();
