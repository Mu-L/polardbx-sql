...
public class OSSFileSystem extends FileSystem implements RateLimitable {
    ...
    public FSDataOutputStream create(Path path, FsPermission permission,
        ...
        long uploadPartSize = OSSUtils.getMultipartSizeProperty(getConf(),
            MULTIPART_UPLOAD_PART_SIZE_KEY, MULTIPART_UPLOAD_PART_SIZE_DEFAULT);
        return new FSDataOutputStream(
            new OSSBlockOutputStream(getConf(),
                store,
                key,
                uploadPartSize,
                new SemaphoredDelegatingExecutor(boundedThreadPool, blockOutputActiveBlocks, true),
    ...
    private boolean copyDirectory(Path srcPath, Path dstPath) throws IOException {
        String srcKey = OSSUtils
            .maybeAddTrailingSlash(pathToKey(srcPath));
        String dstKey = OSSUtils
        ...
        OSSCopyFileContext copyFileContext = new OSSCopyFileContext();
        ...
        while (true) {
            for (OSSObjectSummary objectSummary : objects.getObjectSummaries()) {
                ...
                executorService.execute(new OSSCopyFileTask(
    ...
    public OSSFileSystemStore getStore() {
        return store;
    }
