 ...
 * RuleUtils#getPartialFetch (MPP path): a negative literal offset or fetch must be
 * rejected the same way as CBOUtil#calPushDownFetch (SMP path), instead of being
 * silently summed into a smaller/incorrect fetch value.
 */
public class RuleUtilsTest {
    ...
    private Limit createLimit(RexNode offset, RexNode fetch) {
        return Limit.create(RelTraitSet.createEmpty().replace(MppConvention.INSTANCE), input, offset, fetch);
    }
    ...
    public void testNegativeLiteralOffsetRejected() {
        ...
        try {
            RuleUtils.getPartialFetch(limit);
    ...
    public void testPositiveLiteralOffsetFetchSummed() {
        ...
        RexNode result = RuleUtils.getPartialFetch(limit);
