 ...
 * @author wuzhi
 */
public final class AutoPartitionByLong extends DblePartitionAlgorithm implements RuleAlgorithm {
    ...
    private String mapFileString = null;
    ...
    public void init() {
//        initialize();
        if (StringUtils.isEmpty(mapFileString)) {
...
//
            if (this.partitionMappings != null) {
                for (int i = 0; i <  this.partitionMappings.size(); i++) {
                    Pair<String, Integer> mapItem = this.partitionMappings.get(i);
                    String key = mapItem.getKey();
                    Integer value = mapItem.getValue();
                    String[] pairs = key.split("-");
                    long longStart = NumberParseUtil.parseLong(pairs[0].trim());
                    long longEnd = NumberParseUtil.parseLong(pairs[1].trim());
                    int nodeId = value;
