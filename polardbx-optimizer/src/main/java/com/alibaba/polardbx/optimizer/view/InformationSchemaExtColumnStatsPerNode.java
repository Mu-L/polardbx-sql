...
/**
 * INFORMATION_SCHEMA.EXT_COLUMN_STATS_PER_NODE
 * Per-CN breakdown of externalized column I/O metrics. Returns one row per CN node.
 * Use INFORMATION_SCHEMA.EXT_COLUMN_STATS for the cluster-aggregated single-row view.
 */
public class InformationSchemaExtColumnStatsPerNode extends VirtualView {

    public InformationSchemaExtColumnStatsPerNode(RelOptCluster cluster, RelTraitSet traitSet) {
        super(cluster, traitSet, VirtualViewType.EXT_COLUMN_STATS_PER_NODE);
    }

    public InformationSchemaExtColumnStatsPerNode(RelInput relInput) {
        super(relInput);
    }
    ...
    protected RelDataType deriveRowType() {
        ...
        columns.add(
            new RelDataTypeFieldImpl("STAGING_ACTIVE_SEQ_ID", i++, typeFactory.createSqlType(SqlTypeName.BIGINT)));
        columns.add(
            new RelDataTypeFieldImpl("STAGING_FLUSHED_WATERMARK", i++, typeFactory.createSqlType(SqlTypeName.BIGINT)));
