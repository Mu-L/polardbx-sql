...
//        ImmutableList.of(exp));
  ...
  public RexNode makeCastForConvertlet(RelDataType type, RexNode exp) {
    return makeCastForConvertlet(type, exp, SqlStdOperatorTable.CAST);
  }

  public RexNode makeCastForConvertlet(RelDataType type, RexNode exp, SqlOperator castOp) {
    Preconditions.checkArgument(castOp instanceof SqlCastFunction);
    return new RexCall(
        type,
        castOp,
        ImmutableList.of(exp));
   ...
   */
  public RexLiteral makeBigIntLiteral(Long l) {
    RelDataType bigintType =
        typeFactory.createSqlType(
            SqlTypeName.BIGINT);
    return makeLiteral(l, bigintType, SqlTypeName.BIGINT);
  }
    /**
     * Create an Int literal.
     */
    public RexLiteral makeIntLiteral(Integer integer) {
        RelDataType intType =
                typeFactory.createSqlType(
                        SqlTypeName.INTEGER);
        return makeLiteral(integer, intType, SqlTypeName.INTEGER);
    }
  /**
   * Creates a numeric literal.
   */
  ...
  public RexSystemVar makeSystemVar(String name, VariableScope scope) {
    assert name != null;
    // Here we simply assume that system variables are all strings
    return new RexSystemVar(name, scope,
        typeFactory.createSqlType(SqlTypeName.VARCHAR),
        SqlTypeName.VARCHAR);
  }


  public RexUserVar makeUserSymbolLiteral(String name) {
    assert name != null;
    // Here we simply assume that user variables are all strings
    return new RexUserVar(name,
        typeFactory.createSqlType(SqlTypeName.VARCHAR),
        SqlTypeName.VARCHAR);
  }
...
//        return new RexInputRef(ref.getIndex(), targetType);
//      } else {
...
//    }
  ...
  public RexNode ensureNullability(
      RelDataType type,
      RexNode node,
      boolean nullability) {
    RelDataType targetType = getTypeFactory().createTypeWithNullability(
          type,
          nullability);
    ...
    if (node instanceof RexInputRef) {
      ...
      return ((RexCall) node).clone(targetType, ((RexCall) node).operands);
    }
    return node;
  }
  ...
  public RexDynamicParam makeDynamicParam(
          ...
          int index,
          RelNode rel) {
    return new RexDynamicParam(type, index, rel);
  }
  ...
  public RexNode makeLiteral(Object value, RelDataType type,
    ...
    switch (type.getSqlTypeName()) {
    case CHAR:
      return makeCharLiteral((NlsString) value);
    case VARCHAR:
    ...
    case DECIMAL:
      return makeLiteral((Comparable) value, type, type.getSqlTypeName());
                // case INTEGER_UNSIGNED:
                // return makeExactLiteral(new BigDecimal((Long) value), type);
    case FLOAT:
    case REAL:
    case DOUBLE:
    ...
    case TIMESTAMP:
    case DATETIME:
      TimestampString timestampString = null;
      if (value instanceof TimestampString) {
        timestampString = (TimestampString) value;
      } else if (value instanceof Timestamp) {
        Timestamp t = (Timestamp) value;
        timestampString = new TimestampString(t.getYear(), t.getMonth(),
            t.getDay(), t.getHours(), t.getMinutes(), t.getSeconds());
        timestampString.withNanos(t.getNanos());
      } else if (value instanceof OriginalTimestamp) {
        MysqlDateTime mysqlDateTime = ((OriginalTimestamp) value).getMysqlDateTime();
        timestampString = new TimestampString((int)mysqlDateTime.getYear(), (int)mysqlDateTime.getMonth(),
            (int)mysqlDateTime.getDay(), (int)mysqlDateTime.getHour(), (int)mysqlDateTime.getMinute(), (int)mysqlDateTime.getSecond());
        timestampString.withNanos((int)mysqlDateTime.getSecondPart());
      }
      return makeTimestampLiteral(timestampString, type.getPrecision());
    case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
  ...
  private static Object clean(Object o, RelDataType type) {
    ...
    case BIGINT_UNSIGNED:
        if (o instanceof BigInteger) {
          ...
          return ((UInt64) o).toBigInteger();
        }
        return new BigInteger(String.valueOf(((Number) o).longValue()));
    case DECIMAL:
      if (type.getScale() > 0) {
    ...
    case TIME:
      if (o instanceof TimeString) {
        ...
        c.setTime((Time) o);
        return TimeString.fromCalendarFields(c);
      } else {
    ...
    case DATE:
      if (o instanceof DateString) {
        ...
        Calendar c = Calendar.getInstance();
        c.setTime(new java.util.Date(((Date) o).getTime()));
        return DateString.fromCalendarFields(c);
      } else{
    ...
    case TIMESTAMP:
      if (o instanceof TimestampString) {
        ...
        return TimestampString.fromTimestampFields((Timestamp) o);
      } else {
  ...
  public RexNode makeDynamicParam(RelDataType type, int index, RelNode rel, ImmutableList<RexNode> operands, SqlOperator op,
                               SqlKind kind) {
    return new RexDynamicParam(type, index, rel, operands, op, kind);
  }
}
