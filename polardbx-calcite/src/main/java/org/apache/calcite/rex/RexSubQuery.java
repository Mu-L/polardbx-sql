...
public class RexSubQuery extends RexCall {
  ...
  public RexSubQuery(RelDataType type, SqlOperator op, ImmutableList<RexNode> operands, RelNode rel) {
  ...
  /** Creates an NOT IN sub-query. */
  public static RexSubQuery not_in(RelNode rel, ImmutableList<RexNode> nodes) {
    final RelDataType type = type(rel, nodes);
    return new RexSubQuery(type, SqlStdOperatorTable.NOT_IN, nodes, rel);
  }
  ...
  public static RexNode all(RelNode rel, ImmutableList<RexNode> nodes,
                            SqlQuantifyOperator op) {
    assert op.kind == SqlKind.ALL;
    if (op == SqlStdOperatorTable.ALL_NE) {
      return RexSubQuery.not_in(rel, nodes);
    }
    final RelDataType type = type(rel, nodes);
    return new RexSubQuery(type, op, nodes, rel);
  }
  ...
  /** Creates an not EXISTS sub-query. */
  public static RexSubQuery not_exists(RelNode rel) {
    final RelDataTypeFactory typeFactory = rel.getCluster().getTypeFactory();
    final RelDataType type = typeFactory.createSqlType(SqlTypeName.BIGINT);
    return new RexSubQuery(type, SqlStdOperatorTable.NOT_EXISTS,
            ImmutableList.<RexNode>of(), rel);
  }
  ...
  @Override public RexSubQuery clone(RelDataType type, List<RexNode> operands) {
    return new RexSubQuery(type, getOperator(),
        ImmutableList.copyOf(operands), rel).setHasOptimized(hasOptimized);
  }

  public RexSubQuery clone(RelNode rel) {
    return new RexSubQuery(type, getOperator(), operands, rel).setHasOptimized(hasOptimized);
  }
  ...
  public RexSubQuery setHasOptimized(boolean hasOptimized) {
