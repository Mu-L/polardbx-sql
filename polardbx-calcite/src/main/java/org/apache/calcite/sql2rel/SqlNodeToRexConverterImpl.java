...
public class SqlNodeToRexConverterImpl implements SqlNodeToRexConverter {
  ...
  public RexNode convertLiteral(
    ...
    if (literal instanceof SqlSystemVar) {
      final SqlSystemVar systemVar = (SqlSystemVar) literal;
      return rexBuilder.makeSystemVar(systemVar.getName(), systemVar.getScope());
    }

    if (literal instanceof SqlUserDefVar) {
      return rexBuilder.makeUserSymbolLiteral(((SqlUserDefVar) literal).getName());
    }
    ...
    case INTEGER:
      Integer integer = literal.getValueAs(Integer.class);
      return  rexBuilder.makeLiteral(integer, literal.createSqlType(typeFactory), false);
    case INTEGER_UNSIGNED:
    case BIGINT:
      Long aLong = literal.getValueAs(Long.class);
      return rexBuilder.makeLiteral(aLong, literal.createSqlType(typeFactory), false);
    case BIGINT_UNSIGNED:
        BigInteger bigInteger = literal.getValueAs(BigInteger.class);
        return rexBuilder.makeLiteral(bigInteger, literal.createSqlType(typeFactory), false);
...
//          "incomplete octet");
    ...
    case INTERVAL_SECOND:
      ...
      return rexBuilder.makeLiteral(SqlLiteral.value(literal),
          typeFactory.createSqlIntervalType(sqlIntervalQualifier),
