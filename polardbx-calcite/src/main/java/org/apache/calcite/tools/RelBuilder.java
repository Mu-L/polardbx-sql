...
public class RelBuilder {
  ...
  private final RelFactories.ModifyFactory modifyFactory;
  ...
  protected RelBuilder(Context context, RelOptCluster cluster,
    ...
    this.modifyFactory = Util.first(context.unwrap(RelFactories.ModifyFactory.class),
        RelFactories.DEFAULT_MODIFY_FACTORY);
  ...
  public RelBuilder copy() {
    RelBuilder newRelBuilder = RelBuilder.proto(Contexts.EMPTY_CONTEXT).create(this.cluster, this.relOptSchema);
    Iterator<Frame> i = stack.iterator();
    while(i.hasNext()) {
      newRelBuilder.push(i.next().rel);
    }
    return newRelBuilder;
  }
  ...
  public RexNode literal(Object value) {
    ...
    if (value == null) {
      ...
      return rexBuilder.makeBigIntLiteral(((Number) value).longValue());
...
//    }
    ...
    final RexNode x = RexUtil.composeConjunction(getRexBuilder(), predicates, false);
//    if (x.isAlwaysFalse()) {
//      return empty();
//    }
//    if (!x.isAlwaysTrue()) {
...
//    }
  ...
  public RelBuilder filter(Set<CorrelationId> variablesSet, RexNode... predicates) {
    return filter(ImmutableSet.copyOf(variablesSet), ImmutableList.copyOf(predicates));
  }


  public RelBuilder filter(ImmutableSet<CorrelationId> variablesSet, Iterable<? extends RexNode> predicates) {
    if (checkPredicate(predicates)) {
      return this;
    }
//    final RexNode x = simplifierUnknownAsFalse.simplifyAnds(predicates);
...
//    if (!x.isAlwaysTrue()) {
      final Frame frame = stack.pop();
      final RelNode filter = filterFactory.createFilter(frame.rel, x, variablesSet);
      stack.push(new Frame(filter, frame.fields));
//    }
    return this;
  }

  private boolean checkPredicate(Iterable<? extends RexNode> predicates) {
    if (Iterables.size(predicates) == 0) {
      return true;
    } else if (Iterables.size(predicates) == 1) {
      return onePredicateAndIsTrue(predicates.iterator().next());
    } else {
      return false;
    }
  }

  private boolean onePredicateAndIsTrue(RexNode node) {
    if (node instanceof RexLiteral) {
      RexLiteral literal = (RexLiteral)node;
      if (null == literal.getValue()) {
        return false;
      }
      return "true".equalsIgnoreCase(literal.getValue().toString());
    } else {
      return false;
    }
  }
  ...
  public RelBuilder project(Iterable<? extends RexNode> nodes,
    ...
    return project(nodes, fieldNames, false, ImmutableSet.<CorrelationId>of());
  }
  ...
  public RelBuilder project(
      ...
      Iterable<String> fieldNames,
      boolean force, Set<CorrelationId> var) {
    final List<String> originalNames = new ArrayList<>();
    for(String name : fieldNames) {
      originalNames.add(name);
    }
    return project(nodes, fieldNames, originalNames, force, var);
  }
  ...
  public RelBuilder rename(List<String> fieldNames) {
    ...
    project(fields(), newFieldNames, true, ImmutableSet.<CorrelationId>of());
    ...
    if (Iterables.isEmpty(aggCalls) && !groupKey_.indicator) {
      ...
      if (registrar.extraNodes.size() == fields().size()) {
        ...
        if (unique != null && unique) {
          if (this.peek().getRowType().getFieldCount() > groupSet.cardinality()) {
            // need a project
            return this.project(this.fields(groupSet.asList()));
          } else {
  ...
  public RelBuilder semiJoin(Iterable<? extends RexNode> conditions, SqlNodeList hints) {
    final Frame right = stack.pop();
    final RelNode semiJoin =
        semiJoinFactory.createSemiJoin(peek(), right.rel, and(conditions), hints);
    replaceTop(semiJoin);
    return this;
  }

  /** Creates a {@link org.apache.calcite.rel.core.SemiJoin}. */
  public RelBuilder semiJoin(SqlNodeList hints, RexNode... conditions) {
    return semiJoin(ImmutableList.copyOf(conditions), hints);
  }

  public RelBuilder logicalSemiJoin(Iterable<? extends RexNode> conditions, SqlNodeList hints) {
    final Frame right = stack.pop();
    ...
    final RelNode semiJoin = LogicalSemiJoin.create(
        peek(), right.rel,  and(conditions), joinInfo.leftKeys, joinInfo.rightKeys, JoinRelType.SEMI, hints);
    replaceTop(semiJoin);
    return this;
  }
  ...
  public RelBuilder logicalSemiJoin(List<RexNode> conditions, SqlOperator operator, JoinRelType joinRelType, List<RexNode> linkRexNode,
    ...
    final Frame left = stack.pop();
    final RelNode semiJoin =
            semiJoinFactory.createSemiJoin(left.rel, right.rel, and(conditions), joinRelType, linkRexNode, variablesSet, hints, position);
    ((LogicalSemiJoin)semiJoin).setOperator(operator);
    final ImmutableList.Builder<Field> fields = ImmutableList.builder();
    fields.addAll(left.fields);
    fields.addAll(right.fields);
    stack.push(new Frame(semiJoin, ImmutableList.copyOf(fields.build())));
    return this;
  }
  ...
  public RelBuilder sortLimit(RexNode offsetNode, RexNode fetchNode,
      Iterable<? extends RexNode> nodes) {
  ...
  public RelBuilder modify(TableModify modify) {
    final Frame frame = stack.pop();
    final RelNode newModify = modifyFactory.createModify(modify, frame.rel);
    stack.push(new Frame(newModify, frame.fields));
    return this;
  }
  ...
  public RelBuilder project(List<RexNode> nodes, List<String> fieldNames, Set<CorrelationId> variablesSet) {
    return project(nodes, fieldNames, false, variablesSet);
  }

  public RelBuilder project(List<RexNode> nodes, List<String> fieldNames) {
    return project(nodes, fieldNames, false, ImmutableSet.<CorrelationId>of());
  }
