...
public class PushProjector {
  ...
  public PushProjector(
    ...
    if (childRel instanceof SemiJoin) {
      childFields = ImmutableList.<RelDataTypeField>builder()
          .addAll(((SemiJoin) childRel).getLeft().getRowType().getFieldList())
          .addAll(((SemiJoin) childRel).getRight().getRowType().getFieldList())
          .build();
    } else {
  ...
  public Project createProjectRefsAndExprs(
    ...
    for (RexNode projExpr : preserveExprs) {
      ...
      // FIXME: use better way to avoid alias `*`
      // alias `*` will confuse mysql, so we use `__*__` instead
      String aliasName = ((RexCall) projExpr).getOperator().getName();
      if ("*".equals(aliasName)) {
        aliasName = "__*__";
      }
  ...
  private class InputSpecialOpFinder extends RexVisitorImpl<Void> {
    ...
    private boolean preserve(RexNode call) {
      ...
      // don't preserve implicit_cast function.
      if(RexUtil.containsRootImplicitCast(call)) {
        return false;
      }
      if (!call.getType().isStruct() && preserveExprCondition.test(call)) {
