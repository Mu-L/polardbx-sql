...
public abstract class BuiltInMetadata {
  ...
  public interface CompositePk extends Metadata {
    MetadataDef<CompositePk> DEF = MetadataDef.of(CompositePk.class,
            CompositePk.Handler.class, BuiltInMethod.GET_PK.method);

    /**
     * Get primary key bit set from a specified relational expression.
     *
     * @return Primary key bit set.
     */
    ImmutableBitSet getPrimaryKey();

    /** Handler API. */
    interface Handler extends MetadataHandler<CompositePk> {
      ImmutableBitSet getPrimaryKey(RelNode r, RelMetadataQuery mq);
    }
  }
  ...
  /** Metadata about the origins of columns. */
  public interface ColumnOriginName extends Metadata {
    MetadataDef<ColumnOriginName> DEF = MetadataDef.of(ColumnOriginName.class,
        ColumnOriginName.Handler.class, BuiltInMethod.COLUMN_ORIGIN_NAME.method);
     ...
     * all)
     */
    List<Set<RelColumnOrigin>> getColumnOriginNames();

    /** Handler API. */
    interface Handler extends MetadataHandler<ColumnOriginName> {
      List<Set<RelColumnOrigin>> getColumnOriginNames(RelNode r, RelMetadataQuery mq);
    }
  }

  /** Metadata about the origins of columns. */
  public interface OriginalRowType extends Metadata {
    MetadataDef<OriginalRowType> DEF = MetadataDef.of(OriginalRowType.class,
            OriginalRowType.Handler.class, BuiltInMethod.ORIGINAL_ROW_TYPE.method);
     ...
     * all)
     */
    RelDataType getOriginalRowType();

    /** Handler API. */
    interface Handler extends MetadataHandler<OriginalRowType> {
      RelDataType getOriginalRowType(RelNode r, RelMetadataQuery mq);
    }
  }
  ...
  public interface FunctionalDependency extends Metadata {
    MetadataDef<FunctionalDependency> DEF = MetadataDef.of(FunctionalDependency.class,
        FunctionalDependency.Handler.class, BuiltInMethod.FUNCTIONAL_DEPENDENCY.method);

    /**
     * @param 
     * @return Map of origin columns, or null if this information cannot be
     * determined (whereas empty set indicates definitely no origin columns at
     * all)
     */
    Map<ImmutableBitSet, ImmutableBitSet> getFunctionalDependency(ImmutableBitSet iOutputColumns);

    /** Handler API. */
    interface Handler extends MetadataHandler<FunctionalDependency> {
      Map<ImmutableBitSet, ImmutableBitSet> getFunctionalDependency(RelNode r, RelMetadataQuery mq, ImmutableBitSet iOutputColumns);
    }
  }

  /** Metadata about the covering index. */
  public interface CoveringIndex extends Metadata {
    MetadataDef<CoveringIndex> DEF = MetadataDef.of(CoveringIndex.class,
        CoveringIndex.Handler.class, BuiltInMethod.COVERING_INDEX.method);

    /**
     * For a given plan and global secondary index, determines whether all columns
     * contribute to result values are coming from index table. Each output column may
     * have more than one origin due to expressions such as Union and LogicalProject.
     * The optimizer may use this information for access path selection
     *
     * @return list of set of origin columns, or null if any column not covering by global
     * secondary index is referenced
     */
    List<Set<RelColumnOrigin>> isCoveringIndex(RelOptTable table, String index);

    /** Handler API. */
    interface Handler extends MetadataHandler<CoveringIndex> {
      List<Set<RelColumnOrigin>> isCoveringIndex(RelNode r, RelMetadataQuery mq, RelOptTable table, String index);
    }
  }
  ...
  public interface TableReferences extends Metadata {
     ...
     */
    Set<RelTableRef> getTableReferences(boolean logicalViewLevel);

    /** Handler API. */
    interface Handler extends MetadataHandler<TableReferences> {
      Set<RelTableRef> getTableReferences(RelNode r, RelMetadataQuery mq, boolean logicalViewLevel);
    }
  }
  ...
  /** Metadata about the start up cost of evaluating a relational expression, including
   * all of its inputs. */
  public interface StartUpCost extends Metadata {
    MetadataDef<StartUpCost> DEF = MetadataDef.of(StartUpCost.class,
        StartUpCost.Handler.class,
        BuiltInMethod.START_UP_COST.method);

    RelOptCost getStartUpCost();

    /** Handler API. */
    interface Handler extends MetadataHandler<StartUpCost> {
      RelOptCost getStartUpCost(RelNode r, RelMetadataQuery mq);
    }
  }
  ...
  public interface Parallelism extends Metadata {
    MetadataDef<Parallelism> DEF = MetadataDef.of(Parallelism.class,
        ...
        BuiltInMethod.SPLIT_COUNT.method, BuiltInMethod.WORKER_COUNT.method);
    ...
    interface Handler extends MetadataHandler<Parallelism> {
      ...
      Double workerCount(RelNode r, RelMetadataQuery mq);
    }
  }
