...
//  }
  ...
  public static RelNode create(RelOptCluster cluster,
    ...
    // references to newly generated constant pool.
      // For: RexSlot,RexProgram,Over index
    ...
    class RexShuttleCorrectConstants extends RexShuttle {
      final private List<RexLiteral> correctedConstants = new ArrayList<>();
      final Map<RexInputRef, RexInputRef> correctedConstantPool = new HashMap<>();
      final private int inputFieldCount;
      final private int adjustIndex;
      public RexShuttleCorrectConstants(int inputFieldCount, int adjustIndex) {
        this.inputFieldCount = inputFieldCount;
        this.adjustIndex = adjustIndex;
      }
      
      @Override public RexNode visitInputRef(RexInputRef inputRef) {
        if (!constantPool.containsValue(inputRef)) {
          return inputRef;
        }
        if (correctedConstantPool.containsKey(inputRef)) {
          return correctedConstantPool.get(inputRef);
        }
        int index = inputRef.getIndex() - inputFieldCount;
        RexLiteral rexLiteral = constants.get(index);
        RexInputRef ref = new RexInputRef(correctedConstants.size() + inputFieldCount + adjustIndex,
            rexLiteral.getType());
        correctedConstants.add(rexLiteral);
        correctedConstantPool.put(inputRef, ref);
        return ref;
      }
    }
    ...
    List<WindowKey> listKeys = Arrays.asList( windowMap.keySet().toArray(new WindowKey[0]));
    Collections.sort(listKeys, new Comparator<WindowKey>() {
      @Override
      public int compare(WindowKey o1, WindowKey o2) {

          WindowSortedKey sortedKey1 = new WindowSortedKey(o1);
          WindowSortedKey sortedKey2 = new WindowSortedKey(o2);
        int i = sortedKey1.compareTo(sortedKey2);
        return i;
      }
    });
    int[] groupSplit = new int[listKeys.size()];
    int groupPosition = 0;
    WindowKey preWindowKey = null;
    for (WindowKey windowKey : listKeys) {
      if (preWindowKey == null) {
        preWindowKey = windowKey;
        ++ groupSplit[groupPosition];
      } else {
        if (preWindowKey.equals(windowKey)) {
          ++ groupSplit[groupPosition];
        } else {
          ++ groupSplit[++groupPosition];
          preWindowKey = windowKey;
      ...
      for (RexOver over : windowMap.get(windowKey)) {
    ...
    for (Ord<Group> window : Ord.zip(groups)) {
      for (Ord<RexWinAggCall> over : Ord.zip(window.e.aggCalls)) {
        ...
        if (name == null || name.startsWith("$")) {
          name = "f" + fieldList.size() + "w" + window.i + "$o" + over.i;
        }
    ...
    for (int i = 0; i < groupSplit.length; i++) {
      ...
      to += groupSplit[i];
      int childInputFieldCount = childRel.getRowType().getFieldCount();
      final List<Group> tempGroups = groups.subList(from,to);
      final List<Group> newGroups = new ArrayList<>();
      final AtomicInteger aggSize = new AtomicInteger();
      AtomicInteger aggCallIndex = new AtomicInteger();
      final RexShuttleCorrectConstants correctConstants = new RexShuttleCorrectConstants(inputFieldCount, childRel.getRowType().getFieldCount() - inputFieldCount);
      tempGroups.stream().forEach(t -> {
        RexWindowBound lowerBound = t.lowerBound.accept(correctConstants);
        RexWindowBound upperBound = t.upperBound.accept(correctConstants);
        aggSize.set(aggSize.get() + t.aggCalls.size());
        final List<RexWinAggCall> newAggCalls = new ArrayList<>();
        for (RexWinAggCall tempAggCall:t.aggCalls) {
          //reajust the index
          RexCall newRexCall = (RexCall) tempAggCall.accept(correctConstants);
          final RexWinAggCall newAggCall =
              new RexWinAggCall(
                  (SqlAggFunction) newRexCall.op,
                  newRexCall.type,
                  toInputRefs(newRexCall.operands),
                  aggCallIndex.getAndIncrement(),
                  tempAggCall.distinct);
      ...
      final RelDataType tempIntermediateRowType =
          cluster.getTypeFactory().createStructType(intermediateRowType.getFieldList().subList(0, childInputFieldCount + aggSize.get()));
      final LogicalWindow window =
          LogicalWindow.create(traitSet, childRel, correctConstants.correctedConstants, tempIntermediateRowType,
              newGroups);
      childRel = window;
    ...
    return relBuilder.push(childRel)
  ...
  /** sorted the window key. */
  private static class WindowSortedKey implements Comparable{
    private final WindowKey windowKey;

    WindowSortedKey(WindowKey windowKey) {
      this.windowKey = windowKey;
    }

    @Override public int hashCode() {
      return Objects.hash(this.windowKey.groupSet, this.windowKey.orderKeys);
    }

    @Override public boolean equals(Object obj) {
      return obj == this
          || obj instanceof WindowKey
          && this.windowKey.groupSet.equals(((WindowSortedKey) obj).windowKey.groupSet)
          && this.windowKey.orderKeys.equals(((WindowSortedKey) obj).windowKey.orderKeys);
    }
