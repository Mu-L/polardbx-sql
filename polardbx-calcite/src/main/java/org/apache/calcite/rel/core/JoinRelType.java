...
public enum JoinRelType {
  INNER, LEFT, RIGHT, FULL, SEMI, ANTI, LEFT_SEMI;
  ...
  /**
   * The Spark Type of join:
   *                 `inner`, `cross`, `outer`, `full`, `full_outer`, `left`, `left_outer`,
   *                 `right`, `right_outer`, `left_semi`, `left_anti`.
   *
   * @return
   */
  public String convertToSparkJoinType() {
  ...
  public <T> T innerSide(T left, T right) {
    return this != RIGHT ? right : left;
  }

  public <T> T leftSide(T outer, T inner) {
    return this != RIGHT ? outer : inner;
  }

  public <T> T rightSide(T outer, T inner) {
    return this != RIGHT ? inner : outer;
  }
}
