...
public class RelMdSelectivity
  ...
  public Double getSelectivity(GroupJoin rel, RelMetadataQuery mq,
                               RexNode predicate) {
    final List<RexNode> notPushable = new ArrayList<>();
    final List<RexNode> pushable = new ArrayList<>();
    RelOptUtil.splitFilters(
    ...
    Double selectivity = mq.getSelectivity(rel.copyAsJoin(rel.getTraitSet(), rel.getCondition()), childPred);
    if (selectivity == null) {
      return null;
    } else {
      RexNode pred =
