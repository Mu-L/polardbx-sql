...
public class RelFactories {
  ...
  public static final ModifyFactory DEFAULT_MODIFY_FACTORY =
      new ModifyFactoryImpl();
  ...
  private static class ProjectFactoryImpl implements ProjectFactory {
   ...
   public RelNode createProject(RelNode input, List<? extends RexNode> childExprs,
        List<String> fieldNames, List<String> originalNames, Set<CorrelationId> var) {
      return LogicalProject.create(input, childExprs, fieldNames, originalNames, var);
    }
  }
  ...
  private static class JoinFactoryImpl implements JoinFactory {
    ...
    public RelNode createJoin(RelNode left, RelNode right,
                              RexNode condition, Set<CorrelationId> variablesSet,
                              JoinRelType joinType, boolean semiJoinDone, SqlNodeList hints) {
      return LogicalJoin.create(left, right, condition, variablesSet, joinType,
          semiJoinDone, ImmutableList.<RelDataTypeField>of(), hints);
    }
  ...
  public interface SemiJoinFactory {
     ...
     * @param condition        Join condition
     */
    RelNode createSemiJoin(RelNode left, RelNode right, RexNode condition, SqlNodeList hints);

    RelNode createSemiJoin(RelNode left, RelNode right, RexNode condition, JoinRelType joinType, List<RexNode> linkRexNode, Set<CorrelationId> variablesSet, SqlNodeList hints, String position);
  }
  ...
  private static class SemiJoinFactoryImpl implements SemiJoinFactory {
    ...
    public RelNode createSemiJoin(RelNode left, RelNode right,
                                  RexNode condition, SqlNodeList hints) {
      final JoinInfo joinInfo = JoinInfo.of(left, right, condition);
      return SemiJoin.create(left, right,
        condition, joinInfo.leftKeys, joinInfo.rightKeys, hints);
    }

    @Override public RelNode createSemiJoin(RelNode left, RelNode right, RexNode condition, JoinRelType joinType,
                                            List<RexNode> linkRexNode, Set<CorrelationId> variablesSet,
                                            SqlNodeList hints, String position) {
      final JoinInfo joinInfo = JoinInfo.of(left, right, condition);
      return LogicalSemiJoin.create(left, right,
              condition, joinInfo.leftKeys, joinInfo.rightKeys, joinType, linkRexNode, variablesSet, hints, position);
    }
  }
  ...
  /**
   * Can create a {@link TableModify} of the appropriate type
   * for this rule's calling convention.
   */
  public interface ModifyFactory {
    /** Creates a filter. */
    RelNode createModify(TableModify modify, RelNode input);
  }
   ...
   * returns a vanilla {@link LogicalFilter}.
   */
  private static class ModifyFactoryImpl implements RelFactories.ModifyFactory {
    public RelNode createModify(TableModify modify, RelNode input) {
      return modify.copy(modify.getTraitSet(), ImmutableList.of(input));
    }
  }
}
