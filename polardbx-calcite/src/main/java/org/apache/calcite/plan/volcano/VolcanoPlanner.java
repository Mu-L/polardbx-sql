...
public class VolcanoPlanner extends AbstractRelOptPlanner {
  ...
  /**
   * Whether to enable startUp-Cost optimization or not.
   */
  boolean startUpCostOpt = false;

  boolean enableBranchAndBound = true;

  boolean enablePassThrough = true;
  ...
  protected boolean isValid(Litmus litmus) {
    ...
    RelMetadataQuery metaQuery = root.getCluster().getMetadataQuery();
...
//    }
  ...
  @Nullable RelNode changeTraitsUsingConverters(
    ...
    for (int i = 0; (converted != null) && (i < toTraits.size()); i++) {
      ...
      if (convertedRel != null) {
        assert convertedRel.getTraitSet().getTrait(traitDef).satisfies(toTrait);
  ...
  void propagateCostImprovements(RelNode rel) {
    ...
    Map<RelNode, RelOptCost> propagateRelsStartUpCost = new HashMap<>();
    ...
    propagateRelsStartUpCost.put(rel, getStartUpCost(rel, mq));
    ...
    while ((relNode = propagateHeap.poll()) != null) {
      ...
      RelOptCost startUpCost = requireNonNull(propagateRelsStartUpCost.get(relNode), "propagateRelsStartUpCost.get"
          + "(relNode)");
      ...
      for (RelSubset subset : getSubsetNonNull(relNode).set.subsets) {
        ...
        if (!cost.isLt(subset.bestCost) && !startUpCost.isLt(subset.bestStartUpCost)) {
          continue;
        }
        ...
        if (startUpCost.isLt(subset.bestStartUpCost)) {
          subset.bestStartUpCost = startUpCost;
          subset.bestStartUpRel = relNode;
        }
        ...
        for (RelNode parent : subset.getParents()) {
          ...
          RelOptCost newStartUpCost = getStartUpCost(parent, mq);
          RelOptCost existingStartUpCost = propagateRelsStartUpCost.get(parent);
          if (existingStartUpCost == null || newStartUpCost.isLt(existingStartUpCost)) {
            propagateRelsStartUpCost.put(parent, newStartUpCost);
            if (!hasPropagate) {
              if (existingStartUpCost != null) {
                // Cost reduced, force the heap to adjust its ordering
                propagateHeap.remove(parent);
              }
              propagateHeap.offer(parent);
...
//        && rel.getConvention() != rootConvention;
    // even if rel instanceof PhysicalNode, we consider it 'isLogical' when its convention != rootConvention
    return rel.getConvention() != rootConvention;
  }
  ...
  protected boolean isTransformationRule(VolcanoRuleCall match) {
    ...
    if (match.getRule().getOperand().trait == Convention.NONE
        || match.getRule().getOperand().trait == null) {
      if (match.getRule().getOutConvention() != null && match.getRule().getOutConvention() != Convention.NONE) {
        return false;
      }
    }
  ...
  public RelOptCost getStartUpCost(RelNode rel, RelMetadataQuery mq) {
    ...
    if (rel instanceof RelSubset) {
      return ((RelSubset) rel).getBestStartUpCost();
    }
  ...
  public void setStartUpCostOpt(boolean startUpCostOpt) {
    this.startUpCostOpt = startUpCostOpt;
  }

  public boolean isStartUpCostOpt() {
    return startUpCostOpt;
  }

  public boolean enableBranchAndBound() {
    if (startUpCostOpt) {
      return false;
    }
    return enableBranchAndBound;
  }

  public void setEnableBranchAndBound(boolean enableBranchAndBound) {
    this.enableBranchAndBound = enableBranchAndBound;
  }
  ...
  public void setEnablePassThrough(boolean enablePassThrough) {
    this.enablePassThrough = enablePassThrough;
  }

  public boolean isEnableDerive() {
    return enableDerive;
  }
