...
public class VolcanoRuleCall extends RelOptRuleCall {
  ...
  private static boolean MATCH_RELSUBSET_AS_PLACE_HOLDER = true;
  ...
  protected void onMatch() {
    ...
    try {
      ...
      LOGGER.error("Error while applying rule " + getRule() + ", args " + Arrays.toString(rels));
      throw e;
    }
  }
  /**
  ...
  private void matchRecurse(int solve) {
    ...
    if (solve == operands.size()) {
      ...
      final int[] solveOrder = operand0.solveOrder;
      ...
      if (ascending) {
        ...
        if (MATCH_RELSUBSET_AS_PLACE_HOLDER && previous instanceof RelSubset) {
          // do not match RelSubSet in ascending direction
        ...
        if (parentOperand.childPolicy == RelOptRuleOperandChildPolicy.UNORDERED) {
          ...
          if (operand.getMatchedClass() == RelSubset.class) {
            ...
            if (MATCH_RELSUBSET_AS_PLACE_HOLDER) {
              // match RelSubSet itself in descending direction
