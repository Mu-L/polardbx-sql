...
public class SqlDialect {
  ...
  /**
   * For {@code DATE '1980-12-20' - INTERVAL '108' DAY(3)}, calcite default convert
   * SqlNode to Sql String with the date precision {@code DAY(3)}, but for MYSQL,
   * it is invalid.
   * @return
   */
  public boolean supportDatePrecision() {
    switch (databaseProduct) {
    case MYSQL:
      return false;
    default:
  ...
  /**
   * For different {@code concat} syntax,
   * MySQL: concat('a', 'b')
   * Oracle: 'a' || 'b'
   */
  public boolean supportConcat() {
    switch (databaseProduct) {
    case MYSQL:
      return true;
    default:
