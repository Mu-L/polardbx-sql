...
public abstract class ListScope extends DelegatingScope {
  ...
  findQualifyingTableNames(String columnName, SqlNode ctx,
    ...
    switch (map.size()) {
    case 0:
      // Note: If found alias inside, never find in parent.
      if (this instanceof SelectScope) {
        final SelectScope scope = (SelectScope)this;
        boolean findAlias = false;
        for (SqlNode s : scope.getNode().getSelectList()) {
          final String alias = SqlValidatorUtil.getAlias(s, -1);
          boolean simpleIdentifier = false;
          boolean selfDefinition = false;
          if (s.getKind() == SqlKind.AS) {
            if (((SqlCall) s).operand(0).toString().equalsIgnoreCase(((SqlCall) s).operand(1).toString())) {
              selfDefinition = true;
            }
          } else if (s instanceof SqlIdentifier) {
            simpleIdentifier = true;
          }
          if (alias != null && nameMatcher.matches(alias, columnName) && !selfDefinition && !simpleIdentifier) {
            findAlias = true;
            break;
          }
        }
